...
public class LoginServer {
	...
	public void read(final int opcode, final LoginServerPacket packet) {//Packets
		...
		case 3: {
			...
			break;
		/*case 4: {
			// final String nameOfSender = packet.readString();
			final String nameOfRecipient = packet.readString();
			final int messageSize = packet.readInt();
			final byte[] message = new byte[messageSize - 8];
			for (int i = 0; i < message.length; i++) {
				message[i] = (byte) packet.readByte();
			}
			final Player recipient = World.getPlayer(nameOfRecipient);
			if (recipient != null) {
//				recipient.getFriendsIgnores().sendMessage(Utils.stringToLong(nameOfRecipient), new ChatMessage(Huffman.readEncryptedMessage(150, stream))); // <<the
																								// problem<<<<<<<<<<<<<<<
			}
		}
			break;
		case 5: {
			final String name = packet.readString();
			final String nameOfFriend = packet.readString();
			final int world = packet.readByte();
			final Player player = World.getPlayer(name);
			Player friend = World.getPlayer(nameOfFriend);
			if (player != null) {
				player.getFriendsIgnores().changeFriendStatus(friend, World.containsPlayer(nameOfFriend));
			}
			System.out.println("Received packet 5");
		}
			break;*/
		}
	}
	...
	public void sendPrivateMessage(final String nameOfSender, final String nameOfRecipient, final byte[] message) {
		final LoginServerPacket packet = new LoginServerPacket(6).writeString(nameOfSender).writeString(nameOfRecipient).writeInt(message.length);
