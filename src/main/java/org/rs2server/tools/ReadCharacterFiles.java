	...
	public void getItems(Player player, int itemId, int amount) throws IOException {
		...
		for (File f : files) {
			try {
				...
				player.getActionSender().sendMessage("Corrupted Account found with name; " + f.getName().replace(".dat.gz", "") + ".");
				continue;
			}
		}
	}
	...
	public void readCharacter(IoBuffer buf, Player player, String name, int itemId, int amount) {
		...
		for (int i = 0; i < Equipment.SIZE; i++) {
			...
			if (id != 65535) {
				...
				if (id == itemId && amt >= amount) {
					player.getActionSender().sendMessage(name + " has " + amt + "x " + (CacheItemDefinition.get(id).getName() == null ? id : CacheItemDefinition.get(id).getName()) + " equipped.");
				}
			}
		}
		...
		for (int i = 0; i < Inventory.SIZE; i++) {
			...
			if (id != 65535) {
				int amt = buf.getInt();
				if (id == itemId && amt >= amount) {
					player.getActionSender().sendMessage(name + " has " + amt + "x " + (CacheItemDefinition.get(id).getName() == null ? id : CacheItemDefinition.get(id).getName()) + " in his Inventory.");
				}
			}
		}
		if (buf.hasRemaining()) {
			for (int i = 0; i < Bank.SIZE; i++) {
				...
				if (id != 65535) {
					int amt = buf.getInt();
					if (id == itemId && amt >= amount) {
						player.getActionSender().sendMessage(name + " has " + amt + "x " + (CacheItemDefinition.get(id).getName() == null ? id : CacheItemDefinition.get(id).getName()) + " in his Bank.");
					}
				}
			}
		}
