	...
	public static void handleSwag() {
		...
		for (final File drop : dir.listFiles()) {
			try (final BufferedReader parse = new BufferedReader(
				...
				lootTable.getStaticDrops().stream().filter(Objects::nonNull).forEach(i -> guaranteedList.add(Loot.of(i.getItemID(), i.getMinAmount(), i.getMaxAmount(), 100)));
				int rolls = lootTable.getRolls();

				List<Loot> low = lootTable.getDynamicDrops().stream().filter(Objects::nonNull).filter(l -> l.getHitRollCeil() >= 20)
						.map(i -> Loot.of(i.getItemID(), i.getMinAmount(), i.getMaxAmount(), i.getHitRollCeil())).collect(toList());
				List<Loot> med = lootTable.getDynamicDrops().stream().filter(Objects::nonNull).filter(l -> l.getHitRollCeil() > 10 && l.getHitRollCeil() < 20)
						.map(i -> Loot.of(i.getItemID(), i.getMinAmount(), i.getMaxAmount(), i.getHitRollCeil())).collect(toList());
				List<Loot> high = lootTable.getDynamicDrops().stream().filter(Objects::nonNull).filter(l -> l.getHitRollCeil() <= 10)
						.map(i -> Loot.of(i.getItemID(), i.getMinAmount(), i.getMaxAmount(), i.getHitRollCeil())).collect(toList());
