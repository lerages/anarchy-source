...
public class WidgetComposite {
	...
	public int anInt798;
//	private static final Map<Integer, RSFont> fonts = new HashMap<>();

	/*public RSFont getWidgetFont() throws IOException {
		mediaUnavailable = false;
		if(this.fontID == -1) {
			return null;
		} else {
			RSFont font = fonts.get(this.fontID);
			if(font == null) {
...
/*	public static RSFont loadFont(int id) throws IOException {
		ByteBuffer buffer = Application.cache.read(13, id, 0);
		if (buffer == null) {
			return null;
		}
		...
		buffer.get(tmp);
		PaletteSprite.decode(ByteBuffer.wrap(tmp), id);
		RSFont font = new RSFont(tmp, PaletteSprite.anIntArray1316, PaletteSprite.anIntArray123, PaletteSprite.lastLoadedPaletteSpriteWidth,  PaletteSprite.anIntArray126, PaletteSprite.anIntArray130, RSFont.loadedCharacterPixels);
		return font;
	}*/
...
//			try {
//				System.out.println(getWidgetFont());
//			} catch (IOException e) {
