...
/*	//maybe a player information popup or something can do that, like a debugger like in bot xd
	*//**
	 * Logger instance.
	 *//*
	...
	public GUI() {
		...
		list.addMouseListener(new MouseAdapter() {
			@Override
			public void mouseClicked(MouseEvent e) {//get pressed too i forget which
		...
		logger.info("Starting Lost-Isle...");
		try {
			new RS2Server().bind(RS2Server.PORT).start();
		} catch(Exception ex) {
			logger.log(Level.SEVERE, "Error starting Lost-Isle.", ex);
			System.exit(1);
		}
		World.getWorld();
	}
