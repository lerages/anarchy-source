...
//					System.out.println(food + ", " + last + ", " + getMob().getCombatState().getEatDelay());
					if (getMob().getCombatState().getEatDelay() == 0) {
						getMob().getCombatState().setCanEat(true);
						getMob().playAnimation(Animation.create(829));
						getMob().getActionSender().playSound(Sound.EATING);
						if (food != Food.PURPLE_SWEETS)
							getMob().getInventory().remove(item, slot);
						/*getMob().getCombatState().setCanEat(delay != 3);
						System.out.println(food + ", " + last + ", " + canEat);
						...
						if (food == Food.PURPLE_SWEETS) {
							...
							getMob().getActionSender().sendMessage("You eat the " + item.getDefinition2().name.toLowerCase() + ".");
						}
						...
						if (food == Food.ANGLERFISH) {
							getMob().getSkills().increaseLevelToSetMaximum(Skills.HITPOINTS, food.getHeal(),
									getMob().getSkills().getLevelForExperience(Skills.HITPOINTS) + Constants.getModification(getMob().getSkills().getLevelForExperience(Skills.HITPOINTS)));
						} else {
					 ...
					 */
					getMob().playAnimation(Animation.create(829));
					getMob().getActionSender().playSound(Sound.DRINKING);
					getMob().getInventory().remove(item, slot);
					 ...
					 * Potion Types.
					 */
					String potionName = item.getDefinition2().name.toLowerCase().substring(0, item.getDefinition2().name.length() - 3).replaceAll(" potion", "");
					switch (drink.getPotionType()) {
						...
						case SUPER_RESTORE:
							...
							for (int i = 0; i < drink.getSkills().length; i++) {
								int skill = drink.getSkill(i);
								int modification = (int) Math.floor(8 + (getMob().getSkills().getLevelForExperience(skill) * 0.25));
								/**
								 * Holy wrench increases prayer restoration.
								 */
								if (skill == Skills.PRAYER) {
