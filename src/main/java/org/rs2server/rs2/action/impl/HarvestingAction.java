...
public abstract class HarvestingAction extends Action {
	...
	public void execute() {
		...
		//execute
		currentCycles = getCycleCount();
		getGameObject().decreaseCurrentHealth(1); //decreases the health by one, e.g. lost a logging
		...
		if(getGameObject().getCurrentHealth() < 1) {
			getGameObject().setCurrentHealth(getGameObjectMaxHealth()); //sets its health back to max for when it respawns
