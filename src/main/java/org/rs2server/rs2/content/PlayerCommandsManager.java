...
public class PlayerCommandsManager {
	...
	public static boolean handlePlayerCommandOptions(Player player, int option) {
		...
		case 8:
			...
			player.getActionSender().sendDialogue(null, DialogueType.MESSAGE, -1, null, "There's currently " + World.getWorld().getPlayers().size() + " players online.");
			player.getActionSender().sendMessage("There's currently " + World.getWorld().getPlayers().size() + " players online.");
			break;
		case 9:
			player.getActionSender().closeAll();
