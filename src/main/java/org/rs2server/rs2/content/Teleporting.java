...
public class Teleporting {
    ...
    /**
     * @param mob   The mob being teleported
     * @param spell The teleport spell
            ...
            if (teleport.equalsIgnoreCase((String) teleports[j][4])) {
                ...
                if (mob.getSkills().getLevel(Skills.MAGIC) < (Integer) teleports[j][2]) {
                    if (mob.getActionSender() != null) {
                        mob.getActionSender().sendMessage("You need a magic level of " + (Integer) teleports[j][2] + " to cast " + teleport);
                        return true;
                    }
				...
				if (spell.getRunes() != null) {
					for (int i = 0; i < spell.getRunes().length; i++) {
						if (mob.getInventory() != null && spell.getRune(i) != null && Inventory.hasItem(mob, spell.getRune(i))) {
...
//									mob.getInventory().remove(spell.getRune(i));
							...
							mob.getActionSender().sendMessage("Not enough runes to cast this spell.");//Not enough runes to complete this Action.
							return false;
						}
					}
				}
                if (teleport.equalsIgnoreCase("duelTeleport") || teleport.equalsIgnoreCase("castlewarsTeleport") ||
                        teleport.equalsIgnoreCase("edgevilleTeleport") || teleport.equalsIgnoreCase("karamjaTeleport") ||
                        teleport.equalsIgnoreCase("draynorTeleport") || teleport.equalsIgnoreCase("alkharidTeleport") && mob.isPlayer()) {
                ...
                if (mob.getCombatState().getSpellBook() == 0 && (SpellBook) teleports[i][0] == SpellBook.MODERN_MAGICS || mob.getCombatState().getSpellBook() == 2 && (SpellBook) teleports[i][0] == SpellBook.LUNAR_MAGICS
                		 || mob.getCombatState().getSpellBook() == 3 && (SpellBook) teleports[i][0] == SpellBook.ARCEUUS_MAGICS) {
                ...
                } else if (mob.getCombatState().getSpellBook() == 1 && (SpellBook) teleports[i][0] == SpellBook.ANCIENT_MAGICKS) {
					mob.setCanBeDamaged(false);
     ...
     * @param tablet The item which is being broken
     * @return True on success.
     */
    public static boolean breakTablet(final Player player, final Item tablet) {
		...
		if (BoundaryManager.isWithinBoundaryNoZ(player.getLocation(), "ClanWarsFFAFull")) {
			player.getActionSender().sendMessage("You can't teleport from here, please use the portal to leave.");
			return false;
		}
        for (int j = 0; j < teleTabs.length; j++) {
            ...
            if (tablet.getId() == teleTabs[i][0]) {
                ...
                player.playSound(Sound.TELE_TAB);
     ...
     *
     * @param player The player who is being effected
     * @param item   The item they are preparing to use.
     ...
     *
     * @param player The player whos item is being degraded
     */
    ...
    /**
     * @param id The id we are testing
...
case 3867:
    */
    ...
    private final static int[][] teleTabs = {
            {8007, 3213, 3424},
            {8008, 3222, 3218},
            {8009, 2965, 3379},
            {8010, 2757, 3477},
