...
public class Jewellery {
	...
	private static Object[][] SKILLS_DATA = {
			{11111, 11113, "You use your necklace's last charge."},
			{11109, 11111, "Your necklace has one charge left."},
			{11107, 11109, "Your necklace has two charges left."},
			{11105, 11107, "Your necklace has three charges left."},
			{11970, 11105, "Your necklace has four charges left."},
			{11968, 11970, "Your amulet has five charges left."},
	};
	
	private static Object[][] GLORY_DATA = {
			{1706, 1704, "You use your amulet's last charge."},
	...
	private static Object[][] SLAYER_RING_DATA = {
			{11873, 4155, "Your slayer ring crumbles to dust."},
	...
	public static boolean rubItem(Player player, int slot, int itemId, boolean operating) {
		if (player.getRFD().isStarted() | FightCave.IN_CAVES.contains(player) ||
				BoundaryManager.isWithinBoundaryNoZ(player.getLocation(), "PestControl") || BoundaryManager.isWithinBoundaryNoZ(player.getLocation(), "PestControlBoat")) {
		...
		if (!operating || slot == Equipment.SLOT_AMULET) {
			...
			if (itemId < 11113 && itemId >= 11105) {
					...
					if(itemId == 11111)
						charges = 1;
					DialogueManager.openDialogue(player, 11105);
					player.getJewellery().setGem(GemType.SKILLS_NECKLACE, charges, operating);
					return true;
				
			} else if (itemId == 11968 || itemId == 11970) {
				int charges = itemId == 11968 ? 5 : 6;
				DialogueManager.openDialogue(player, 11105);
				player.getJewellery().setGem(GemType.SKILLS_NECKLACE, charges, operating);
					return true;
	...
	/**
	 * Notice: This may be set, without being used at all. (Eg. if the player clicks rub, but then clickes the minimap).
	 * Sets how many charges the glory have left, so we can display the correct message.
	 *
	 * @param gemId       The id of the specific gem, Glory = 1, Ring of Dueling = 2, Games Necklace = 3.
	 * @param charge      The charge our gem have left. :)
	 * @param isOperating true if you're operating the gem, false if not.
	 */
	 ...
	 *
	 * @param player   The player, who's using a gem for teleporting.
	 * @param location The location of where to go. (Edgewille, Castle wars, Duel arena).
	 */
	public void gemTeleport(final Player player, final Location location) {
		...
		switch (gem) {			
			...
			/*
			 * Player is using a skills necklace.
			 */
			case SKILLS_NECKLACE:
				...
				Object[] data10 = SKILLS_DATA[gemCharge - 1];
				if (!con.replace((Integer) data10[0], (Integer) data10[1])) {
					return;
				}
				player.getActionSender().sendMessage("<col=7f00ff>" + (String) data10[2]);
				break;
			/*
