...
public class HerbBox {
    ...
    private static final Item HERB_BOX = new Item(11738, 1);
    ...
    public Optional<List<Item>> getHerbRewards() {
        if (!player.getInventory().hasItem(HERB_BOX)) {
        ...
        outer:
        while(rewards.size() < 10) {
            for (Item i : shuffled) {
                if (rewards.size() >= 10) {
                    break outer;
                }

                if (random.nextInt(100) <= HERB_REWARDS.get(i)) {
                    rewards.add(i);
                }
            }
        }
