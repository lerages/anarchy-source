...
public class SpiderWeb {
	...
	public static boolean slash(Player player, GameObject obj) {
		...
		// so pretty much an optional is like, something that returns a object containing a value, or just an object containing no value, rather than null.
		// so this is where we use that isPresent method
		...
		/*	for (int i = 0; i < player.getInventory().size(); i++) {
			...
			if (containsSharpObject(player.getInventory().get(i).getDefinition2().getName())) {
				requiredWep = true;
			}
		}*/
		if (!requiredWep) {
