...
public class DragonfireShield {
	...
	public static void empty(final Player player) {
		...
		World.getWorld().submit(new Tickable(1) {
			public void execute() {
				player.playGraphics(Graphic.create(1168)); this.stop();
	...
	public static void dfsSpec(final Player player, final Mob victim) {
		...
		if(player.dfsWait > 0) {
			player.getActionSender().sendMessage("Let your dragonfire shield cool down for " + player.dfsWait + " more seconds.");
			return;
		}
		player.dfsCharges--;
		player.dfsWait = 240;
