...
public class Magic {
	...
	public void handleTeleGrab(GroundItemService.GroundItem item) {
		if (player.getSkills().getLevelForExperience(Skills.MAGIC) < 33) {
			player.getActionSender().sendMessage("You need a Magic level of 33 to use this spell.");
			return;
		}
		...
		if (player.hasAttribute("lastTeleGrab") && System.currentTimeMillis() - (long) player.getAttribute("lastTeleGrab") < 4000) {
			return;
		}
		int clientSpeed;
		int gfxDelay;
		if(player.getLocation().distance(item.getLocation()) <= 1) {
			clientSpeed = 70;
			gfxDelay = 80;
		} else if(player.getLocation().distance(item.getLocation()) >= 1 && player.getLocation().distance(item.getLocation()) <= 5) {
			clientSpeed = 90;
			gfxDelay = 100;
		} else if(player.getLocation().distance(item.getLocation()) >= 5 && player.getLocation().distance(item.getLocation()) <= 8) {
			clientSpeed = 110;
			gfxDelay = 120;
		} else {
			clientSpeed = 130;
		...
		Projectile projectile = Projectile.create(player.getLocation(), item.getLocation(), TELEGRAB_PROJECTILE, 45, 50, clientSpeed, 43, 0, 10, 48);
		...
		World.getWorld().submit(new StoppingTick(delay) {
			...
			public void executeAndStop() {
				Optional<GroundItemService.GroundItem> groundItemOptional = groundItemService.getGroundItem(item.getItem().getId(), item.getLocation());
				if (groundItemOptional.isPresent()) {
					World.getWorld().getRegionManager().getRegionByLocation(groundItemOptional.get().getLocation()).
							getPlayers().stream().filter(Objects::nonNull).forEach(i -> i.getActionSender().sendStillGFX(144, 0, groundItemOptional.get().getLocation()));
				...
				switch(spellId) {
					...
					case 23:
						if (player.getSkills().getLevel(Skills.MAGIC) < 41) {
							player.getActionSender().sendMessage("You need a Magic level of 41 to cast this spell.");
							return;
						}
						if (!staffService.containsRune(player, new Item(561)) || !staffService.containsRune(player, new Item(554, 4))) {
							player.getActionSender().sendMessage("You do not have the required runes to cast this spell.");
							return;
						}
						boolean success = false;
						for (final SmithingUtils.SmeltingBar bar : SmithingUtils.SmeltingBar.values()) {
//							if (bar.getItemsRequired()[0].getId() == 440 && player.getInventory().getCount(453) >= 2) {
//								player.getActionQueue().addAction(new SuperheatItemAction(player, SmithingUtils.SmeltingBar.STEEL, 1));
//								success = true;
//								break;
//							}
							if (bar.getItemsRequired()[0].getId() == item.getId()) {
								SmithingUtils.SmeltingBar smeltingBar = bar;
								if (smeltingBar.getItemsRequired()[0].getId() == 440 && player.getInventory().getCount(453) >= 2) {
				...
				case 11:
					...
					if (!staffService.containsRune(player, new Item(561)) || !staffService.containsRune(player, new Item(554, 3))) {
						player.getActionSender().sendMessage("You do not have the required runes to cast this spell.");
						return;
					}
					if (item.getId() == 995) {
						player.getActionSender().sendMessage("You cannot convert gold into gold.");
						return;
					}
					...
					if (item.getDefinition().isNoted()) {
						...
						if (price <= 0) {
							player.getActionSender().sendMessage("Error occured price was less than or equal to 0, Please report to a staff member.");
							return;
						}
					}
					if (price <= 0) {
				...
				case 32:
					...
					if (!staffService.containsRune(player, new Item(561)) || !staffService.containsRune(player, new Item(554, 5))) {
						player.getActionSender().sendMessage("You do not have the required runes to cast this spell.");
						return;
					}
					if (item.getId() == 995) {
					...
					player.getSkills().addExperience(Skills.MAGIC, 65); // 3* 195
					player.getInventory().remove(slot, new Item(item.getId()));
					player.getInventory().add(new Item(995, price));
					player.getActionSender().switchTab(4);
					staffService.removeRune(player, new Item(561));
				...
				case 3:
					if (player.getSkills().getLevel(Skills.MAGIC) < 7) {
						player.getActionSender().sendMessage("You need a Magic level of 7 to cast this spell.");
						return;
					}
					if (!staffService.containsRune(player, new Item(555)) || !staffService.containsRune(player, new Item(564))) {
						player.getActionSender().sendMessage("You do not have the required runes to cast this spell.");
						return;
					}
					...
					if (item.getId() == 11072) { // sapphire bracelet
						...
						player.getSkills().addExperience(Skills.MAGIC, 50);
				...
				case 14:
					if (player.getSkills().getLevel(Skills.MAGIC) < 27) {
						player.getActionSender().sendMessage("You need a Magic level of 27 to cast this spell.");
						return;
					}
					if (!staffService.containsRune(player, new Item(556, 3)) || !staffService.containsRune(player, new Item(564))) {
						player.getActionSender().sendMessage("You do not have the required runes to cast this spell.");
						return;
					}
				...
				case 26:
					if (player.getSkills().getLevel(Skills.MAGIC) < 49) {
						player.getActionSender().sendMessage("You need a Magic level of 49 to cast this spell.");
						return;
					}
					if (!staffService.containsRune(player, new Item(554, 5)) || !staffService.containsRune(player, new Item(564))) {
						player.getActionSender().sendMessage("You do not have the required runes to cast this spell.");
						return;
					}
				...
				case 34:
					if (player.getSkills().getLevel(Skills.MAGIC) < 57) {
						player.getActionSender().sendMessage("You need a Magic level of 57 to cast this spell.");
						return;
					}
					if (!staffService.containsRune(player, new Item(557, 10)) || !staffService.containsRune(player, new Item(564))) {
						player.getActionSender().sendMessage("You do not have the required runes to cast this spell.");
						return;
					}
				...
				case 49:
					...
					if (!staffService.containsRune(player, new Item(555, 15)) || !staffService.containsRune(player, new Item(564)) || !staffService.containsRune(player, new Item(557, 15))) {
						player.getActionSender().sendMessage("You do not have the required runes to cast this spell.");
						return;
					}
					...
					if (item.getId() == 1664) { // Dragonstone necklace
						...
						player.getInventory().remove(slot, new Item(item.getId()));
						player.getInventory().add(new Item(11105));
						player.getActionSender().switchTab(4);
						staffService.removeRune(player, new Item(555, 15));
						staffService.removeRune(player, new Item(557, 15));
						staffService.removeRune(player, new Item(564));
				...
				case 61:
					if (player.getSkills().getLevel(Skills.MAGIC) < 87) {
						player.getActionSender().sendMessage("You need a Magic level of 87 to cast this spell.");
						return;
					}
					if (!staffService.containsRune(player, new Item(554, 20)) || !staffService.containsRune(player, new Item(564)) || !staffService.containsRune(player, new Item(557, 20))) {
						player.getActionSender().sendMessage("You do not have the required runes to cast this spell.");
						return;
					}
				...
				case 63:
					...
					break;
				default: System.out.println("Unhandled magic on item spell: " + spellId + ".");
				}
				break;
			}
		}
