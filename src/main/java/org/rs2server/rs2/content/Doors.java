...
public class Doors {
	...
	public static boolean manageDoor(GameObject door) {
		...
		if (door.getId() == 11726 || (door.getId() >= 26502 && door.getId() <= 26505) || door.getId() == 7179 || door.getId() == 7182 || door.getId() == 14234 || door.getId() == 14236 || door.getId() == 14235 || door.getId() == 14233 || door.getId() == 7407 || door.getId() == 7408 || door.getId() == 26760 || door.getId() == 20925) {
			return false;
		}
		if (door.getId() == 24309 && (door.getLocation().equals(Location.create(2847, 3541, 2))) || door.getId() == 24306 && (door.getLocation().equals(Location.create(2847, 3540, 2)))) {
			return false;
		}
		...
		/*GameObject second = getSecondDoor(door);
		if (second != null) {
			boolean open = (door.getId() % 2) != 0;
			int rotation = getRotation(door);
			int dir = open ? door.getDirection() : rotation;
			...
			Location l = door.getLocation().transform((int) cardinality.getFaceVector().getX(), (int) cardinality.getFaceVector().getY(), 0);

			GameObject replacement = new GameObject(l, door.getId() + (open ? 1 : -1), 0, open ? rotation : ((cardinality.getFace() + 3) % 4), false);
		...
		World.getWorld().unregister(door, true);
		RegionClipping.removeClipping(door);
//        System.out.println(d.object.getId() + ", " + d.object.getLocation() + ", " + d.currentX + ", " + d.currentY + ", " + getOpposite(d.object.getId()));
		GameObject object = new GameObject(Location.create(d.currentX, d.currentY, door.getZ()), getOpposite(door.getId()), door.getType(), d.currentRotation, false);
		World.getWorld().register(object);
		RegionClipping.addClipping(object);
		return true;
	}

	private static void handleDoubleDoor(GameObject first, GameObject second) {
		openDoubleDoor(first, second);
		openDoubleDoor(second, first);
	}

	private static void openDoubleDoor(GameObject door, GameObject second) {
		Doors d = getDoor(door);
		if (d == null)
			return;
		int xAdjustment = 0;
