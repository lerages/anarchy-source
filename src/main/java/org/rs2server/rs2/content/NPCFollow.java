...
public class NPCFollow {
	...
	public static boolean walkTowards(NPC npc, Mob other, Location target) {
		...
		/*
		 * We get the closest of our own surrounding coordinates, to the
		 * target.
		 */
							...
							for(int yOffset1 = 1; yOffset1 < npc.getSize(); yOffset1++) {
								for(int xOffset2 = 1; xOffset2 < 1; xOffset2++) {
									for(int yOffset2 = 1; yOffset2 < 1; yOffset2++) {
										if(e.getLocation().getX() + xOffset2 == closest.getX() + xOffset1 && e.getLocation().getY() + yOffset2 == closest.getY() + yOffset1) {
				...
				if(!clip) {
					...
					for(GameObject o : r.getGameObjects()) {
						if(o.getType() != 22) {
							...
							/*
							 * If the size is bigger than one (TzTokJad, Hill Giants etc)
							 * we do a few nasty for loops..
							 */
							if(o.getLocation().getZ() == npc.getLocation().getZ()) {
								for(int xOffset = 1; xOffset < npc.getSize(); xOffset++) {
									for(int yOffset = 1; yOffset < npc.getSize(); yOffset++) {
										if(o.getLocation().getX() == closest.getX() + xOffset && o.getLocation().getY() == closest.getY() + yOffset) {
											//System.out.println("Attempting to clip: " + npc + " " + o);
											clip = true;
											break;
										}
...
//			npc.getWalkingQueue().finish();
	...
	public static Location[] getValidSpots(int size, Location location) {
		...
		int index = 0;
		list[index++] = (Location.create(location.getX() - 1, location.getY() - 1, location.getZ()));
		list[index++] = (Location.create(location.getX() + size, location.getY() - 1, location.getZ()));
		list[index++] = (Location.create(location.getX() + size, location.getY() + size, location.getZ()));
		list[index++] = (Location.create(location.getX() - 1, location.getY() + size, location.getZ()));
		for(int i = 0; i < size; i++) {			
			list[index++] = (Location.create(location.getX() - 1, location.getY() + i, location.getZ()));
			list[index++] = (Location.create(location.getX() + i, location.getY() - 1, location.getZ()));
			list[index++] = (Location.create(location.getX() + i, location.getY() + size, location.getZ()));
			list[index++] = (Location.create(location.getX() + size, location.getY() + i, location.getZ()));
		}
		return list;
	}
