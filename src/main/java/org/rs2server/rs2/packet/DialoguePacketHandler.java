...
public class DialoguePacketHandler implements PacketHandler{
	...
	public static final Class<?> getQuestByNpc(NPC npc) {
		switch(npc.getId()) {
		case 1158:
			//return LostCity.class;
		case 684:
		case 1902:
			return DesertTreasure.class;
		case 4626:
			return CooksAssistant.class;
		case 3855:
		case 3839:
	...
	public void handle(Player player, Packet packet) {
		...
		if (player.getInterfaceState().getChatboxInterface() == 94 && interfaceId == 0 && option == -1) {
			if (player.getInterfaceState().getDestroyItemId() > 0) {
				if (child == 4) { // Yes, destroy item
					...
					hookService.post(new GameItemInventoryActionEvent(player, GameItemInventoryActionEvent.ClickType.DESTROY, new Item(player.getInterfaceState().getDestroyItemId(), 1), -1));
				} else { // No, cancel
		...
		//System.out.println("Option: " + option + ", Interface: " + interfaceId + ", Child: " + child);
		if (DialogueSkillManager.handleSkillAction(player, child)) {
			return;
		} 
		else if (TeleportManager.handleTeleportAction(player, option)) {
			return;
		} else {

			if (option > 0) {
				DialogueManager.advanceDialogue(player, (option - 1));//option - 1
			} else {
