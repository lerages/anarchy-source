...
public class PlayerOptionPacketHandler implements PacketHandler {
	...
	private static final int OPTION_ATTACK = 250, OPTION_CHALLENGE = 39, OPTION_2 = 111, OPTION_3 = 239, OPTION_4 = 247, OPTION_5 = 65, OPTION_SPELL = 150, ITEM_ON_PLAYER = 170;
	...
	private void itemOnPlayer(Player player, Packet packet) {
			...
			public void execute() {
				switch (interfaceId) {
					case Inventory.INTERFACE:
						...
						switch (item.getId()) {
							case 13190:
								if (recipient.isIronMan() || recipient.isHardcoreIronMan()) {
									player.getActionSender().sendMessage("The other player is a Iron Man and cannot receive this item.");
									return;
								}
								DialogueManager.openDialogue(player, 13193);
								player.setAttribute("otherPlayer", recipient);
								break;
						}
						break;
				}
		...
		player.addCoordinateAction(player.getWidth(),
				player.getHeight(), recipient.getLocation(), recipient.getWidth(), recipient.getHeight(), 1, action);
	...
	private void optionAttack(final Player player, Packet packet) {
		...
		if (BoundaryManager.isWithinBoundaryNoZ(player.getLocation(), "DuelArena") &&
			...
			Action action = new Action(player, 0) {
				...
				@Override
				public void execute() {
					player.getRequestManager().request(RequestType.DUEL, victim);
					this.stop();
				}
			};
			pathfindingService.travelToPlayer(player, victim);
			player.addCoordinateAction(player.getWidth(), player.getHeight(), victim.getLocation(), 1, 1, 1, action);
			return;
		}
...
//			if (distance > 1) {
		player.addCoordinateAction(player.getWidth(),
				player.getHeight(), other.getLocation(), other.getWidth(), other.getHeight(), 1, action);
		pathfindingService.travelToPlayer(player, other);
	...
	private void option3(Player player, Packet packet) {
		...
		if (target != null && target != player) {
			...
			if (player.getInterfaceState().isInterfaceOpen(Bank.BANK_INVENTORY_INTERFACE) || player.getInterfaceState().isInterfaceOpen(Trade.TRADE_INVENTORY_INTERFACE) || player.getInterfaceState().isInterfaceOpen(Shop.SHOP_INVENTORY_INTERFACE) || player.getInterfaceState().isInterfaceOpen(PriceChecker.PRICE_INVENTORY_INTERFACE)) {
//                player.getActionSender().removeInterfaces(player.getAttribute("tabmode"), ((int) player.getAttribute("tabmode") == 548 ? 59 : 53));
				player.getActionSender().removeInventoryInterface();
	 ...
	 * @param packet
	 */
	private void option4(Player player, Packet packet) {//not actually trade but ok.
		int id = packet.getShortA() & 0xFFFF;
		if (id < 0 || id >= Constants.MAX_PLAYERS) {
			return;
		}
	...
	private void option5(Player player, Packet packet) {
		...
		if (target != null && target != player) {
			...
			action = new Action(player, 0) {
				...
				@Override
				public void execute() {
					//player.getRequestManager().request(RequestType.DICE, target);
					this.stop();
				}
			};
//			if (distance > 1) {
			player.addCoordinateAction(player.getWidth(),
					player.getHeight(), target.getLocation(), target.getWidth(), target.getHeight(), 1, action);
...
//				if(!BoundaryManager.isWithinBoundaryNoZ(player.getLocation(), "MultiCombat") || !player.inMulti()) {
//					player.getActionSender().sendMessage("You must be in a multi area to cast that spell.");
//					return;					
//				}
