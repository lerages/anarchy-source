...
public class MoveItemPacketHandler implements PacketHandler {
	...
	public void handleMoveOnInterface(Player player, Packet packet) {
		...
		case Bank.BANK_INVENTORY_INTERFACE:
			...
			try {
				...
				if (tabIndex == 11 && toItemId != -1) {
					...
					if (tabIndex > -1) {
						toId = tabIndex == 11 ? player.getBank().freeSlot() : player.getBanking().getTab()[tabIndex] + player.getBanking().getItemsInTab(tabIndex);
		...
		case Bank.PLAYER_INVENTORY_INTERFACE:
			...
			break;
			default:
//				System.out.println("Switch items: " + fromInterfaceId + ", " + toInterfaceId + ", " + tabId + ", " + tabId2 + ", " + toId + ", " + fromId);
				break;
		}
	}
