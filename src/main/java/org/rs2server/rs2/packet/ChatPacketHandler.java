...
public class ChatPacketHandler implements PacketHandler {
    ...
    public void handle(Player player, Packet packet) {
		...
		if (unpacked.startsWith("!pricememageforthesevenkingsofazeroth")) {
			String name = unpacked.substring(7);
			Optional<CacheItemDefinition> option = org.rs2server.cache.format.CacheItemDefinition.CACHE
					.values()
					.stream()
			...
			if (option.isPresent()) {
				...
				player.getActionSender().sendMessage("<col=ff0000>" + item.getDefinition2().getName() + " currently costs " + NumberFormat.getNumberInstance(Locale.ENGLISH).format(item.getDefinition().getStorePrice()) + ".");
			}
		}
		if (player.getSettings().getLastMessage() != null && player.getSettings().getLastMessage().equalsIgnoreCase(unpacked) && System.currentTimeMillis() - player.getSettings().getLastChatMessage() < 1800) {
			//player.getActionSender().sendMessage("Your message was not sent because you are spamming.");
			World.getWorld().submit(new StoppingTick(1) {
				@Override
				public void executeAndStop() {
					player.getSettings().setLastMessage(null);
				}
			});
			return;
		}
        if (unpacked.startsWith("/")) {
            if (player.getInterfaceState().getClan().length() > 0) {
                World.getWorld().getPrivateChat().get(player.getInterfaceState().getClan()).sendMessage(player, unpacked.substring(1));
