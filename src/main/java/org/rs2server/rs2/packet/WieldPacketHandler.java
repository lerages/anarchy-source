				...
				if (slot >= 0 && slot < Inventory.SIZE) {
					...
					if (item != null && item.getId() == id) {
						if (item.getId() >= 5509 && item.getId() <= 5515) {
							hookService.post(new GameItemInventoryActionEvent(player, GameItemInventoryActionEvent.ClickType.WIELD_OPTION, item, slot));
							return;
						}
						...
						if ((maxCapes != null || maxHoods != null)
							...
							player.getActionSender().sendMessage("You need a total level of " + Constants.MAX_LEVEL + " to equip this.");
							return;
						}
						if (player.hasQueuedSwitching()) {
							player.getWeaponSwitchQueue().add(slot);
						} else {
							new WieldItemAction(player, item.getId(), slot, 0).execute();// this should work then if ur not on queued? this code should still work..
						}
//					if (player.hasQueuedSwitching()) {
//						player.getWeaponSwitchQueue().add(slot);
//					}
