...
/*    static {
        info = new WorldDef[12];//change this to noob
        info[0] = new WorldDef(1, 1, "localhost");//2
        info[1] = new WorldDef(2, 1024, "localhost"); // 1024 is high risk
        info[2] = new WorldDef(3, 4, "localhost");
        info[3] = new WorldDef(4, 8, "localhost");//8
        info[4] = new WorldDef(5, 16, "localhost");
        info[5] = new WorldDef(6, 32, "localhost");
        info[6] = new WorldDef(7, 64, "localhost");
        info[7] = new WorldDef(8, 128, "localhost");
        info[8] = new WorldDef(9, 256, "localhost");
        info[9] = new WorldDef(10, 512, "localhost");
        info[10] = new WorldDef(11, 2048, "localhost");
        info[11] = new WorldDef(12, 4096, "localhost"); // doubt it goes bigger than this but idk
    }*/
    ...
    public static void main(String[] s) throws InterruptedException{
        WorldList.listen();//noob restarted the server lol its laready running lol
    ...
    protected static byte[] block() {
        ...
        for (WorldDef w : info) {
            ...
            pb.putRS2String("OS-Anarchy");
            pb.put((byte)2); // country - idx of the countries array client sided
            pb.putShort(World.getWorld().getPlayers().size()); // player count   World.getWorld().getPlayers().size()
        	}
        	if (w.id == 2) {
                pb.putShort(2);//w.id
                pb.putInt(w.mask);
                pb.putRS2String(w.ip);
                pb.putRS2String("Zenyte OS");
                pb.put((byte)2); // country - idx of the countries array client sided
                pb.putShort(World.getWorld().getPlayers().size()); // player count    World.getWorld().getPlayers().size()
            	}
        	if (w.id == 3) {
                ...
                pb.putInt(w.mask);
                pb.putRS2String(w.ip);
                pb.putRS2String("OFFLINE");//OS-Anarchy Beta
                pb.put((byte)2); // country - idx of the countries array client sided
                pb.putShort(0); // player count World.getWorld().getPlayers().size()
            	}	
        }
