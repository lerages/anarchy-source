...
/**
 * Defines the farming tools storable in the tool store (accessible via Tool Leprechauns).
 *
 * @author tommo
 */
public enum FarmingTool {
	...
	public static FarmingTool forInventoryActionButtonId(int inventoryActionButtonId) {
		return Arrays.stream(values()).filter(t -> t.getInventoryActionButtonId() == inventoryActionButtonId).findFirst().orElse(null);
	}

	public static FarmingTool forStoreActionButtonId(int storeActionButtondId) {
		return Arrays.stream(values()).filter(t -> t.getStoreActionButtonId() == storeActionButtondId).findFirst().orElse(null);
	}
}
