...
import org.rs2server.rs2.domain.service.api.content.trade.TradeContainer;
...
class DiceGameTransaction {
	...
	public final void complete(Player winner) {// TODO Capacity checks and implement accept button
		for (Item item : playerContainer) {
			if (winner.getInventory().getCount(item.getId()) + item.getCount() < 0) {
				groundItemService.createGroundItem(winner, new GroundItemService.GroundItem(item, winner.getLocation(), winner, false));
			}
