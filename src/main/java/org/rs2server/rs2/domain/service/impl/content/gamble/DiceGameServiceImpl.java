...
public class DiceGameServiceImpl implements DiceGameService {
	...
	private static final Access TRADE_ACCESS_SELF = Access.of(TRADE_WIDGET, LEFT_TRADE_COMPONENT, NumberRange.of(0, COMPONENT_CAPACITY), OPTIONS_ACCESS);
	private static final Access TRADE_ACCESS_OTHER = Access.of(TRADE_WIDGET, RIGHT_TRADE_COMPONENT, NumberRange.of(0, COMPONENT_CAPACITY), OPTIONS_ACCESS);

	private static final Access TRADE_INVENTORY_ACCESS = Access.of(TRADE_INVENTORY_WIDGET, 0, NumberRange.of(0, COMPONENT_CAPACITY), OPTIONS_ACCESS);

	private static final Object[] TRADE_PARAMETERS_2 = new Object[]{"", "", "", "", "Remove-X", "Remove-All", "Remove-10", "Remove-5", "Remove", -1, 0, 7, 4, 81, 335 << 16 | 24};
	...
	@Inject
	DiceGameServiceImpl(HookService service, PermissionService permissionService, GroundItemService groundItemService, PlayerService playerService) {
	...
	public final void onLogout(@Nonnull GamePlayerLogoutEvent event) {
		...
		if (transaction != null && !player.getInterfaceState().isInterfaceOpen(DICE_WIDGET) && !transaction.isDueling()) {
	...
	public final void onDiceRequest(@Nonnull GameDiceRequestEvent tradeRequest) {
		...
		if (permissionService.isNot(trader, PermissionService.PlayerPermissions.ADMINISTRATOR) && permissionService.isNot(partner, PermissionService.PlayerPermissions.ADMINISTRATOR) &&
				(permissionService.is(trader, PermissionService.PlayerPermissions.IRON_MAN) || permissionService.is(partner, PermissionService.PlayerPermissions.IRON_MAN))) {
	...
	public final void onWidgetClick(@Nonnull GameInterfaceButtonEvent offerEvent) {
		if (offerEvent.getInterfaceId() == CONFIRMATION_WIDGET && offerEvent.getPlayer().getInterfaceState().isInterfaceOpen(CONFIRMATION_WIDGET)) {
			...
			switch (offerEvent.getButton()) {
				case SECOND_ACCEPT_BUTTON:
					...
					transaction.get(player).get().getPlayer().getActionSender().sendString(CONFIRMATION_WIDGET, SECOND_WAITING_CHILD, "Waiting for other player...");
					transaction.getOther(player).get().getPlayer().getActionSender().sendString(CONFIRMATION_WIDGET, SECOND_WAITING_CHILD, "Other player has accepted");
					break;
				case SECOND_DECLINE_BUTTON:
					endTransaction(transaction, true);
					break;
			}
		} else if (offerEvent.getInterfaceId() == TRADE_INVENTORY_WIDGET && offerEvent.getPlayer().getInterfaceState().isInterfaceOpen(TRADE_WIDGET)) {
			...
			if (tradeOption.isPresent() && partnerOption.isPresent()) {
				...
				if (!item.getDefinition().isTradable() && permissionService.isNot(player, PermissionService.PlayerPermissions.ADMINISTRATOR) && permissionService.isNot(partnerOption.get().getPlayer(), PermissionService.PlayerPermissions.ADMINISTRATOR)) {
				...
				if (menuIndex == 10) {
					player.getInterfaceState().openEnterAmountInterface(TRADE_INVENTORY_WIDGET, offerEvent.getChildButton(), item.getId());
					return;
				}

				transaction.add(tradeOption.get(), new Item(item.getId(), offerAmount));
			}
		} else if (offerEvent.getInterfaceId() == TRADE_WIDGET && offerEvent.getPlayer().getInterfaceState().isInterfaceOpen(TRADE_WIDGET)) {
			...
			if (tradeOption.isPresent()) {
				...
				if (offerEvent.getButton() == ACCEPT_BUTTON) {
					...
					openConfirmationWidget(transaction);
					container.getPlayer().getActionSender().sendString(TRADE_WIDGET, WAITING_CHILD, "Waiting for other player...");
					transaction.getOther(container.getPlayer()).get().getPlayer().getActionSender().sendString(TRADE_WIDGET, WAITING_CHILD, "Other player has accepted");
					return;
				}
				...
				if (menuIndex == 10) {
					player.getInterfaceState().openEnterAmountInterface(TRADE_WIDGET, offerEvent.getChildButton(), item.getId());
					return;
				}
	...
	public void openGameInterface(@Nonnull DiceGameTransaction transaction) {
		...
		if (!trader.isEnteredPinOnce() && trader.getDatabaseEntity().getPlayerSettings().isBankSecured()) {
			trader.getActionSender().sendMessage(partner.getName() + " attempted to start a transaction with you, but you need to enter your bank pin at the bank first.");
			return;
		}
		if (!partner.isEnteredPinOnce() && partner.getDatabaseEntity().getPlayerSettings().isBankSecured()) {
			partner.getActionSender().sendMessage(trader.getName() + " attempted to start a transaction with you, but you need to enter your bank pin at the bank first.");
			return;
		}
			...
			public void executeAndStop() {
				...
				if (finalPlayerARoll1 > finalPlayerBRoll1) {
					...
					if (winner != null) {
						...
						partner.getActionSender().sendString(DICE_WIDGET, DICE_WINNER_CHILD, winner.getName() + " has won!");
						...
						transaction.getPlayerContainer().stream().filter(Objects::nonNull).forEach(i -> groundItemService.createGroundItem(null, new GroundItemService.GroundItem(playerLocation, i, playerName, false, false, true)));
						transaction.getPartnerContainer().stream().filter(Objects::nonNull).forEach(i -> groundItemService.createGroundItem(null, new GroundItemService.GroundItem(playerLocation, i, playerName, false, false, true)));
					}
				} else {
					Player winner = playerService.getPlayer(partnerName);
					if (winner != null) {
						winner.getActionSender().sendString(DICE_WIDGET, DICE_WINNER_CHILD, "You have won!");
						player.getActionSender().sendString(DICE_WIDGET, DICE_WINNER_CHILD, winner.getName() + " has won!");
						...
						player.getActionSender().sendMessage(partnerName + " has won the Dice duel.");
						transaction.getPlayerContainer().stream().filter(Objects::nonNull).forEach(i -> groundItemService.createGroundItem(null, new GroundItemService.GroundItem(partnerLocation, i, partnerName, false, false, true)));
						transaction.getPartnerContainer().stream().filter(Objects::nonNull).forEach(i -> groundItemService.createGroundItem(null, new GroundItemService.GroundItem(partnerLocation, i, partnerName, false, false, true)));
					}
				}
	...
	public void setAvailableInventorySpace(@Nonnull DiceGameTransaction transaction) {
		Player trader = transaction.getPlayerContainer().getPlayer();
		Player partner = transaction.getPartnerContainer().getPlayer();
		trader.getActionSender().sendString(TRADE_WIDGET, FREE_INVENTORY_SLOTS_COMPONENT, partner.getName() + " has " + partner.getInventory().freeSlots() + "<br> free inventory slots.");
		partner.getActionSender().sendString(TRADE_WIDGET, FREE_INVENTORY_SLOTS_COMPONENT, trader.getName() + " has " + trader.getInventory().freeSlots() + "<br> free inventory slots.");
		Optional<Integer> priceOption = transaction.getPlayerContainer().stream().filter(i -> i != null && i.getDefinition() != null).map(i -> i.getDefinition().getStorePrice() * i.getCount())
				.reduce((a, b) -> a + b);

		Optional<Integer> partnerPriceOption = transaction.getPartnerContainer().stream().filter(i -> i != null && i.getDefinition() != null).map(i -> i.getDefinition().getStorePrice() * i.getCount())
				.reduce((a, b) -> a + b);
		...
		String otherFormattedPrice = partnerPrice < 0 ? "A lot of" : NumberFormat.getNumberInstance(Locale.ENGLISH).format(partnerPrice);
		trader.getActionSender().sendString(TRADE_WIDGET, PLAYER_CONTAINER_PRICE, "You offer:<br>(Value: <col=ffffff>" + formattedPrice + "</col> coins)");
		trader.getActionSender().sendString(TRADE_WIDGET, PARTNER_CONTAINER_PRICE, partner.getName() + " offers:<br>(Value: <col=ffffff>" + otherFormattedPrice + "</col> coins)");

		partner.getActionSender().sendString(TRADE_WIDGET, PLAYER_CONTAINER_PRICE, "You offer:<br>(Value: <col=ffffff>" + otherFormattedPrice + "</col> coins)");
		partner.getActionSender().sendString(TRADE_WIDGET, PARTNER_CONTAINER_PRICE, trader.getName() + " offers:<br>(Value: <col=ffffff>" + formattedPrice + "</col> coins)");
	...
	public void addTransactionListeners(@Nonnull Player player) {
		...
		if (containerOption.isPresent() && partnerContainerOption.isPresent()) {
			...
			player.getInterfaceState().addListener(partnerContainer, new InterfaceContainerListener(player, -2, 60981, 90));
			player.getInterfaceState().addListener(partnerContainer, new InterfaceContainerListener(player, -1, 50, 80));
	...
	public boolean openConfirmationWidget(@Nonnull DiceGameTransaction transaction) {
		...
		if (trader.getInterfaceState().isInterfaceOpen(CONFIRMATION_WIDGET) && traderContainer.isAccepted() && partnerContainer.isAccepted()) {
			openDiceGame(transaction);
			return true;
		}

		if (trader.getInterfaceState().isInterfaceOpen(TRADE_WIDGET) && traderContainer.isAccepted() && partnerContainer.isAccepted()) {

			if (partner.getInventory().freeSlots() < traderContainer.size()) {
				partner.getActionSender().sendMessage("You don't have enough inventory space to complete this transaction.");
				trader.getActionSender().sendMessage("Your partner have enough inventory space to complete this transaction.");
				endTransaction(transaction, true);
				return false;
			}

			if (trader.getInventory().freeSlots() < partnerContainer.size()) {
				trader.getActionSender().sendMessage("You don't have enough inventory space to complete this transaction.");
				partner.getActionSender().sendMessage("Your partner doesn't have enough inventory space to complete this transaction.");
				endTransaction(transaction, true);
				return false;
			}
			Optional<Integer> priceOption = transaction.getPlayerContainer().stream().filter(i -> i != null && i.getDefinition() != null).map(i -> i.getDefinition().getStorePrice())
					.reduce((a, b) -> a + b);

			Optional<Integer> partnerPriceOption = transaction.getPartnerContainer().stream().filter(i -> i != null && i.getDefinition() != null).map(i -> i.getDefinition().getStorePrice())
					.reduce((a, b) -> a + b);

			int price = priceOption.isPresent() ? priceOption.get() : 0;
			int partnerPrice = partnerPriceOption.isPresent() ? partnerPriceOption.get() : 0;
			trader.getActionSender().sendString(CONFIRMATION_WIDGET, SECOND_TRADING_WITH, "<col=00FFFF>Dicing with:<br><col=00FFFF>" + partner.getName())
					.sendString(CONFIRMATION_WIDGET, 24, "In return you will receive:<br>(Value: <col=ffffff>" + NumberFormat.getNumberInstance(Locale.ENGLISH).format(partnerPrice) + "</col> coins)")
					.sendString(CONFIRMATION_WIDGET, 23, "You are about to give:<br>(Value: <col=ffffff>" + NumberFormat.getNumberInstance(Locale.ENGLISH).format(price) + "</col coins")
					.sendInterface(CONFIRMATION_WIDGET, false);
			partner.getActionSender().sendString(CONFIRMATION_WIDGET, SECOND_TRADING_WITH, "<col=00FFFF>Dicing with:<br><col=00FFFF>" + trader.getName())
					.sendString(CONFIRMATION_WIDGET, 24, "In return you will receive:<br>(Value: <col=ffffff>" + NumberFormat.getNumberInstance(Locale.ENGLISH).format(price) + "</col> coins)")
					.sendString(CONFIRMATION_WIDGET, 23, "You are about to give:<br>(Value: <col=ffffff>" + NumberFormat.getNumberInstance(Locale.ENGLISH).format(partnerPrice) + "</col coins")
					.sendInterface(CONFIRMATION_WIDGET, false);
