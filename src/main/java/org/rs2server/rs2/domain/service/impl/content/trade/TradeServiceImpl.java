...
public final class TradeServiceImpl implements TradeService {
	...
	private static final int CONFIRM_MESSAGE_COMPONENT = 4;
	...
	@Inject
	TradeServiceImpl(HookService hookService, PermissionService permissionService, PlayerVariableService playerVariableService) {
	...
	public final void onTradeRequest(@Nonnull GameTradeRequestEvent tradeRequest) {
		...
		if ((permissionService.is(trader, PermissionService.PlayerPermissions.IRON_MAN) || permissionService.is(partner, PermissionService.PlayerPermissions.IRON_MAN)) || (permissionService.is(trader, PermissionService.PlayerPermissions.HARDCORE_IRON_MAN) || permissionService.is(partner, PermissionService.PlayerPermissions.HARDCORE_IRON_MAN) ||
				(permissionService.is(trader, PermissionService.PlayerPermissions.ULTIMATE_IRON_MAN) || permissionService.is(partner, PermissionService.PlayerPermissions.ULTIMATE_IRON_MAN)))) {
			trader.sendMessage("Unable to trade due to the fact that either you or your partner is an iron man.");
			return;
		}
...
//			trader.sendMessage("Unable to trade due to the fact that either you or your partner is an admin");
//			return;
//		}
		...
		if (offerEvent.getInterfaceId() == CONFIRMATION_WIDGET && offerEvent.getPlayer().getInterfaceState().isInterfaceOpen(CONFIRMATION_WIDGET)) {
			...
			if (tradeOption.isPresent()) {
				...
				if (offerEvent.getButton() == ACCEPT_BUTTON) {
					...
					return;
				} else if (offerEvent.getButton() == DECLINE_BUTTON) {
					endTransaction(transaction, true);
					return;
				} 
				...
				transaction.remove(tradeOption.get(), new IndexedTradeItem(new Item(item.getId(), takeAmount), offerEvent.getChildButton()));
			}
		}
	}
	...
	public boolean openConfirmationWidget(@Nonnull Transaction transaction) {
		...
		if (trader.getInterfaceState().isInterfaceOpen(TRADE_WIDGET) && traderContainer.isAccepted() && partnerContainer.isAccepted()) {
			...
			Optional<Integer> priceOption = transaction.getTraderContainer().stream().filter(i -> i != null && i.getDefinition() != null).map(i -> i.getDefinition().getStorePrice())
					.reduce((a, b) -> a + b);
			...
			trader.getActionSender().sendString(CONFIRMATION_WIDGET, SECOND_TRADING_WITH, "<col=00FFFF>Trading with:<br><col=00FFFF>" + partner.getName())
			...
			partner.getActionSender().sendString(CONFIRMATION_WIDGET, SECOND_TRADING_WITH, "<col=00FFFF>Trading with:<br><col=00FFFF>" + trader.getName())
			...
			//Send String: Interface = 334, Child = 24, String = In return you will receive:<br>(Value: <col=ffffff>2,283</col> coins)
			//Send String: Interface = 334, Child = 23, String = You are about to give:<br>(Value: <col=ffffff>10</col> coins)
	...
	public void setAvailableInventorySpace(@Nonnull Transaction transaction) {
		...
		Optional<Integer> priceOption = transaction.getTraderContainer().stream().filter(i -> i != null && i.getDefinition() != null).map(i -> i.getDefinition().getStorePrice() * i.getCount())
				.reduce((a, b) -> a + b);
