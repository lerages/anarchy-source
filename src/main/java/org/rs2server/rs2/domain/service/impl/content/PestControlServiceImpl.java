...
public final class PestControlServiceImpl implements PestControlService {
    ...
    private static final int ATTACK_ONE_POINT = 86;
    private static final int STRENGTH_ONE_POINT = 87;
    private static final int DEFENCE_ONE_POINT = 88;
    private static final int RANGED_ONE_POINT = 89;
    private static final int MAGIC_ONE_POINT = 90;
    private static final int HITPOINTS_ONE_POINT = 91;
    private static final int PRAYER_ONE_POINT = 92;

    private static final int TEN_POINT_OFFSET = 15;
    private static final int ONE_HUNDRED_POINT_OFFSET = 7;
    ...
    public void openShop(@Nonnull Player player) {
        player.getActionSender()
                .sendString(SHOP_INTERFACE_ID, POINT_CHILD_ID, "Points: " + player.getDatabaseEntity().getStatistics().getPestControlPoints())
    ...
    public void onObjectClick(final GameObjectActionEvent clickEvent) {
        if (clickEvent.getActionType() == GameObjectActionEvent.ActionType.OPTION_1) {
            ...
            PestControlBoat boat = boats.get(0);// TODO: redo code to support multiple boats in future
            switch (object.getId()) {
                case 14314:
	...
	public void onShopClick(final PestControlShopEvent event) {
		...
		if (event.getButton() == 146 && player.getSelectedItem() != -1 && item != null) {
			...
			if (cost > points) {
				player.getActionSender().sendMessage("You don't have enough points to purchase this item.");
				return;
			}
			if (player.getInventory().add(new Item(item.getItemId()))) {
    ...
    public void addBoatMember(PestControlBoat boat, @Nonnull Player player) {
        ...
        player.getActionSender().sendString(407, 15, "Points: " + player.getDatabaseEntity().getStatistics().getPestControlPoints()).sendString(407, 16, "Novice").sendWalkableInterface(BOAT_WIDGET_ID);
	...
	public boolean containsPlayer(@Nonnull Player player) {
		...
		return (boat != null && (boat.getPlayers().contains(player) || (boat.getInstance() != null && boat.getInstance().getPlayers().contains(player))));
	}

	@Override
	public void handleDeath(@Nonnull Player player) {
		player.setTeleportTarget(PestControlInstance.START_LOCATION_BASE.transform(Misc.random(0, 1), Misc.random(0, 1), 0));
		...
		if (instance != null) {
			...
			instance.getPortals().stream().forEach(p -> player.getActionSender().sendString(408, p.getCardinality().getHealthChild(), "" + (p.isDestroyed() ? 0 : p.getSkills().getLevel(3))));
	...
	public final void onPlayerLogout(@Nonnull GamePlayerLogoutEvent event) {
		...
		if (boat != null) {
			Optional<Player> boatOptional = boat.getPlayers().stream().filter(p -> p.getName().equalsIgnoreCase(player.getName())).findAny();
			...
			if (instance != null) {
				Optional<Player> playerOptional = instance.getPlayers().stream().filter(p -> p.getName().equalsIgnoreCase(player.getName())).findAny();
    ...
    private void handleGates(Player player, GameObject object) {
        ...
        Location l = object.getLocation().transform((int) cardinality.getFaceVector().getX(), (int) cardinality.getFaceVector().getY(), 0);

        GameObject replacement = new GameObject(l, object.getId() + (open ? 1 : -1), 0, open ? rotation : ((cardinality.getFace() + 3) % 4), false);

        World.getWorld().register(replacement);
        RegionClipping.addClipping(replacement);

        l = second.getLocation().transform((int) cardinality.getFaceVector().getX(), (int) cardinality.getFaceVector().getY(), 0);

        replacement = new GameObject(l, second.getId() + (open ? 1 : -1), 0, open ? getRotation(second) : ((cardinality.getFace() + 3) % 4), false);
    ...
    private GameObject getSecondDoor(GameObject object) {
        ...
        for (int x = -1; x <= 1; x++) {
            for (int y = -1; y <= 1; y++) {
                ...
                GameObject other = object.getRegion().getGameObject(l.transform(x, y, 0), o -> o.getDefinition().getName().equals(object.getDefinition().getName()));
                if (other != null) {
                    return other;
                }
            }
    ...
    private int getObjectType(int objectId) {
        if (objectId == 14225 || objectId == 14226
                || objectId == 14228 || objectId == 14229) {
            return 9;
        }
        return 0;
    }
