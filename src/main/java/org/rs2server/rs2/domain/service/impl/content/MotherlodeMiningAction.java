...
public class MotherlodeMiningAction extends HarvestingAction {
	...
	public void onSuccessfulHarvest(final Item item) {
		if (getMob().isPlayer()) {
			...
			if (randomGem != null) {
				getMob().getActionSender().sendMessage("You just found a " + randomGem.getDefinition2().getName() + "!");
	...
	public double getExperience() {
		if (Misc.random(10000) == 0) {
			Pet.Pets pets = Pet.Pets.ROCK_GOLEM;
			if (!getMob().isPlayer()) {
			}
			Player player = (Player) getMob();
			if (player.getPet() != null) {
				return MINING_EXP_PER_PAY_DIRT * (getMob().isPlayer() ? miningService.getProspectorKitExperienceModifier((Player) getMob()) : 1f);
			} else {
				...
				World.getWorld().sendWorldMessage("<col=884422><img=33> News:" + player.getName() + " has just received a rock golem pet.");
				return MINING_EXP_PER_PAY_DIRT * (getMob().isPlayer() ? miningService.getProspectorKitExperienceModifier((Player) getMob()) : 1f);
			}
		}
		return MINING_EXP_PER_PAY_DIRT * (getMob().isPlayer() ? miningService.getProspectorKitExperienceModifier((Player) getMob()) : 1f);
