...
public class LootingBagServiceImpl implements LootingBagService {
	...
	@Inject
	public LootingBagServiceImpl(HookService hookService, GroundItemService groundItemService, ItemService itemService, PlayerService playerService) {
		...
		if (mob.isPlayer()) {
			...
			player.getLootingBag().stream().filter(Objects::nonNull).forEach(i ->
					groundItemService.createGroundItem(k.isPlayer() ? (Player)k : player,
							new GroundItemService.GroundItem(i, player.getLocation(), k.isPlayer() ? (Player) k : player, false, k.isPlayer() && k != player, k.isPlayer() && k == player)));
			...
			if (Misc.random(29) == 0) {
				if (!itemService.playerOwnsItem(killer, 11941)) {
					groundItemService.createGroundItem(killer, new GroundItemService.GroundItem(new Item(11941), npc.getLocation(), killer, false));
					killer.getActionSender().sendMessage("<col=7f00ff>You've received a Looting bag.");
				}
			}
		}
	}
	...
	public void check(@Nonnull Player player) {
		...
		player.getActionSender().sendAccessMask(1024, 81, 5, 0, 27)
				.sendString(81, 6, "Bag Value: " + NumberFormat.getNumberInstance(Locale.ENGLISH).format(getBagValue(player)))
	...
	public void deposit(@Nonnull Player player, int slot, int id, int amount) {
		...
		if (item.getId() == 11941) {
			player.getActionSender().sendDialogue("", ActionSender.DialogueType.MESSAGE, -1, null, "You may be surprised to learn that bagception is not permitted.");
			return;
		}
		if (!player.isInWilderness()) {
			player.getActionSender().sendMessage("You can't deposit items into the Looting bag unless you are in the Wilderness.");
			return;
		}
		...
		try {
			...
			updateCS2(player);
			player.getActionSender().sendString(81, 6, "Bag Value: " + NumberFormat.getNumberInstance(Locale.ENGLISH).format(getBagValue(player)));
	...
	public void updateCS2(@Nonnull Player player) {
		...
		for (int i = 0; i < Inventory.SIZE; i++) {
			...
			if (item != null && item.getId() != 11941 && item.getDefinition() != null) {
				priceList.add(item.getDefinition().getStorePrice());
			} else {
				priceList.add(-1);
			}
		}
	...
	public int getBagValue(@Nonnull Player player) {
		Optional<Integer> priceOption = player.getLootingBag().stream().filter(i -> i != null && i.getDefinition() != null).map(i -> i.getDefinition().getStorePrice())
				.reduce((a, b) -> a + b);
