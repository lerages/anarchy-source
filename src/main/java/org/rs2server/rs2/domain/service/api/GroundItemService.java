...
public interface GroundItemService {
	...
	class GroundItemTick extends Tickable {
		...
		@Override
		public void execute() {
			//System.out.println("Ground items updated");
			Region region = null;
			if (ticks == maxTicks - 200) {
				...
				if (groundItemOptional.isPresent()) {
					...
					if (groundItem.getItem().getDefinition() != null && groundItem.getItem().getDefinition().isTradable()) {
						region = World.getWorld().getRegionManager().getRegionByLocation(groundItem.getLocation());
			...
			if (ticks == maxTicks) {
				region = World.getWorld().getRegionManager().getRegionByLocation(item.getLocation());
