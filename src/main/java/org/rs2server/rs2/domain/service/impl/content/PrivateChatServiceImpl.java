...
import org.rs2server.rs2.domain.model.player.PlayerPrivateChatEntity;
		...
		friendsList.stream().forEach(i -> {
			...
			if (friend == null) {
				...
				otherFriendsList.stream().forEach(f -> {
					...
					if (otherFriend == null) {
						System.out.println("Player: " + f + " is null");
						...
						System.out.println(f + ", " + i + ", " + list.contains(f) + ", " + otherStatus);
