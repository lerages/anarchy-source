...
public class BountyHunterServiceImpl implements BountyHunterService {
	...
	private static final int KDR_CHILD_ID = 30;
	private static final int CURRENT_ROGUE_CHILD = 10;
	private static final int CURRENT_HUNTER_CHILD = 11;
	private static final int RECORD_HUNTER_CHILD = 12;
	private static final int RECORD_ROGUE_CHILD = 13;

	private static final Access BOUNTY_SHOP_ACCESS = Access.of(BOUNTY_STORE_WIDGET_ID, 2, NumberRange.of(0, 49), AccessBits.optionBit(10));

	private static final int BOUNTY_POINT_CONFIG = 1132;
	private static final int HUNTER_RECORD_CONFIG = 1135;

	@Inject
	BountyHunterServiceImpl(PlayerVariableService variableService, ItemService itemService, HookService hookService, GroundItemService groundItemService, PlayerService playerService) {
	...
	public final void openWidget(@Nonnull Player player) {
		...
		player.getActionSender().sendInterfaceConfig(BOUNTY_WIDGET_ID, 26, inWilderness)//23 26
	...
	public final void updateWidget(@Nonnull Player player) {
		if (player.getBountyTarget() != null) {
			...
			if (!target.isInWilderness()) {
				...
				wildernessLevel = color + "Lvl " + depth + "-" + (depth + 3) + ", Cmb " + target.getSkills().getCombatLevel() + "";
			}
	...
	public final void resetWidget(@Nonnull Player player) {
		player.getActionSender().sendString(BOUNTY_WIDGET_ID, TARGET_CHILD_ID, "").sendString(BOUNTY_WIDGET_ID, LVL_CHILD_ID, "");
		setWealth(player, Wealth.NONE);
	}

	@Override
	public final void assignTarget(@Nonnull Player player) {
		Optional<Player> availablePlayers = WILDERNESS_PLAYER_LIST.stream().filter(p -> p != player && hasRequiredCombatLevel(player, p)).filter(p -> p.getBountyTarget() == null).findFirst();
	...
	public final void sendHintIcons(@Nonnull Player player) {
		player.getActionSender().sendSetTargetEntity(player.getBountyTarget());
		player.getBountyTarget().getActionSender().sendSetTargetEntity(player);
	}
	...
	public final void openBountyShop(@Nonnull Player player) {
		player.getActionSender().sendInterface(BOUNTY_STORE_WIDGET_ID, false).sendConfig(BOUNTY_POINT_CONFIG, player.getDatabaseEntity().getBountyHunter().getBountyShopPoints());
		...
		if (player.getBounty() == null) {
			...
			if (!player.isInWilderness() && player.getBountyTarget() != null) {
				...
				if (bounty.getSafeDelay() <= 0) {
					...
					player.getBountyTarget().getActionSender().sendMessage("<col=FF0000>Your target is no longer available, so you shall be assigned a new target.");
					bounty.reset();
					player.getActionSender().removeWalkableInterface();
					player.getActionSender().sendMessage("<col=FF0000>You have not returned to the Wilderness in a timely manner and have lost your target.");
					return;
				}

				int delay = bounty.getSafeDelay() / 100;
				if (bounty.getSafeDelay() == 200 || bounty.getSafeDelay() == 100) {
					player.getActionSender().sendMessage("<col=FF0000>You have " + (delay) + "" + (delay == 1 ? " minute " : " minutes ") + "to return to the Wilderness before you lose your target.");
				}
	...
	public final void onPlayerLogout(@Nonnull GamePlayerLogoutEvent event) {
		...
		if (player.getBounty() != null) {
			if (player.getBountyTarget() != null) {
				...
				target.getActionSender().sendMessage("<col=FF0000>Your target is no longer available, so you shall be assigned a new target.");
				resetWidget(target);
	...
	public final void onBountyShopClick(@Nonnull GameInterfaceButtonEvent event) {
		if (event.getInterfaceId() == BOUNTY_STORE_WIDGET_ID) {
			...
			if (event.getMenuIndex() == 0) {
				BountyHunterService.BountyShopRewards.cost(clickedItem).ifPresent(i -> event.getPlayer().getActionSender().sendMessage(clickedItem.getDefinition2().getName() + " currently costs: " + NumberFormat.getNumberInstance(Locale.ENGLISH).format(i) + " Bounties."));
				return;
			}
				...
				if (costOption.isPresent()) {
					...
					if (bountyHunterEntity.getBountyShopPoints() >= cost && event.getPlayer().getInventory().add(clickedItem)) {
						...
						if (bountyHunterEntity.getBountyShopPoints() >= maxCost && event.getPlayer().getInventory().add(new Item(clickedItem.getId(), maxAmount))) {
							...
							event.getPlayer().getActionSender().sendMessage("Either your inventory is too full to buy an item or you don't have enough Bounties.");
						}
					}
				} else {
		...
		if (mob.isPlayer() && k.isPlayer()) {
			...
			if (player.getBounty() != null) {
				...
				if (player.getBountyTarget() != null) {
					Player target = player.getBountyTarget();
					if (killer == target) {
						killer.getActionSender().sendMessage("<col=FF0000>You've killed your target: " + player.getName() + "!");
						Bounty bounty = player.getBounty();
						...
						if (RANDOM.nextInt(4) == 0) {
							groundItemService.createGroundItem(killer, new GroundItemService.GroundItem(new Item(12746), player.getLocation(), killer, false));
						}
						/*if (!killer.getDatabaseEntity().getBountyHunter().getLastKilled().equalsIgnoreCase(player.getName())
								&& !killer.getDatabaseEntity().getBountyHunter().getLastKilledUUID().equalsIgnoreCase(player.getDetails().getUUID())) {
							int amount = Misc.random(200, 500);
							playerService.giveItem(killer, new Item(13330, amount), true);
							killer.getActionSender().sendMessage("You received " + amount + "x Blood money for killing " + player.getName() + ".");
						}*/
						...
						resetWidget(target);
						player.getActionSender().removeWalkableInterface();
						player.getActionSender().sendInteractionOption("null", 1, true).sendInteractionOption("null", 2, false);
					}/* else {
						if (!killer.getDatabaseEntity().getBountyHunter().getLastKilled().equalsIgnoreCase(player.getName())
								&& !killer.getDatabaseEntity().getBountyHunter().getLastKilledUUID().equalsIgnoreCase(player.getDetails().getUUID())) {
							int amount = Misc.random(50, 100);
							playerService.giveItem(killer, new Item(13330, amount), true);
							killer.getActionSender().sendMessage("You received " + amount + "x Blood money for killing " + player.getName() + ".");
						}
					}*/
