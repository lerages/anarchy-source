...
public class BankDepositBoxServiceImpl implements BankDepositBoxService {
    ...
    public void openDepositBox(@Nonnull Player player) {
        int inventoryChild = (int) player.getAttribute("tabmode") == 161 ? 63 : (int) player.getAttribute("tabmode") == 164 ? 61 : 65;
        int equipmentChild = (int) player.getAttribute("tabmode") == 161 ? 64 : (int) player.getAttribute("tabmode") == 164 ? 62 : 66;
        player.getActionSender().sendConfig(867, 37843994)
               // .removeInterface(equipmentChild)
               // .removeInterface(inventoryChild)
                .sendAccessMask(548, 47, -1, -1)
                .sendAccessMask(548, 48, -1, -1)
                .sendCS2Script(915, new Object[]{3}, "i")
                .sendInterface(192, false)
                .sendAccessMask(1180734, 192, 2, 0, 27);
    }

    @Override
    public void handleInterfaceActions(@Nonnull Player player, int button, int childButton, int childButton2, int menuIndex) {
