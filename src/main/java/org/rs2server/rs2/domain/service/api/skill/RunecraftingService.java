...
public interface RunecraftingService {
	...
	enum AltarType {
		AIR_ALTAR(14897, 1, 556, 13, false, new int[]{1, 11, 22, 33, 44, 55, 66, 77, 88, 99}),
		MIND_ALTAR(14898, 2, 558, 13.5, false, new int[]{1, 14, 28, 42, 56, 70, 84, 98}),
		WATER_ALTAR(14899, 5, 555, 14, false, new int[]{1, 19, 38, 57, 76, 95}),
		EARTH_ALTAR(14900, 9, 557, 14.5, false, new int[]{1, 26, 52, 78}),
		FIRE_ALTAR(14901, 14, 554, 15, false, new int[]{1, 35, 70}),
		BODY_ALTAR(14902, 20, 559, 15.5, false, new int[]{1, 46, 92}),
		COSMIC_ALTAR(14903, 27, 564, 19, true, new int[]{1, 59}),
		CHAOS_ALTAR(14906, 35, 562, 19.5, true, new int[]{1, 74}),
		ASTRAL_ALTAR(14911, 40, 9075, 19.7, true, new int[]{1, 82}),
		NATURE_ALTAR(14905, 44, 561, 20, true, new int[]{1, 91}),
		LAW_ALTAR(14904, 54, 563, 20.5, true, new int[]{1}),
		BLOOD_ALTAR(27978, 77, 565, 23.5, true, new int[]{1}),
		SOUL_ALTAR(27980, 90, 566, 25.5, true, new int[]{1}),
		DEATH_ALTAR(14907, 65, 560, 20.5, true, new int[]{1, 99}),
		;
		...
		AltarType(int objectId, int levelReq, int runeId, double xp, boolean pure, int[] multiples) {
			this.objectId = objectId;
			this.levelReq = levelReq;
			this.runeId = runeId;
			this.xp = xp;
	...
	class RuneCraftingAction extends ProductionAction {
		...
		private static final int DARK_ESSENCE = 7938;
		...
		public Item[] getConsumedItems() {
			return new Item[]{new Item(type.isPure() ? PURE_ESSENCE : player.getInventory().getCount(RUNE_ESSENCE) > 0 ?  RUNE_ESSENCE : PURE_ESSENCE, productionCount)};
		}
	...
	void handleMysteriousAltarInteraction(@Nonnull Player player, @Nonnull MysteriousAltarType type, @Nonnull GameObjectActionEvent clickEvent);

	void handleAltarInteraction(@Nonnull Player player, @Nonnull AltarType type);

	void handleEssenceMineInteraction(@Nonnull Player player, @Nonnull EssenceMine mine, @Nonnull GameObjectActionEvent clickEvent);
