...
public final class LootGenerationServiceImpl implements LootGenerationService {
	...
	/*	return Errors.log().getWithDefault(() -> {
			Map<Integer, NpcLootTable> tableMap = new HashMap<>();
			Files.list(Paths.get("./data/json/drops2/"))
					.map(p -> GSON.fromJson(Errors.log().getWithDefault(() -> new String(Files.readAllBytes(p)), ""), NpcLootTable.class))
					.forEach(t -> t.npcIds.forEach(n -> tableMap.put(n, t)));
			return tableMap;
		}, new HashMap<>());*/
	}
	...
	public final Item generateCasketLoot() {
		Optional<Loot> tableOption = Optional.of(LootTable.equalityTable(4012, 4012, 4012, 1038, 1040, 1042, 1044, 1046, 1048, 1050, 12887, 12888, 12889, 12890, 12891).getRandomLoot());
