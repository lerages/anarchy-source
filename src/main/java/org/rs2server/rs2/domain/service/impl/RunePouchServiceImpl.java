...
import org.rs2server.rs2.domain.service.impl.content.RunePouchVariables;
...
public class RunePouchServiceImpl implements RunePouchService {
	...
	@Inject
	RunePouchServiceImpl(HookService hookService, PlayerVariableService variableService, GroundItemService groundItemService) {
	...
	public final void onItemClick(@Nonnull GameItemInventoryActionEvent event) {
		if (event.getClickType() != GameItemInventoryActionEvent.ClickType.OPTION_1 && event.getClickType() != GameItemInventoryActionEvent.ClickType.OPTION_4) { return; }
				...
				case 4:
					switch (event.getMenuIndex()) {
						...
						case 1:
							...
							break;
						case 2:
							withdraw(player, event.getChildButton(), event.getChildButton2(), player.getRunePouch().getCount(event.getChildButton2()));
							break;
					}
					break;
				default:
					...
					if (item != null) {
						switch (event.getMenuIndex()) {
							...
							case 1:
								...
								break;
							case 2:
								deposit(player, event.getChildButton(), event.getChildButton2(), player.getInventory().getCount(event.getChildButton2()));
								break;
						}
	...
	public void openPouchInterface(@Nonnull Player player) {
		player.getActionSender().sendInterface(190, false)
				.sendInterfaceInventory(149)
				.sendAccessMask(0, 548, 48, -1, -1)
				.sendAccessMask(1311774, 190, 4, 0, 2)
				.sendAccessMask(1311774, 190, 8, 0, 27)
	...
	public void updatePouchInterface(@Nonnull Player player) {
		Item one = player.getRunePouch().get(0);
		Item two = player.getRunePouch().get(1);
		Item three = player.getRunePouch().get(2);
		if (one != null) {
			Optional<RuneVarp> varpOptional = RuneVarp.of(one.getId());
			if (varpOptional.isPresent()) {
				...
				variableService.set(player, RunePouchVariables.FIRST_SLOT_AMOUNT, one.getCount());
		...
		if (two != null) {
			Optional<RuneVarp> varpOptional = RuneVarp.of(two.getId());
			if (varpOptional.isPresent()) {
				RuneVarp varp = varpOptional.get();
				variableService.set(player, RunePouchVariables.SECOND_SLOT_RUNE, varp.getVarp());
				variableService.set(player, RunePouchVariables.SECOND_SLOT_AMOUNT, two.getCount());
		...
		if (three != null) {
			Optional<RuneVarp> varpOptional = RuneVarp.of(three.getId());
			if (varpOptional.isPresent()) {
				RuneVarp varp = varpOptional.get();
				variableService.set(player, RunePouchVariables.THIRD_SLOT_RUNE, varp.getVarp());
				variableService.set(player, RunePouchVariables.THIRD_SLOT_AMOUNT, three.getCount());
