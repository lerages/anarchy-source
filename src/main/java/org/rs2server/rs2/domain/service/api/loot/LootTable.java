...
public class LootTable {
	...
	/**
	 * Returns a new loot table which is the merged table between this and given loot table.
	 * @param lootTable The loot table to merge with.
	...
	/**
	 * Generates a drop map containing a specified amount of entries, in addition to all of the guarenteed drops.
	 * @param amount The goal amount of elements in our drop table.
	 * @return A map of loot to the amount of loot to be dropped.
	 */
	public Map<Loot, Integer> dropMap(int amount) {
		Map<Loot, Integer> partitioned = getLoot().stream().filter(l -> l.getProbability() == 100).collect(toMap(Function.identity(), Loot::getMinAmount));
	...
	/**
	 * Creates a loot table where all of the elements have the same default probability.
	 * @param ids The ids of the elements in the loot table.
	...
	/**
	 * The probabilities of all of the items in the loot table may not add up to 100%,
	 * therefore we calculate a factor by which all of the probability must be divided by to
	 * essentially treat the loot table as all items adding up to 100%.
	 ...
	 * @param amount the amount of loot to generate.
	 * @return The random loot entry. Never null.
	 */
