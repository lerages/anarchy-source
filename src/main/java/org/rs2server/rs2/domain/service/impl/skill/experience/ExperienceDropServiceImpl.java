...
public final class ExperienceDropServiceImpl implements ExperienceDropService {
	...
	public void openExperienceDrop(@Nonnull Player player) {
		final PlayerVariableService playerVariableService = Server.getInjector().getInstance(PlayerVariableService.class);
		...
		if (event.getInterfaceId() == EXPERIENCE_DROP_WIDGET) {
			...
			switch(event.getButton()) {
				...
				case SELECTED_SKILL_CHILD:
					playerVariableService.set(player, ExperienceVariables.SELECTED_SKILL_VARIABLE, event.getChildButton() - 1);
					break;
				case PROGRESS_BAR_CHILD:
					playerVariableService.set(player, ExperienceVariables.PROGRESS_BAR_VARIABLE, event.getChildButton() - 1);
					break;
				case SPEED_CHILD:
	...
	public void onOrbClicked(@Nonnull GameInterfaceButtonEvent event) {
		if (event.getInterfaceId() == EXPERIENCE_DROP_ORB_WIDGET && event.getButton() == EXPERIENCE_DROP_ORB_CHILD) {
			...
			if (event.getMenuIndex() == 0) {
				...
				//TODO: move resizable to database.
				if (player.getAttribute("orbOpen") == null) {
					...
					player.getActionSender().removeInterface(122);
				}
