...
public class AbyssServiceImpl implements AbyssService {
	...
	private enum AbyssRifts {
		...
		AbyssRifts(int id, Location location) {
			this.id = id;
			this.location = location;
		}

		public static Optional<AbyssRifts> of(int id) {
			return Arrays.stream(AbyssRifts.values()).filter(i -> i.getId() == id).findAny();
		}
	...
	public void enterAbyss(@Nonnull Player player) {
		player.teleport(ENTER_LOCATION, 0, 0, true);
		drainPlayer(player);
	}
	...
	public void onObjectClick(GameObjectActionEvent event) {
		...
		Optional<AbyssRifts> riftOptional = AbyssRifts.of(object.getId());
		if (riftOptional.isPresent()) {
			AbyssRifts rift = riftOptional.get();
