...
public class MotherlodeMineServiceImpl implements MotherlodeMineService {
	...
	private static final Logger logger = LoggerFactory.getLogger(MotherlodeMineServiceImpl.class);
	...
	/**
	 * The amount of pay-dirt in the sack when the player should be warned it is getting full.
	 */
	...
	/**
	 * A map of players to the amount of pay dirt they've put through the hopper
	 * and is waiting in the sack for them.
	 */
	...
	public void onNpcClick(final GameNpcActionEvent clickEvent) {
		if (clickEvent.getNpc().getId() != PROSPECTOR_PERCY_NPC_ID) return;

		if (clickEvent.getActionType() == GameNpcActionEvent.ActionType.OPTION_1) {
			Shop.open(clickEvent.getPlayer(), 25, -1);
	...
	public void claimOreInSack(@Nonnull Player player) {
		...
		if (amount == 0) {
			player.getActionSender().sendDialogue("", ActionSender.DialogueType.MESSAGE_MODEL_LEFT, PAY_DIRT_ITEM_ID, null,
					"The sack is empty.");
			return;
		}
		...
		if (amount > freeSpace) {
			...
			player.getActionSender().sendDialogue("", ActionSender.DialogueType.MESSAGE_MODEL_LEFT, PAY_DIRT_ITEM_ID, null,
					"You collect your ore from the sack.", "The sack still contains " + remaining + " ores.");
			...
			player.getActionSender().sendDialogue("", ActionSender.DialogueType.MESSAGE_MODEL_LEFT, PAY_DIRT_ITEM_ID, null, "You collect your ore from the sack.", "The sack is now empty");
	...
	public void depositPayDirt(@Nonnull Player player) {
		if (player.getInventory().getCount(PAY_DIRT_ITEM_ID) == 0) {
			player.getActionSender().sendDialogue("", ActionSender.DialogueType.MESSAGE_MODEL_LEFT, PAY_DIRT_ITEM_ID, null,
					"You have no pay-dirt in your inventory.");
			return;
		}
	...
	private List<PayDirtOre> generateOres(@Nonnull final Player player, final int amount) {
		...
		for (int i = 0; i < amount; i++) {
			...
			// Note that atleast 1 ore MUST have a 100% chance else this will throw a null pointer (in this case, coal).
			...
			PayDirtOre ore = Arrays.stream(PayDirtOre.values())
					...
					.filter(o -> (chance >= 0.9f ? 1f : chance) <= o.getChance()) // Round it if necessary since the upper bound is exclusive
					.findFirst().get();
	...
	/**
	 * Replaces all depleted veins within the Motherlode mine regions with mineable ore veins.
	 */
	private void replaceDepletedVeinObjects() {
		...
		final Set<Region> regions = ImmutableSet.of(regionManager.getRegion(116, 176),
				regionManager.getRegion(116, 177),
				regionManager.getRegion(117, 176),
		...
		regions.stream().forEach(r -> depletedVeins.addAll(r.getGameObjects().stream()
				.filter(Objects::nonNull)
				.filter(o -> depletedVeinIds.contains(o.getId()))
				.collect(Collectors.toSet())));

		depletedVeins.stream()
				.filter(o -> o.getRegion() != null)
				//.forEach(o -> regionService.addGameObject(o.getRegion(), new GameObject(o.getLocation(), veinId, o.getType(), o.getDirection(), true)));
				//.forEach(o -> regionService.removeGameObject(o.getRegion(), o));
				.forEach(o -> regionService.replaceObject(o.getRegion(), o, new GameObject(o.getLocation(), veinId, o.getType(), o.getDirection(), false)));
	}

	private enum PayDirtOre {
		COAL(453, 30, 0, 1f),
		GOLDEN_NUGGET(12012, 30, 0, 0.1f),
		GOLD(444, 40, 15, 0.1f),
		MITHRIL(447, 55, 30, 0.3f),
		ADAMANTITE(449, 70, 45, 0.15f),
		RUNITE(451, 85, 75, 0.04f);
		...
		/**
		 * @param itemId The ore item id.
		 * @param miningLevel The required mining level.
		 * @param experience The mining experience gained (the base experience, not multiplied)
		 * @param chance The chance for this ore to be found in range [0..1]
		 */
	...
	/**
	 * A wrapper around an NPC for pay dirt which slides along the water after placing
	 * pay dirt in the hopper.
	 *
	 * @author tommo
	 */
	...
	public static class DepletedVeinsRestorationTickable extends Tickable {
		...
		@Override
		public void execute() {
			// After the initial tick to replace all depleted veins with ore veins, extend the tick delay.
			setTickDelay(280);
