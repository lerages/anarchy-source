...
public class PlayerUpdateTask implements Task {
	...
	public void execute() {
		...
		if (player.isMapRegionChanging()) {
			...
			/*DynamicTileBuilder builder = DynamicTileBuilder.of(6400, 6400);
			DynamicTileBuilder geBuilder = DynamicTileBuilder.copyOf(3191, 3488);
			DynamicTileBuilder tzhaarBuilder = DynamicTileBuilder.copyOf(2408, 5104);

			builder.attachAll(geBuilder.attach(tzhaarBuilder.get(5, 5),
					DynamicTileBuilder.NeighborDirection.CENTER).getTiles(),
					DynamicTileBuilder.NeighborDirection.CENTER);

			player.getActionSender().sendDynamicRegion(builder.insert());*/
		}
		...
		updatePlayer(updateBlock, player, false); //tbh doesn't matter. Writes to update block 
		...
		for (Iterator<Player> it$ = player.getLocalPlayers().iterator(); it$.hasNext();) {
			 ...
			 */
			if (World.getWorld().getPlayers().contains(otherPlayer) && !otherPlayer.isTeleporting() && otherPlayer.getLocation().isWithinDistance(player.getLocation()) && !player.isMultiplayerDisabled()) {
				/*
				 * Update the movement.
				 */
	 ...
	 */
	public void updatePlayerMovement(PacketBuilder packet, Player otherPlayer) {//we aint mising anymore of these are we its possible
		/*
	...
	public void addNewPlayer(PacketBuilder packet, Player otherPlayer) {
		 ...
		 */
		packet.putBits(11, otherPlayer.getIndex());// fairly certain ive completed this method
		//System.out.println("Player: " + otherPlayer.getName() + ", Face: " + otherPlayer.getDirection());
		packet.putBits(1, 1);// its just so weird rofl ask leanbow if he knows anything about facing when entering a new region
		packet.putBits(5, xPos)//maybe we're setting direction wrong when you click attack? we could check
		.putBits(3, otherPlayer.getDirection()) // ye should be correct lol, but we could check. maybe directions are fucked? rofl
		.putBits(1, 1)
		.putBits(5, yPos);//we sure this isnt wrong
...
//		}
	...
	/**
	 * 
	 * AppendUpdateBlock(Player p, PacketBuilder updateBlock, boolean forceAppearance)
	 * 
	 * @param player, player we're updating.
	 * @param updateBlock, block being written to.
	 * @param forceAppearance, for login, and new players
	 * Mask values
	 * 0x400 - Graphics
	 * 0x40 - ChatText
	 * 0x80 - Hit1
	...
	public void updatePlayer(PacketBuilder packet, Player otherPlayer, boolean forceAppearance) {
		...
		synchronized(otherPlayer) {
			...
			if (flags.get(UpdateFlag.ANIMATION) && otherPlayer.getCurrentAnimation() != null) {// && player.getCurrentAnimation() != null) {
				mask |= 0x4;
			}
	...
	private static void appendChatTextUpdate(Player p, PacketBuilder updateBlock) {
		...
		if (text != null) {
			...
			byte[] chatStr = new byte[256]; chatStr[0] = (byte) unpacked.length();
	...
	private  void appendForceMovement(Player otherPlayer, PacketBuilder updateBlock) {		
		...
		updateBlock.putByteC((byte) (location.getLocalX(myLocation) + otherPlayer.getForceWalk()[0])); //first x to go to
		updateBlock.putByteC((byte) (location.getLocalY(myLocation) + otherPlayer.getForceWalk()[1])); //first y to go to
		updateBlock.putByteC((byte) (location.getLocalX(myLocation) + otherPlayer.getForceWalk()[2])); //second x to go to
		updateBlock.putByteC((byte) (location.getLocalY(myLocation) + otherPlayer.getForceWalk()[3])); //second y to go to
		updateBlock.putLEShortA(otherPlayer.getForceWalk()[4]); //speed going to location
		updateBlock.putShortA(otherPlayer.getForceWalk()[5]); //speed returning to original location
	 ...
	 * @param otherPlayer The player.
	 */
	private void appendPlayerAppearanceUpdate(PacketBuilder packet, Player otherPlayer) {//its this lol
		...
		if(otherPlayer.getPnpc() != -1) {
			playerProps.put((byte) 255);// client expects (id << 8) + i to = 65535
			...
			if (chest != null) {
				boolean chestBody = (chest.getEquipmentDefinition() != null && chest.getEquipmentDefinition().getType() == EquipmentType.PLATEBODY);
				if (!chestBody) {
...
//			boolean fullHelm = false;//TODO INCORPORATE FULL HELMS rofl
//			if (app.getGender() == 0) {
//				if (helm != null) {
//					fullHelm = (helm != null
