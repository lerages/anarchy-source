...
public class NPCUpdateTask implements Task {
	...
	public void execute() {
		...
		for (Iterator<NPC> it$ = player.getLocalNPCs().iterator(); it$.hasNext();) {
			NPC npc = it$.next();
			if (/*(!player.isRebuildNpcList() && player.getLocation().isWithinDistance(npc.getLocation())) || */(npc.instancedPlayer != null && npc.instancedPlayer != player) || World.getWorld().getNPCs().contains(npc) && !npc.isTeleporting() && npc.getLocation().isWithinDistance(player.getLocation()) && (!isInFightCaves || player.getInstancedNPCs().contains(npc))) {
				/*
				 * Update the movement.
				 */
				 ...
				 */
				if (npc.getUpdateFlags().isUpdateRequired()) {//maybe call update required?
		 ...
		 */
		for (NPC npc : player.isMultiplayerDisabled() ? player.getInstancedNPCs() : World.getWorld().getRegionManager().getLocalNpcs(player)) {
			/*
			...
			if (/*(!player.isRebuildNpcList() && player.getLocation().isWithinDistance(npc.getLocation())) || */(isInFightCaves && !player.getInstancedNPCs().contains(npc)) || !player.getLocation().isWithinDistance(npc.getLocation())) {
				continue;
			}
		...
		if (!updateBlock.isEmpty()) {
			 ...
			 */
			packet.finishBitAccess();// does it happen with players too? say 2 players r fighting in wild and a 3rd came to see the fight r they fucked
		}
...
//		updateBlock.putLEShortA(npc.getFaceLocation().getY());
	...
	private static void appendHit1Update(NPC npc, PacketBuilder updateBlock) {
		...
		/*int hpRatio = npc.getSkills().getLevel(Skills.HITPOINTS) * 30 / npc.getSkills().getLevelForExperience(Skills.HITPOINTS);
