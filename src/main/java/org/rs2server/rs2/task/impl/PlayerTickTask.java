...
//			if (item != null) {
//				player.getInventory().fireItemChanged(weaponSwitchSlot);
//			}
//		}
		while ((weaponSwitchSlot = player.getWeaponSwitchQueue().poll()) != null) {
			Item item = player.getInventory().get(weaponSwitchSlot);
...
//			
			if (item != null) {// TODO Improve this ...
				new WieldItemAction(player, item.getId(), weaponSwitchSlot, 0).execute();// basically to do this we gotta split apart the send interface code from the senditem code so that the interface changes but the items dont change til the queue, but the interface still updates auto
			}
		}
		if (player.hasAttribute("antiFire")) {
			if (System.currentTimeMillis() - (long)player.getAttribute("antiFire", 0L) < 360000) {
				if (System.currentTimeMillis() - (long)player.getAttribute("antiFire", 0L) > 15000 && System.currentTimeMillis() - (long)player.getAttribute("aantiFire", 0L) < 14000) {
					player.getActionSender().sendMessage("<col=ff0000>Your anti fire potion is about to wear off!");
				}
			} else if ((long)player.getAttribute("antiFire", 0L) > 0L && System.currentTimeMillis() - (long)player.getAttribute("antiFire", 0L) > 360000) {
		...
		if (BountyHunterServiceImpl.WILDERNESS_PLAYER_LIST.contains(player) && !player.isInWilderness() && player.getBountyTarget() == null) {
			...
			if (player.getCombatState().canMove()) {
				...
				while (true) {
					if (requiredDistance != 1) {
						if (!ProjectilePathFinder.clippedProjectile(player, player.getInteractingEntity()) && !(player.getInteractingEntity() instanceof Zulrah) && !(player.getInteractingEntity() instanceof Whirlpool)) {
		...
		//player.getActionSender().sendString(274, 15, "  Players Online: " + World.getWorld().getPlayers().size());//<img=4>
	}
}
