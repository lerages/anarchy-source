...
public class NPCTickTask implements Task {
	...
	public void execute() {
		...
		if (npc.getInteractingEntity() != null && (npc.getInteractingEntity().isDestroyed() || npc.getInteractingEntity().getCombatState().isDead())) {
			npc.resetInteractingEntity();
		}
		
		if (npc.getInteractingEntity() != null && npc.hasAttribute("facing") && !(npc instanceof Zulrah) && !(npc instanceof Pet)) {
		...
		if (npc.getId() == 1101) {
			if (npc.getInstancedPlayer() != null && npc.getInstancedPlayer() instanceof Player) {
				if (npc.getLocation().distance(npc.getInstancedPlayer().getLocation()) > 12) {
					npc.getInstancedPlayer().getActionSender().sendMessage("The Giant Sea Snake goes back into the water...");
		...
		if(npc.getCombatDefinition() != null && npc.getCombatDefinition().isAggressive() && !npc.getCombatState().isDead() && npc.getInteractingEntity() == null) {
			List<Mob> enemiesInArea = new ArrayList<>();//Help me make it so these nigs always attack
			if(npc.getRegion().getPlayers().size() > 0) {
				for(Player player : World.getWorld().getPlayers()) {
					boolean withinDistance = npc.getLocation().isWithinDistance(player.getLocation());
					if(!withinDistance || player.getLocation().getZ() != npc.getLocation().getZ()) {
						continue;// sec
					}
					...
					if(player != null && !player.getCombatState().isDead()) {
						if (bounds != null && BoundaryManager.isWithinBoundaryNoZ(player.getLocation(), bounds) || BoundaryManager.isWithinBoundaryNoZ(player.getLocation(), "MultiCombat_Bandos") || BoundaryManager.isWithinBoundaryNoZ(player.getLocation(), "MultiCombat_Zammy") || BoundaryManager.isWithinBoundaryNoZ(player.getLocation(), "MultiCombat_Armadyl") || BoundaryManager.isWithinBoundaryNoZ(player.getLocation(), "MultiCombat_Bandos") || BoundaryManager.isWithinBoundaryNoZ(player.getLocation(), "MultiCombat_Sara") || BoundaryManager.isWithinBoundaryNoZ(player.getLocation(), "RFD"))
							enemiesInArea.add(player);
						else if (bounds == null && npc.getActiveCombatAction().canHit(npc, player, true, false) && player.getSkills().getCombatLevel() <= npcCombatLvl && player.getLocation().isWithinDistance(npc, player, 5))
							enemiesInArea.add(player);
					}
				}
			}
			if(enemiesInArea.size() > 0) {
				...
				//if ((!BoundaryManager.isWithinBoundaryNoZ(p.getLocation(), "MultiCombat") || !BoundaryManager.isWithinBoundaryNoZ(npc.getLocation(), "MultiCombat")) || (!p.inMulti()) || !npc.inMulti() || !BoundaryManager.isWithinBoundaryNoZ(p.getLocation(), "RFD") || !BoundaryManager.isWithinBoundaryNoZ(p.getLocation(), "FightCaves")) {
					//if (p.getCombatState().getLastHitTimer() < (System.currentTimeMillis() + 10000)) {
		...
		if(npc.canMove()) {
			if(!npc.isInteracting() && !npc.getCombatState().isDead() && npc.getCombatState().canMove() && npc.getWalkingQueue().isEmpty() && random.nextInt(7) == 1) {
				...
				if (distance > 0) {
					...
					if(x >= npc.getMinLocation().getX() && x <= npc.getMaxLocation().getX() && y >= npc.getMinLocation().getY() && y <= npc.getMaxLocation().getY()) {
			...
			if (npc.getInteractingEntity() != null && npc.getInteractionMode() == InteractionMode.ATTACK) {
				if (npc.getCombatState().canMove()) {
					Boundary bounds = npc.getHomeArea();
					boolean caveNPC = (npc.getCombatDefinition() != null && npc.getCombatDefinition().isFightCavesNPC());
					if (bounds != null) {
						if (!caveNPC && !BoundaryManager.isWithinBoundaryNoZ(npc.getInteractingEntity().getLocation(), bounds)) { 
							npc.resetInteractingEntity();
						}
					} else if (!caveNPC && npc.getLocation().distanceToEntity(npc, npc.getInteractingEntity()) > 10) {// || npc.getLocation().distance(npc.getHomeArea().getCenterPoint()) > 10) {
						npc.resetInteractingEntity();
					}
					if (npc.getInteractingEntity() != null) {
						...
						while (true) {
							...
							if (distance > requiredDistance) {
								if (npc instanceof PestControlNpc || caveNPC || npc.getLocation().distance(npc.getSpawnLocation()) < 15 || npc.getCombatDefinition().getGodWarsTeam() != null) {
									//NPCFollow.handleCombatFollowing(npc, npc.getInteractingEntity(), requiredDistance);
									Following.combatFollow(npc, npc.getInteractingEntity());
								} else if (npc.getLocation().distance(npc.getSpawnLocation()) >= 15 && npc.getLocation().distance(npc.getInteractingEntity().getLocation()) > 5) {
									npc.resetInteractingEntity();
									World.getWorld().doPath(new DefaultPathFinder(), npc, npc.getSpawnLocation().getX(), npc.getSpawnLocation().getY());
								}
							}
							break;
						}
