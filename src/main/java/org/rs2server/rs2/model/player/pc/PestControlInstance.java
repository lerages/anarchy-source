...
public final class PestControlInstance implements Streamable<Player> {
	...
	public void start() {
		...
		players.forEach(p -> {
			...
			portals.stream().map(PestControlPortal::getCardinality).forEach(c -> p.getActionSender().sendInterfaceConfig(408, c.getShieldChild(), false));
		...
		players.forEach(p -> {
			if (p.getCombatState().isDead()) {
				World.getWorld().submit(new StoppingTick(10) {
					...
					public void executeAndStop() {
						...
						if (p.hasAttribute("hits_dealt") && (int) p.getAttribute("hits_dealt") >= 50) {
							p.getActionSender().sendMessage("You have been rewarded 10 Pest Control Points for your valiant effort.");
							int points = p.getDatabaseEntity().getStatistics().getPestControlPoints();
							p.getDatabaseEntity().getStatistics().setPestControlPoints(points + 5 * Constants.PEST_MODIFIER);
						} else {
							p.getActionSender().sendMessage("You have not fought hard enough and proven yourself to earn anything.");
						}
				...
				if (p.hasAttribute("hits_dealt") && (int) p.getAttribute("hits_dealt") >= 50) {
					p.getActionSender().sendMessage("You have been rewarded 10 Pest Control Points for your valiant effort.");
					int points = p.getDatabaseEntity().getStatistics().getPestControlPoints();
					p.getDatabaseEntity().getStatistics().setPestControlPoints(points + 10 * Constants.PEST_MODIFIER);
				} else {
					p.getActionSender().sendMessage("You have not fought hard enough and proven yourself to earn anything.");
				}
				p.removeAttribute("hits_dealt");
	...
	public void tick() {
		stream().forEach(a -> {
			...
			a.getActionSender().sendString(408, 2, TimeUnit.MILLISECONDS.toMinutes((boat.getGameTimeRemaining() * 600)) + " mins");
			getPortals().stream().forEach(p -> a.getActionSender().sendString(408, p.getCardinality().getHealthChild(), "" + (p.isDestroyed() ? 0 : p.getSkills().getLevel(3))));
		});
		if (allPortalsDead()) {
	...
	public boolean allPortalsDead() {
		return (bluePortal.isDestroyed() && redPortal.isDestroyed() && yellowPortal.isDestroyed() && purplePortal.isDestroyed());
	}
