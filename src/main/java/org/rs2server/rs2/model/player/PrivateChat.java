...
public class PrivateChat {
	...
	public boolean addClanMember(Player applicant) {
		...
		if(ignores.contains(applicant.getNameAsLong())) {
			applicant.getActionSender().sendClanMessage("You do not have a high enough rank to join this clan channel.");
			return false;
		}
	...
	public void sendMessage(Player p, String message) {
		...
		for(Player p2 : getMembers()) {
			p2.getActionSender().sendClanMessage(p.getName(), channelName, message, permissionService.getHighestPermission(p).ordinal());
		}
	}
			...
			if(p != null) {
				if (p.getPrivateChat() == null) {
					World.getWorld().getPrivateChat()
							.put(p.getName(), new PrivateChat(p.getName(), ""));
				}
	...
	public void updateFriendList(boolean online) {
		...
		for(Player p : World.getWorld().getPlayers()) {
			if(p.getPrivateChat() != null && p.getPrivateChat().getFriends().containsKey(player.getNameAsLong()) && !ignores.contains(p.getNameAsLong())) { //don't send that the player is logged in if they are on our ignore list
	...
	public void sendMessage(long to, String message) {
		...
		if(p != null) {
			...
			p.getActionSender().sendRecievedPrivateMessage(player.getNameAsLong(), permissionService.getHighestPermission(player).ordinal(), message);
		}
	}
	...
	public boolean canShareLoot(Player pl) {
		return pl.getSettings().isLootsharing() && getFriends().containsKey(pl.getNameAsLong()) ? getFriends().get(pl.getNameAsLong()).getId() >= lootRank.getId() : false;
	}
