...
public class CerberusContent extends Content {
    ...
    public void process() {
        ...
        if (cerberus != null && cerberus.getSkills().getLevel(Skills.HITPOINTS) > 0 && World.getWorld().getNPCs().contains(cerberus) && !cerberus.getCombatState().isDead() && BoundaryManager.isWithinBoundaryNoZ(player.getLocation(), "Cerberus")) {
			cerberus.doCombat();
        } else {
            if (respawnTimer == 0)
                spawnCerberus();
