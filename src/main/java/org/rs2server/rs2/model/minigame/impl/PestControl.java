...
public class PestControl extends AbstractMinigame {
	...
	public PestControl(Boat level, int height) {
		this.level = level;
		PestControlManager.getPestControlManager().heights[height / 4] = true; //set it to used
//		init();
	}
...
//		updateInterfaces(true);
	...
	protected void updateInterfaces(boolean sendInterface) {
		String first = "Next Departure: " + Integer.toString((waitTicks + 100) / 100) + " min" + (waitTicks > 100 ? "s" : "");
		...
		for (Player player : players) {
			...
			player.getActionSender().sendString(407, 15, third + Integer.toString(player.getSettings().getPestPoints()));
...
//		int id = getRandomId();
//		voidKnight = new NPC(NPCDefinition.forId(id), id, VOID_KNIGHT_POS.transform(0, 0, height), null, null, NormalDirection.SOUTH.npcIntValue());
...
//					player.getActionSender().sendString(408, 11, Integer.toString((Integer) player.getAttribute("hits_dealt", 0)));
//					player.getActionSender().sendString(408, 0, time);
//					for (int i = 0; i < 4; i++) {
//						player.getActionSender().sendString(408, ((Portal)npcs.get(i).getAttribute("portal")).interfacConfig / 2 + 4, Integer.toString(npcs.get(i).getSkills().getLevel(Skills.HITPOINTS)));
//					}
//				}
//				if (gameTicks >= 1900) {
...
//							npc.getSkills().setLevel(Skills.HITPOINTS, 2500);
//						else
//							npc.getSkills().setLevel(Skills.HITPOINTS, npc.getCombatDefinition().getSkills().get(Skill.HITPOINTS));
//						clip(npc, portal, 1);
//					}
//				}
...
//
//						//		if (spawn) {
//						int id = PestControlManager.PC_NPCS[level.ordinal()][RANDOM.nextInt(PestControlManager.PC_NPCS[level.ordinal()].length)];
//						NPC spawnNpc = new NPC(NPCDefinition.forId(id), id, portal.spawnLocs[RANDOM.nextInt(portal.spawnLocs.length)].transform(0, 0, height), portal.spawn.transform(-10, -10, height), portal.spawn.transform(10, 10, height), RANDOM.nextInt(4));
...
//		World.getWorld().submit(gameTick);
	...
	private void clip(NPC npc, Portal portal, int state) {
		if (portal == Portal.WEST || portal == Portal.EAST) { //0x200000
			...
			for (int i = 0; i < 5; i++) {
				RegionClipping.addClipping(base.getX(), base.getY() + i, base.getZ(), 256);
				if (i >= 1 && 4 < i) {
					if (state == 0) {
						RegionClipping.addClipping(base.getX() - 1, base.getY() + i, base.getZ(), 256);
						RegionClipping.addClipping(base.getX() + 1, base.getY() + i, base.getZ(), 256);
					} else if (state == 1) {
						RegionClipping.removeClipping(base.getX() - 1, base.getY() + i, base.getZ(), 256);
						RegionClipping.removeClipping(base.getX() + 1, base.getY() + i, base.getZ(), 256);
					}
				}
			}
			...
			Location base = portal.spawn.transform(-1, 1, height);
			for (int i = 0; i < 5; i++) {
				RegionClipping.addClipping(base.getX() + i, base.getY(), base.getZ(), 256);
				if (i >= 1 && 4 < i) {
					if (state == 0) {
						RegionClipping.addClipping(base.getX() + i, base.getY() - 1, base.getZ(), 256);
						RegionClipping.addClipping(base.getX() + i, base.getY() + 1, base.getZ(), 256);
					} else if (state == 1) {
						RegionClipping.removeClipping(base.getX() + i, base.getY() - 1, base.getZ(), 256);
						RegionClipping.removeClipping(base.getX() + i, base.getY() + 1, base.getZ(), 256);
					}
				}
			}
