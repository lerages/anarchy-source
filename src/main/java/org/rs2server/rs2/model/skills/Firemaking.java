 ...
 */
public class Firemaking {//don't use that system. o lol, try looking for firemaking usign that system itd be way easier to implmenent
	...
	private enum Log {
		 ...
		 * @param id The item id of the logging.
		 * @param reqLevel The required level to light the logging.
		 * @param exp The experience one gains from lighting the logging.
		 */
	...
	/**
	 * Lights a logging.
	 * @param log The logging to light.
	 */
	public void light(final Log log) {
		...
		player.setLighting(true);//been trying to put htis back ffs and yes increase dleay
		...
		//final GroundItemDefinition groundItem = new GroundItemDefinition(player.getName(), player.getLocation(), item.getId(), item.getCount());
...
//				GroundItemDefinition logs = GroundItemController.getGroundItem(player.getLocation(), item.getId());
				Optional<GroundItemService.GroundItem> logs = groundItemService.getGroundItem(item.getId(), player.getLocation());
				if(!logs.isPresent()) {
					this.stop();
					return;
				}
				...
				if(addExperience(log)) {
					...
					player.getActionSender().sendMessage("Congratulations! You have advanced a level in Firemaking to " + player.getSkills().getLevelForExperience(Skills.FIREMAKING) + ".");
				}
				World.getWorld().submit(new Tickable(180) {
					@Override
					public void execute() {
						groundItemService.createGroundItem(player, new GroundItemService.GroundItem(new Item(592), fire.getLocation(), player, true));
				...
				for(GameObject obj : player.getRegion().getGameObjects()) {
					if(obj != null && obj.getType() != 22 && obj.getType() != 0) { 
						if(obj.getLocation().equals(Location.create(player.getLocation().getX() - 1, player.getLocation().getY(), player.getLocation().getZ())) && obj.getHeight() == player.getHeight()) {
							badDirections.add("West");
						} else if(obj.getLocation().equals(Location.create(player.getLocation().getX(), player.getLocation().getY(), player.getLocation().getZ())) && obj.getHeight() == player.getHeight()) {
	 ...
	 * @param log The logging we will add experience for.
