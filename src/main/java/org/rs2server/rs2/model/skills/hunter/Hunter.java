...
//	      trap.getOwner().setTrapsLaid(trap.getOwner().getTrapsLaid() - 1);
	  ...
	  public static boolean canLay(Player client) {
	    if (!goodArea(client)) {
	      client.getActionSender().sendMessage("You see no reason to place that here.");
	      return false;
	    }
	    ...
	    for (final Trap trap : traps) {
	      ...
	      if (trap.getGameObject().getLocation().getX() == client.getLocation().getX()
	        ...
	        client.getActionSender()
	          .sendMessage("You cannot place the trap here.");
	        return false;
	      }
	    }
...
//	        if (trap == null)
//	          continue;
//	        if (trap.getOwner() != null && trap.getOwner().getName().equals(client.getName())
//	              && !Location.hasDistance(trap.getGameObject().getLocation(), client.getLocation(), 50)) {
//	          deregister(trap);
//	          client.getActionSender()
...
//	    }
	          ...
	          if (theTrap.getOwner() == client) {
	            if (theTrap instanceof SnareTrap) {
	              ...
	              if (theTrap.getGameObject().getId() == 19180) {
	                ...
	                client.getInventory()
	                  .add(new Item(10088, 20 + Misc.random(30)));
	                ...
	                client.getInventory()
	                  .add(new Item(10090, 20 + Misc.random(30)));
	                client.getInventory().add(new Item(9978, 1));
	                client.getActionSender()
	                  .sendMessage("You've successfully caught a Golden Warbler.");
	                client.getSkills().addExperience(Skills.HUNTER, 47);

	              } else if (theTrap.getGameObject().getId() == 19186) {

	                client.getInventory()
	                  .add(new Item(10091, 20 + Misc.random(50)));
	                client.getInventory().add(new Item(9978, 1));
	                client.getActionSender()
	                  .sendMessage("You've successfully caught a Copper Longtail.");
	                client.getSkills().addExperience(Skills.HUNTER, 61);

	              } else if (theTrap.getGameObject().getId() == 19182) {

	                client.getInventory()
	                  .add(new Item(10089, 20 + Misc.random(30)));
	                client.getInventory().add(new Item(9978, 1));
	                client.getActionSender()
	                  .sendMessage("You've successfully caught a Cerulean Twitch.");
	                client.getSkills().addExperience(Skills.HUNTER, 65);

	              } else if (theTrap.getGameObject().getId() == 19178) {

	                client.getInventory()
	                  .add(new Item(10087, 20 + Misc.random(30)));
	                client.getInventory().add(new Item(9978, 1));
	                client.getActionSender()
	                  .sendMessage("You've successfully caught a Tropical Wagtail.");
	              ...
	              if (theTrap.getGameObject().getId() == 19191) {
	                ...
	                client.getActionSender()
	                  .sendMessage("You've successfully caught a red chinchompa!");
	  ...
	  public static void catchNPC(Trap trap, NPC npc) {
	    if (trap.getTrapState().equals(TrapState.CAUGHT))
	      return;
	    if (trap.getOwner() != null) {
	      if (trap.getOwner().getSkills().getLevel(22) < requiredLevel(
	        npc.getId())) {
	      ...
	      deregister(trap);
	      if (trap instanceof SnareTrap)
//	        register(new SnareTrap(new GameObject(getObjectIDByNPCID(npc.getId()),
//	                                               new Location(trap.getGameObject().getLocation()
//	                                                              .getX(),
//	                                                             trap.getGameObject().getLocation()
//	                                                               .getY(), trap.getGameObject().getLocation().getZ())), Trap.TrapState.CAUGHT,
//	                                100, trap.getOwner()));
//	      else
//	        register(new BoxTrap(new GameObject(getObjectIDByNPCID(npc.getId()),
//	                                             new Position(trap.getGameObject().getPosition().getX(),
//	                                                           trap.getGameObject().getPosition()
//	                                                             .getY(), trap.getGameObject().getLocation().getZ())), Trap.TrapState.CAUGHT, 100,
//	                              trap.getOwner()));
...
//	    if (p.getTrapsLaid() > 0) {
//	      for (Trap trap : traps) {
//	        if (trap.getOwner() != null && trap.getOwner().getName().equals(p.getName())) {
//	          deregister(trap);
//	          if (trap instanceof SnareTrap) {
//	            p.getInventory().add(new Item(10006, 1));
//	          } else if (trap instanceof BoxTrap) {
//	            p.getInventory().add(new Item(10008, 1));
//	            p.playAnimation(Animation.create(827));
//	          }
//	        }
//	      }
//	    }
