...
public class NPCPickpocketAction extends SkillAction {
    ...
    public boolean commence(Player player) {
        if (player.getSkills().getLevel(Skills.THIEVING) < npcData.getThievingLevels()[0]) {
            player.getActionSender().sendMessage("You need a Thieving level of " + npcData.getThievingLevels()[0] + " to steal this npc's pocket.");
            return false;
        }
        ...
        player.getActionSender().sendMessage("You attempt to pick the " + npc.getDefinition().getName().toLowerCase() + "'s pocket...");
    ...
    private String getMessage(Player player) {
        if (slot == 0) {
            ...
            player.playGraphics(DOUBLE_LOOT_GFX);
            player.playAnimation(DOUBLE_LOOT_ANIMATION);
            return "Your lighting-fast reactions allow you to steal double loot.";
        } else if (slot == 2) {
            player.playGraphics(TRIPLE_LOOT_GFX);
            player.playAnimation(TRIPLE_LOOT_ANIMATION);
            return "Your lighting-fast reactions allow you to steal triple loot.";
        }
        player.playGraphics(QUADRUPLE_LOOT_GFX);
        player.playAnimation(QUADRUPLE_LOOT_ANIMATION);
