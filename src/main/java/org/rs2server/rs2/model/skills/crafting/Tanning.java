...
public class Tanning {
	...
	public static boolean startTanning(Player player, Hides hide, int childId, int amount) {
		...
		if (!player.getInventory().hasItem(new Item(995, hide.getCost() * amount))) {
			player.getActionSender().sendDialogue("", ActionSender.DialogueType.MESSAGE, -1, null, "You don't have enough coins to have the hides tanned.");
			player.getActionSender().sendMessage("You don't have enough coins to have the hides tanned.");
			return false;
		}
		...
		if (player.getInventory().add(new Item(hide.getTannedLeather(), amount))) {
			player.getActionSender().sendMessage("The tanner tans " + amount + " cowhides for you.");
		}
		return true;
	}
