...
public class DialogueSkillManager {
    ...
    public static boolean handleSkillAction(Player player, int child) {
        if (player.getInterfaceAttribute("cookItem") != null
            ...
            if (skin != null) {
                SnakeskinCrafting.SnakeskinCraftingAction produceAction = new SnakeskinCrafting.SnakeskinCraftingAction(player, skin, amount);
            ...
            return true;
        } else if (player.getAttribute("craftingType") != null && player.getAttribute("dhide_type") != null && (Integer) player.getAttribute("craftingType") == 2) {
            int amount = 0;
			...
			switch ((HerbloreType) player.getInterfaceAttribute("herblore_type")) {
				case PRIMARY_INGREDIENT:
					player.getActionQueue().addAction(new Herblore(player, amount, PrimaryIngredient.forId((int) player.getInterfaceAttribute("herblore_index"), (int) player.getInterfaceAttribute("vial_index")), null, HerbloreType.PRIMARY_INGREDIENT));
					break;
...
//            System.out.println("Amount: " + amount);
            ...
            if (child >= 5 && child <= 8) {
                if (group == FletchingGroup.OAK_LOGS) {
                    item = FletchingItem.OAK_SHORTBOW_U;
                } else if (group == FletchingGroup.WILLOW_LOGS) {
                    item = FletchingItem.WILLOW_SHORTBOW_U;
                } else if (group == FletchingGroup.MAPLE_LOGS) {
                    item = FletchingItem.MAPLE_SHORTBOW_U;
                } else if (group == FletchingGroup.YEW_LOGS) {
                    item = FletchingItem.YEW_SHORTBOW_U;
                } else if (group == FletchingGroup.MAGIC_LOGS) {
                    item = FletchingItem.MAGIC_SHORTBOW_U;
                }
            } else if (child >= 9 && child <= 13) {
                if (group == FletchingGroup.LOGS) {
                    item = FletchingItem.SHORTBOW_U;
                } else if (group == FletchingGroup.OAK_LOGS) {
                    item = FletchingItem.OAK_LONGBOW_U;
                } else if (group == FletchingGroup.WILLOW_LOGS) {
                    item = FletchingItem.WILLOW_LONGBOW_U;
                } else if (group == FletchingGroup.MAPLE_LOGS) {
                    item = FletchingItem.MAPLE_LONGBOW_U;
                } else if (group == FletchingGroup.YEW_LOGS) {
                    item = FletchingItem.YEW_LONGBOW_U;
                } else if (group == FletchingGroup.MAGIC_LOGS) {
                    item = FletchingItem.MAGIC_LONGBOW_U;
                }
            } else if (child >= 14 && child <= 17) {
                if (group == FletchingGroup.LOGS) {
                    item = FletchingItem.LONGBOW_U;
                } else if (group == FletchingGroup.OAK_LOGS) {
                    item = FletchingItem.OAK_STOCK;
                } else if (group == FletchingGroup.WILLOW_LOGS) {
                    item = FletchingItem.WILLOW_STOCK;
                } else if (group == FletchingGroup.MAPLE_LOGS) {
                    item = FletchingItem.MAPLE_STOCK;
                } else if (group == FletchingGroup.YEW_LOGS) {
                    item = FletchingItem.YEW_STOCK;
                }
            } else if (child >= 18 && child <= 21) {
                if (group == FletchingGroup.LOGS) {
                    item = FletchingItem.WOODEN_STOCK;
                }
            }
            if ((child == 9 || child == 8 || child == 7 || child == 6) && group == FletchingGroup.LOGS) {
                item = FletchingItem.ARROW_SHAFTS;
            }
            ...
            if (item != null) {
                //System.out.println(item + ", " + item.getLevel());
            ...
            if (item.getType() == FletchingType.ATTACHING) {
                amount = child == 5 ? 1 : child == 4 ? 5 : 10;// that looks right to me.  maybe the fact amount is 0 ? idk print the fucking amount!!!
            } else {
