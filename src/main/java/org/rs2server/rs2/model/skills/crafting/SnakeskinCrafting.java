...
public class SnakeskinCrafting {
    ...
    public static class SnakeskinCraftingAction extends SkillAction {
        ...
        public boolean commence(Player player) {
            if (player.getSkills().getLevel(Skills.CRAFTING) < toProduce.getLevelReq()) {
                ...
                sb.append(" to make a ").append(CacheItemDefinition.get(toProduce.getReward().getId()).getName().toLowerCase()).append(".");
        ...
        public boolean execute(Player player) {
            if (amount > 0) {
                if (!player.getInventory().contains(THREAD.getId())) {
                    ...
                    amount = 0;
                    player.getActionSender().sendMessage("You have ran out of " + CacheItemDefinition.get(SNAKESKIN).getName() + ".");
                    return false;
                }
    ...
    public enum Snakeskin {
        ...
        public static Snakeskin of(int id) { return snakeskinMap.get(id);}
    ...
    public static boolean handleItemOnItem(Player player, Item itemUsed, Item usedWith) {
        ...
        for (Snakeskin skin : Snakeskin.values()) {
            ...
            String name = TextUtils.upperFirst(CacheItemDefinition.get(skin.getReward().getId()).getName().replace("Snakeskin ", ""));
