...
public class InterfaceState {
	...
	/**
	 * The item id which is to be destroyed when the player is in the 'Are you sure you want to destroy this item' interface.
	 */
	...
	public void closeEnterAmountInterface(int amount) {
		try {
			...
			case Bank.PLAYER_INVENTORY_INTERFACE:
				...
				player.getActionSender().sendConfig(304, (player.getSettings().getLastWithdrawnValue() * 2) | (player.getSettings().isSwapping() ? 0 : 1));
				break;
			...
			case Trade.TRADE_INVENTORY_INTERFACE:
				if (player.getTransaction() != null && isInterfaceOpen(TradeServiceImpl.TRADE_WIDGET)) {
					player.getTransaction().remove(player.getTransaction().get(player).get(), new IndexedTradeItem(null, amount));
				} else if (player.getDiceGameTransaction() != null && isInterfaceOpen(DiceGameServiceImpl.TRADE_WIDGET)) {
					player.getDiceGameTransaction().remove(player.getDiceGameTransaction().get(player).get(), new IndexedDiceItem(null, amount));
				}
//				Trade.removeItem(player, enterAmountSlot, enterAmountId, amount);
				break;
			case Trade.PLAYER_INVENTORY_INTERFACE:
				if (player.getTransaction() != null && isInterfaceOpen(TradeServiceImpl.TRADE_WIDGET)) {
					player.getTransaction().add(player.getTransaction().get(player).get(), new Item(enterAmountId, amount));
				} else if (player.getDiceGameTransaction() != null && isInterfaceOpen(DiceGameServiceImpl.TRADE_WIDGET)) {
					player.getDiceGameTransaction().add(player.getDiceGameTransaction().get(player).get(), new Item(enterAmountId, amount));
				}
//				Trade.offerItem(player, enterAmountSlot, enterAmountId, amount);
			...
			case 306:
				if(player.getInterfaceAttribute("fletch_item") != null) {
					player.getActionQueue().addAction(new FletchingAction(player, amount, (FletchingItem) player.getInterfaceAttribute("fletch_item")));
			...
			/*case 320://Skill tab
				if (amount > 99) {
					player.getActionSender().sendMessage("You can only set a stat level 1 through 99.");
					return;
				}
				int skill_id = player.getInterfaceAttribute("skillLevelChange");
				String skill_name = "";
				double exp = player.getSkills().getExperienceForLevel(amount);
				if (skill_id == 0) {
					skill_name = "Attack";
				} else if (skill_id == 1) {
					skill_name = "Defence";
				...
				if(skill_id == 0 || skill_id == 2 || skill_id == 4 || skill_id == 6) {
					if(player.getEquipment().get(Equipment.SLOT_WEAPON) != null) {
						player.getActionSender().sendMessage("You can't change your " + skill_name + " level whilst wielding any equipment.");
						return;
					}
				}
				if (skill_id == 5) {
				...
				player.getActionSender().sendMessage("Your "+ skill_name +" level has been set to "+ amount +".");
