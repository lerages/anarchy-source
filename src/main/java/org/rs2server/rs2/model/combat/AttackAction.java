...
public class AttackAction extends Action {
	...
	public void execute() {
		...
		if (target == null || target.isDestroyed() || target.getCombatState().isDead() || target.getSkills().getLevel(Skills.HITPOINTS) < 1) {
		...
		if (target instanceof Whirlpool) {
			requiredDistance = requiredDistance + 5;
		}
		...
		CombatAction action_ = CombatFormulae.getActiveCombatAction(mob);
		...
		if ((!(target instanceof Zulrah) && !(target instanceof Whirlpool)) && (action == MagicCombatAction.getAction() || action == RangeCombatAction.getAction()) && !ProjectilePathFinder.clippedProjectile(mob, target)) {
			return;
		}
	...
	public Location[] allowedLocations(Location t, int size) {
		return new Location[] {
				Location.create(t.getX(), t.getY() + size, t.getZ()),
				Location.create(t.getX() + size, t.getY(), t.getZ()),
				Location.create(t.getX(), t.getY() - size, t.getZ()),
				Location.create(t.getX() - size, t.getY(), t.getZ()) };
	}
	...
	private void attack(Mob target) {
		...
		if (!(mob instanceof PestControlPortal || mob instanceof VoidKnight)) {
			...
			if (action == MagicCombatAction.getAction() && !mob.isNPC() && mob.getCombatState().getQueuedSpell() == null && mob.getAutocastSpell() == null) {
				this.stop();
			}
