...
public class Kreearra extends AbstractCombatAction {
	...
	public void hit(final Mob attacker, final Mob victim) {
		...
		case MAGIC:
			for(final Player near : World.getWorld().getPlayers()) {
				if(near != null && Misc.getDistance(near.getLocation(), attacker.getLocation()) < 10 && near != victim && near != attacker && near.getSkills().getLevel(Skills.HITPOINTS) > 0) {
					...
					World.getWorld().submit(new Tickable(hitDelay) {
						public void execute() {
							int damage = damage(maxHitMagic, attacker, near, AttackType.MAGIC, Skills.MAGIC , Prayers.PROTECT_FROM_MAGIC, false, false);
							damage = random.nextInt(damage < 1 ? 1 : damage + 1);
			...
			damage = damage(maxHit, attacker, victim, AttackType.MAGIC, Skills.MAGIC , Prayers.PROTECT_FROM_MAGIC, false, false); //these ignore prayers
			damage = random.nextInt(damage < 1 ? 1 : damage + 1);
			if(damage > victim.getSkills().getLevel(Skills.HITPOINTS)) {
				damage = victim.getSkills().getLevel(Skills.HITPOINTS);
			}
		...
		case RANGE:
			...
			for(final Player near : World.getWorld().getPlayers()) {
				if(near != null && Misc.getDistance(near.getLocation(), attacker.getLocation()) < 10 && near != victim && near != attacker) {
					...
					World.getWorld().submit(new Tickable(hitDelay) {
						public void execute() {
							int damage = damage(maxHitRange, attacker, near, AttackType.RANGE, Skills.RANGE , Prayers.PROTECT_FROM_MISSILES, false, false); //these ignore prayers
							damage = random.nextInt(damage < 1 ? 1 : damage + 1);
							if(damage > near.getSkills().getLevel(Skills.HITPOINTS)) {
								damage = near.getSkills().getLevel(Skills.HITPOINTS);
							}
			...
			attacker.playAnimation(Animation.create(6978));
			attacker.playProjectile(Projectile.create(attacker.getCentreLocation(), victim.getCentreLocation(), 1199, 45, 50, rClientSpeed, 43, 35, victim.getProjectileLockonIndex(), 10, 48));
			...
			randomHit = damage(maxHit, attacker, victim, AttackType.RANGE, Skills.RANGE , Prayers.PROTECT_FROM_MISSILES, false, false); //these ignore prayers
