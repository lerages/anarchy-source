...
public final class CombatState {
	...
	public void startAttacking(Mob victim, boolean retaliating) {
		...
		if(requiresDistance && mob.getLocation().isWithinDistance(victim.getLocation(), mob.getActiveCombatAction().distance(mob))) {
...
//			}
	...
	public static enum CombatStyle {
		...
		BASH(9, new int[] {Skills.ATTACK }, new double[] {1.33}),
		
		POUND(10, new int[] {Skills.STRENGTH }, new double[] {1.33}),
		
		FOCUS(11, new int[] {Skills.DEFENCE }, new double[] {1.33});
		/**
	...
	/**
	 * The delay before you can equip another weapon, used to endGame emotes overlapping (EG: whip using dds anim).
	 */
	 ...
	 * The active prayers.
	 */
	private boolean[] prayers = new boolean[29];
	
	/**
	 * The active quick prayers.
	 */
	private boolean[] quickPrayers = new boolean[29];
...
//        }
	...
	public void calculateBonuses() {
		...
		/* +7 prayer bonus for full veracs with amulet of the damned */
		// TODO this doesnt work, wtf?
		/*if (CombatFormulae.fullVerac(mob) && CombatFormulae.hasAmuletOfTheDamned(mob)) {
			setBonus(11, getBonus(11) + 7);
		}*/
	}
	...
	/**
	 * @param spellbookSwap The spellbookSwap to set.
	 */
	...
	/**
	 * @param canSpellbookSwap The canSpellbookSwap to set.
	 */
	...
	/**
	 * @param skullTicks the skullTicks to set
	 */
	...
	public void updateCombatWith(Mob attacker) {
		if (mob.isPlayer() && attacker.isPlayer()) {
			...
			if (player2.getBounty() != null && player2.getBountyTarget() != null && player2.getBountyTarget() == player) {
				return;
			}
		}
