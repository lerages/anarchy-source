...
public class KrilTsutsaroth extends AbstractCombatAction {
	...
	public void hit(final Mob attacker, final Mob victim) {
		...
		case MELEE:
			...
			randomHit = damage(maxHit, attacker, victim, AttackType.CRUSH, Skills.STRENGTH, Prayers.PROTECT_FROM_MELEE, false, ignore);
		...
		case MAGIC:
			...
			/*for(final Player near : attacker.getLocalPlayers()) {
				if(near != null && Misc.getDistance(near.getLocation(), attacker.getLocation()) < 10 && near != victim && near != attacker && near.getSkills().getLevel(Skills.HITPOINTS) > 0) {
					...
					World.getWorld().submit(new Tickable(hitDelay) {
						public void execute() {
							int damage = damage(maxHitRange, attacker, near, AttackType.MAGIC, Skills.MAGIC, Prayers.PROTECT_FROM_MAGIC, false, false);
			...
			attacker.playProjectile(Projectile.create(attacker.getCentreLocation(), victim.getCentreLocation(), 1225, 45, 50, rClientSpeed, 43, 35, victim.getProjectileLockonIndex(), 10, 48));
			//victim.playGraphics(Graphic.create(346, rGfxDelay, 100));

			blockAnimation = false;
			randomHit = damage(maxHit, attacker, victim, AttackType.MAGIC, Skills.MAGIC, Prayers.PROTECT_FROM_MAGIC, false, false);
			randomHit = random.nextInt(randomHit < 1 ? 1 : randomHit + 1);
			if(randomHit > victim.getSkills().getLevel(Skills.HITPOINTS)) {
				randomHit = victim.getSkills().getLevel(Skills.HITPOINTS);
			}
...
////				victim.getCombatState().setPoisonDamage(16, attacker);
//			}
			break;
		}		
