			...
			for(final Player near : localPlayers) {
				if(near != null && near != attacker && near.getSkills().getLevel(Skills.HITPOINTS) > 0) {
					if (attacker.getCentreLocation().isWithinDistance(attacker, near, 10)) {
						...
						attacker.playProjectile(Projectile.create(attacker.getCentreLocation(), near.getCentreLocation(), 316, 45, 50, rClientSpeed, 43, 35, near.getProjectileLockonIndex(), 10, 48));
					}
				...
				public void execute() {
					...
					for(final Player near : localPlayers) {
						if(near != null && near != attacker && near != victim && near.getSkills().getLevel(Skills.HITPOINTS) > 0) {
							if(attacker.getCentreLocation().isWithinDistance(attacker, near, 10)) {
								int hitz = MagicCombatAction.getAction().damage(maxHit, attacker, near, AttackType.MAGIC, Skills.MAGIC, Prayers.PROTECT_FROM_MAGIC, false, true);
