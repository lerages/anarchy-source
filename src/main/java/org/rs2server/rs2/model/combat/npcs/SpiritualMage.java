...
public class SpiritualMage extends AbstractCombatAction {
	...
	public void hit(final Mob attacker, final Mob victim) {
		...
		case MAGIC:
			...
			attacker.playProjectile(Projectile.create(attacker.getCentreLocation(), victim.getCentreLocation(), -1, 45, 50, clientSpeed, 10, 35, victim.getProjectileLockonIndex(), 10, 48));
		...
		World.getWorld().submit(new Tickable(hitDelay) {
			@Override
			public void execute() {
				victim.playGraphics(Graphic.create(hit > 0 ? 78 : 85, 0, hit > 0 ? 0 : 100));
				if(hit > 0) {
