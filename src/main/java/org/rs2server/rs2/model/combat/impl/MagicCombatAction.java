...
public class MagicCombatAction extends AbstractCombatAction {
	...
	public void hit(final Mob attacker, final Mob victim) {
		...
		// straight dont get this rofl, some etard shit going on here
		executeSpell(spell, attacker, attacker, victim, spell,
				attacker.getAutocastSpell() != null, clientSpeed, INSTANCE,
				gfxDelay, delay);
	...
	@Override
	public int damage(int maxHit, Mob attacker, Mob victim,
					  AttackType attackType, int skill, int prayer, boolean special,
					  boolean ignorePrayers) {
		/*double attackBonus = attacker.getCombatState().getBonus(
				attackType.getId()) == 0 ? 1 : attacker.getCombatState()
				.getBonus(attackType.getId());
		if (attackBonus < 1) {
			attackBonus = 1;
		}
		...
		double defenceCalc = defenceBonus * victim.getSkills().getLevel(Skills.DEFENCE); // +1 as its
		// exclusive
		...
		*//**
		...
		if (random.nextFloat() <= 0.25f && CombatFormulae.fullAhrim(attacker) && CombatFormulae.hasAmuletOfTheDamned(attacker)) {
			attackCalc *= 1.3;
		}
		*/
		...
		double bonus1 = 1;
		double attackCalc = attackBonus * (attacker.getSkills().getLevel(skill) * bonus1); // +1 as its exclusive
		if (attacker.isPlayer() && victim.isNPC()) {
			final Player player = (Player) attacker;
			final NPC npc = (NPC) victim;
			if (player.getSlayer().getSlayerTask() != null && player.getSlayer().getSlayerTask().getName().contains(npc.getDefinition().getName())
					&& CombatFormulae.hasImbuedSlayerHelm(player)) {
				attackCalc *= 1.15;
				maxHit *= 1.15;
			}
		}
		...
		*//*
		...
		if (weapon != null
				&& (weapon.getId() == 11791 || weapon.getId() == 12904)) {
		...
		double EA = attacker.getSkills().getLevel(Skills.MAGIC) + attacker.getSkills().getLevelForExperience(Skills.MAGIC);
		if (attacker.getCombatState().getPrayer(Prayers.MYSTIC_WILL)) {
			EA *= 1.05;
		} else if (attacker.getCombatState().getPrayer(Prayers.MYSTIC_LORE)) {
			EA *= 1.10;
		...
		EA = Math.round(EA) + 14;
		if (CombatFormulae.fullVoidMage(attacker)) {
			EA *= 1.3;
		}
		if (attacker.isPlayer() && victim.isNPC()) {
			final Player player = (Player) attacker;
		...
		double EM = victim.getSkills().getLevel(Skills.DEFENCE) + victim.getSkills().getLevelForExperience(Skills.DEFENCE);
		EM *= 0.1;
		...
		if (hitSucceed < random.nextDouble()) {
			 ...
			 * protection prayer, their damage is simply reduced by 40%.
			 */
	...
	public void addExperience(Mob attacker, int damage) {
		...
		if (attacker.getCombatState().getCombatStyle() == CombatStyle.DEFENSIVE_AUTOCAST) {
			attacker.getSkills().addExperience(Skills.MAGIC,
					(damage) * 4 * Constants.COMBAT_EXP);//1.33
			attacker.getSkills().addExperience(Skills.DEFENCE,
					(damage) * 4 * Constants.COMBAT_EXP);
		} else {
			attacker.getSkills().addExperience(Skills.MAGIC,
					(damage) * 4 * Constants.COMBAT_EXP);
		}
	...
	public static void executeSpell(Spell spell, final Mob mob, Object... args) {
		...
		if (mob.isNPC()) {
			...
			if (npc.getCombatDefinition().getSpell() == null || npc.getCombatDefinition().getSpell() == Spell.CARRALLANGAR_TELEPORT) {
				npc.playAnimation(npc.getCombatDefinition().getAttack());
				if (npc.getDrawbackGraphic() != null)
					npc.playGraphics(npc.getDrawbackGraphic());
			}

			npc.playProjectile(Projectile.create(npc.getCentreLocation(),
					victim.getCentreLocation(), npc.getCombatDefinition()
							.getProjectileId(), 60, 50, (Integer) args[4], 43,
					35, victim.getProjectileLockonIndex(), 15, 48));

			((MagicCombatAction) args[5]).hitEnemy(mob, victim, npc
							.getCombatDefinition().getSpell(), null, PoisonType.NONE,
					false, npc.getCombatDefinition().getMaxHit(),
					(Integer) args[7], 0);
		} else if (spell == Spell.HOME_TELEPORT_MODERN
				|| spell == Spell.HOME_TELEPORT_LUNAR
				|| spell == Spell.HOME_TELEPORT_ANCIENT || spell == Spell.HOME_TELEPORT_ARCEUUS) {
			// if(BoundaryManager.isWithinBoundaryNoZ(mob.getLocation(),
			// World.getWorld().getFightPits().getBoundary()) ||
			// BoundaryManager.isWithinBoundaryNoZ(mob.getLocation(),
			...
			if (((Player) mob).getBountyHunter() != null) {
				mob.getActionSender()
						.sendMessage("You can't teleport in here.");
				return;
			}
			...
			mob.getActionQueue().addAction(new Action(mob, 2) {
				...
				public void execute() {
					...
					mob.playSound(Sound.HOME_TELEPORT_START);
					this.stop();
				}
			});
			mob.getActionQueue().addAction(new Action(mob, 8) {
				@Override
				public CancelPolicy getCancelPolicy() {
					return CancelPolicy.ALWAYS;
				}
				...
				public void execute() {
					...
					mob.playSound(Sound.HOME_TELEPORT_NEXT);
					this.stop();
				}
			});
			mob.getActionQueue().addAction(new Action(mob, 11) {
				...
				public void execute() {
					...
					mob.playSound(Sound.HOME_TELEPORT_END);
					this.stop();
				}
			});
			...
			mob.getActionQueue().addAction(new Action(mob, 19) {
				...
				@Override
				public void execute() {
					mob.setTeleportTarget(Entity.HOME);
					this.stop();
				}
			});
		} else if (!Teleporting.teleport(mob, spell)
				&& ScriptManager.getScriptManager().invokeWithFailTest(
				spell.getSpellName(), args)) {
			int random = Misc.random(7);
			for (int i = 0; i < spell.getRunes().length; i++) {
//				if (mob.getEquipment() != null && mob.getEquipment().get(Equipment.SLOT_WEAPON) != null && (mob.getEquipment().get(Equipment.SLOT_WEAPON).getId() != 11791 && mob
//						.getEquipment().get(Equipment.SLOT_WEAPON).getId() != 12904
//						&& random != 7)) {
				if (mob.getInventory() != null && spell.getRune(i) != null) {
//						if (Inventory.removeRune(mob, spell.getRune(i)) > 0) {
...
//						}
			...
			if (mob.getCombatState().spellbookSwap()
					&& spell != Spell.SPELLBOOK_SWAP) {
				mob.getActionSender().sendSidebarInterface(
						105,
						MagicCombatAction.SpellBook.LUNAR_MAGICS
								.getInterfaceId());
				mob.getCombatState().setSpellBook(
						MagicCombatAction.SpellBook.LUNAR_MAGICS
								.getSpellBookId());
				mob.getCombatState().setSpellbookSwap(false);
			}
		} else if (spell == Spell.BOUNTY_LOCATE || spell == Spell.BOUNTY_LOCATE_ANCIENTS || spell == Spell.BOUNTY_LOCATE_LUNAR) {
			Player player = (Player) mob;
			if (player.hasAttribute("teleporting") || player.getBountyTarget() == null || !player.isInWilderness() || !player.getBountyTarget().isInWilderness() || player.getDatabaseEntity().getPlayerSettings().isTeleBlocked()) {
				return;
			}
			if (mob.hasAttribute("lastBountyTele") && System.currentTimeMillis() - (long) mob.getAttribute("lastBountyTele") < 180000) {
			...
			for (int x = victim.getLocation().getX() - 7; x < victim.getLocation().getX() + 7; x++) {
				for (int y = victim.getLocation().getY() - 7; y < victim.getLocation().getY() + 7; y++) {
					Location loc = Location.create(x, y, victim.getLocation().getZ());
					if (Location.inWild(loc) && RegionClipping.getClippingMask(loc.getX(), loc.getY(), loc.getZ()) < 1) {
						locations.add(loc);
					}
				}
			}
...
//		} else if (spell == Spell.BOUNTY_LOCATE) {
//			for (int i = 0; i < spell.getRunes().length; i++) {
//				if (mob.getInventory() != null && spell.getRune(i) != null) {
...
//			Player victim = ((Player) mob).getBountyHunter().getTarget();
//			for (int x = victim.getLocation().getX() - 7; x < victim
//					.getLocation().getX() + 7; x++) {
//				for (int y = victim.getLocation().getY() - 7; y < victim
//						.getLocation().getY() + 7; y++) {
//					if (RegionClipping.getClippingMask(x, y, victim
//							.getLocation().getZ()) < 1) {
//						locations.add(Location.create(x, y, victim
//								.getLocation().getZ()));
//					}
//				}
//			}
//			mob.playAnimation(Animation.create(7524));
//			mob.playGraphics(Graphic.create(1275));
...
//					mob.playAnimation(Animation.create(7526));
//					mob.playGraphics(Graphic.create(1280));
//					mob.setTeleportTarget(locations.get(mob.getRandom()
//							.nextInt(locations.size())));
//				}
//			});
		} else if (mob.getActionSender() != null) {
			if (!Teleporting.teleport(mob, spell)) {
				// mob.getActionSender().sendMessage("Spell script missing: " +
				// spell.getSpellName() + ".");
			}
		}
	}

	public static boolean canCastSpell(final Mob attacker, Mob victim,
									   Spell spell) {
		if (attacker.isNPC()) {
			return true;
		}
		...
		if (attacker.getSkills().getLevel(Skills.MAGIC) < spell
				.getLevelRequired()) {
		...
		if (!attacker.isNPC()) {
			switch (spell.getSpellBook()) {
				case MODERN_MAGICS:
					if (spell != Spell.TRIDENT_OF_THE_SEAS && spell != Spell.TRIDENT_OF_THE_SWAMP && (attacker.getCombatState().getSpellBook() == SpellBook.ANCIENT_MAGICKS.getSpellBookId() ||
					...
					if (spell == Spell.CHARGE) {
						if (attacker.getCombatState().isCharged()) {
							if (attacker.getActionSender() != null) {
								attacker.getActionSender().sendMessage(
										"Your current charge is too powerful.");
							}
							return false;
						}
					}
				...
				case ANCIENT_MAGICKS:
					if (attacker.getCombatState().getSpellBook() == SpellBook.MODERN_MAGICS.getSpellBookId() ||attacker.getCombatState().getSpellBook() == SpellBook.LUNAR_MAGICS.getSpellBookId()) {
						return false;
					}
					break;
				case LUNAR_MAGICS:
					if (attacker.getCombatState().getSpellBook() == SpellBook.MODERN_MAGICS.getSpellBookId() || attacker.getCombatState().getSpellBook() == SpellBook.ANCIENT_MAGICKS.getSpellBookId()) {
						return false;
					}
					if (attacker.getSkills().getLevelForExperience(Skills.DEFENCE) < 45) {
						if (attacker.getActionSender() != null) {
							attacker.getActionSender()
									.sendMessage(
											"You need a Defence level of 45 to use Lunar spells.");
						}
						return false;
					}
		...
		if (spell.getRequiredItem() != null) {
			if (attacker.isPlayer()
					&& attacker.getEquipment().getCount(
					spell.getRequiredItem().getId()) < spell
					.getRequiredItem().getCount()) {
				attacker.getActionSender()
						.sendMessage(
								"You must be wielding a "
										+ spell.getRequiredItem().getDefinition2().getName()
										+ " to cast this spell.");
				return false;
			}
		}
...
//							.sendMessage(
//									"You need to be in bounty hunter to use this spell.");
//					return false;
//				}
//				if (player.getBountyHunter().getTarget() == null) {
//					player.getActionSender().sendMessage(
//							"You need to have a target to use this spell.");
//					return false;
//				}
//				if (player.getCombatState().getLastHitTimer() > System
//						.currentTimeMillis()) {
//					player.getActionSender().sendMessage(
//							"You can't cast this during combat.");
//					return false;
//				}
//				if (player.getBountyHunter().getTarget().getBountyHunter() == null) {
//					return false;
//				}
//			}
//			break;
			...
			case STUN:
				if (victim != null && victim.isAttackDrained()) {
					attacker.getActionSender().sendMessage(
							"Your foe's attack has already been weakened.");
					return false;
				}
				break;
			case WEAKEN:
			case ENFEEBLE:
				if (victim != null && victim.isStrengthDrained()) {
					attacker.getActionSender().sendMessage(
							"Your foe's strength has already been weakened.");
					return false;
				}
				break;
			case CURSE:
			case VULNERABILITY:
				if (victim != null && victim.isDefenceDrained()) {
					attacker.getActionSender().sendMessage(
							"Your foe's defence has already been weakened.");
					return false;
				}
				break;
			case CRUMBLE_UNDEAD:
				if (!victim.getDefinedName().toLowerCase().contains("skeleton")
						&& !victim.getDefinedName().toLowerCase()
						.contains("zombie")
	...
	public void hitEnemy(final Mob attacker, Mob victim, final Spell spell,
						 final Graphic graphic, final PoisonType poisonType, boolean multi,
						 int maxDamage, final int delay, final int freezeTimer) {
		if (!BoundaryManager.isWithinBoundaryNoZ(attacker.getLocation(),
				"MultiCombat") || !attacker.inMulti()) {
			multi = false;
		}
		...
		for (int i = 0; i < enemies.size(); i++) {
			...
			int hit = damage(maxDamage, attacker, enemy, AttackType.MAGIC, Skills.MAGIC, Prayers.PROTECT_FROM_MAGIC, false, false);
			//tome of fire
			//this code is terrible but it's required for tome of fire to work.
			/*if (spell.getSpellName().toLowerCase().contains("fire"))
			{
				fire = true;
			}*/
			if (graphic != null) {
				...
				if (isPlayer) {
					...
					if (player.isMultiplayerDisabled()) {
						player.getActionSender().sendStillGFX(hit < 0 ? 85 : graphic.getId(), hit < 0 ? 100 : graphic.getHeight(), enemy.getCentreLocation());
					} else {
						enemy.playGraphics(hit < 0 ? Graphic.create(85,
								graphic.getDelay(), 100) : graphic);
					}
				} else {
					enemy.playGraphics(hit < 0 ? Graphic.create(85,
							graphic.getDelay(), 100) : graphic);
			...
			if (hit > -1) {
				hit = (hit > 0 ? random.nextInt(hit) : 0)
						+ (maxDamage > 0 ? 1 : 0); // 1 because we started at -1
				if (hit > 0) {
					...
					if (freezeTimer > 0) {
						if (enemy.getCombatState().canMove()
								&& enemy.getCombatState().canBeFrozen()) {
							/*
							 * If the enemy has protect from magic, freeze time
							 * is halved.
							 */
							...
							if (enemy.getActionSender() != null) {
								enemy.getActionSender().sendMessage(
										"You have been frozen!");
							}
							World.getWorld().submit(
									new Tickable(freezeTimer + delay) {
										public void execute() {
											enemy.getCombatState().setCanMove(
													true);
							...
							World.getWorld().submit(
									new Tickable(freezeTimer + delay + 3) {
										public void execute() {
					...
					if (CombatFormulae.fullAhrim(attacker)) {
						...
						if (ahrim == 1) {
							victim.getSkills().decreaseLevelToMinimum(
									Skills.STRENGTH, 5);
				...
				for (int i = 0; i < enemies.size(); i++) {
					...
					if (attacker.isPlayer()) {
						...
						if (victim instanceof Whirlpool) {
							...
							if (kraken.getDisturbedWhirlpools().size() == 4 && kraken.getTransformId() != KrakenServiceImpl.KRAKEN) {
					...
					enemy.getActiveCombatAction()
							.defend(attacker, enemy, false);

					if (hit != -1) { // the effect will not occur if we have
						// splashed
						if (poisonType != PoisonType.NONE) {
							if (enemy.getCombatState().getPoisonDamage() < 1
									&& enemy.getCombatState().canBePoisoned()
									&& random.nextInt(11) == 3) {
								enemy.getCombatState().setPoisonDamage(
										poisonType.getRangeDamage(), attacker);
								if (enemy.getActionSender() != null) {
						...
						switch (spell) {
							case SHADOW_RUSH:
							case SHADOW_BURST:
								enemy.getSkills().decreaseLevelOnce(
										Skills.ATTACK,
										(int) (enemy.getSkills()
							...
							case BLOOD_BARRAGE:
								...
								attacker.getSkills().increaseLevelToMaximum(
										Skills.HITPOINTS, heal);
								if (attacker.getActionSender() != null) {
									attacker.getActionSender()
											.sendMessage(
													"You drain some of your opponent's health.");
								}
								break;
							case TELE_BLOCK:
								if(enemy.isPlayer()) {
									...
									opp.getDatabaseEntity().getPlayerSettings().setTeleBlockTimer(opp.getCombatState().getPrayer(Prayers.PROTECT_FROM_MAGIC) ? 250 : 500);
							...
							case FLAMES_OF_ZAMORAK:
								enemy.getSkills()
										.decreaseLevelOnce(Skills.MAGIC, 5);
								break;
							case CONFUSE:
						...
						if (drainedLevel != -1 && drainModifier != 0) {
							int levelBefore = enemy.getSkills().getLevel(
									drainedLevel);
							enemy.getSkills()
									.decreaseLevelOnce(
											drainedLevel,
											(int) (drainModifier * enemy
													.getSkills()
													.getLevelForExperience(
															drainedLevel)));
							int levelDifference = levelBefore
									- enemy.getSkills().getLevel(drainedLevel);
							if (levelDifference < 1) {
								levelDifference = 1;
							}
							World.getWorld().submit(
									new Tickable(levelDifference
											* SkillsUpdateTick.CYCLE_TIME) {
										public void execute() {
											switch (spell) {
												case CONFUSE:
												case STUN:
		...
		if (spell.getRunes() != null) {
			for (int i = 0; i < spell.getRunes().length; i++) {
				if (spell.getRune(i) != null) {
					if (!deleteRune(mob, spell.getRune(i))) {
						if (mob.getInventory() != null
								&& mob.getInventory().getCount(spell.getRune(i).getId()) < spell.getRune(i).getCount()) {
							String runeName = NameUtils.formatName(spell.getRune(i).getDefinition2().getName()).toLowerCase();
							if (mob.getActionSender() != null) {
								mob.getActionSender().sendMessage(
										"You do not have enough " + runeName
	...
	public static void setAutocast(Mob mob, Spell spell, int config,
								   boolean defensive) {
		/*
		 * if(defensive) { config += 93; }
		 */
		if (mob.getEquipment() != null && spell != null) {
			mob.getEquipment().fireItemChanged(Equipment.SLOT_WEAPON);
		}
		if (spell == null) {// we don't want to set our atk style back to
			// accurate if we aren't already autocasting
			if (mob.getCombatState().getCombatStyle() == CombatStyle.AUTOCAST
				...
				if (mob.getAttribute("lastAttackType") != null) {
					mob.getCombatState().setAttackType(
							(AttackType) mob.getAttribute("lastAttackType"));
					mob.getCombatState().setCombatStyle(
							(CombatStyle) mob.getAttribute("lastCombatStyle"));
			...
			mob.setAttribute("lastAttackType", mob.getCombatState()
					.getAttackType());
			mob.setAttribute("lastCombatStyle", mob.getCombatState()
					.getCombatStyle());
			mob.getCombatState().setCombatStyle(
					defensive ? CombatStyle.DEFENSIVE_AUTOCAST
							: CombatStyle.AUTOCAST);
			mob.getInterfaceState().setLastUsedAutocast(config);
			if (mob.getActionSender() != null) {
				mob.getActionSender().sendConfig(43, 4);
				mob.getActionSender().sendConfig(108, config);
				mob.getActionSender().sendString(593, 2, "Combat lvl: " + mob.getSkills().getCombatLevel());
				// mob.getActionSender().sendInterfaceConfig(90, defensive ? 176
				// : 83, true); //Hides the spellbook icon
				// mob.getActionSender().sendInterfaceConfig(90, config, false);
				// //Shows the spell icon
			}
		}
...
//		mob.getActionSender().sendSidebarInterfaces();
		mob.setAutocastSpell(spell);
	}

	public static final int FIRE_RUNE = 554, WATER_RUNE = 555, AIR_RUNE = 556,
			EARTH_RUNE = 557, MIND_RUNE = 558, BODY_RUNE = 559,
			DEATH_RUNE = 560, NATURE_RUNE = 561, CHAOS_RUNE = 562,
			LAW_RUNE = 563, COSMIC_RUNE = 564, BLOOD_RUNE = 565,
			SOUL_RUNE = 566, ASTRAL_RUNE = 9075;
	...
	public static enum Spell {
		...
		WIND_STRIKE(1, 1, SpellBook.MODERN_MAGICS, SpellType.COMBAT,
				new Item[]{new Item(AIR_RUNE, 1), new Item(MIND_RUNE, 1)},
				"windStrike", null),
		...
		WATER_STRIKE(4, 5, SpellBook.MODERN_MAGICS, SpellType.COMBAT,
				new Item[]{new Item(WATER_RUNE, 1), new Item(AIR_RUNE, 1),
						new Item(MIND_RUNE, 1)}, "waterStrike", null),

		/**
		 * Earth Strike.
		 */
		EARTH_STRIKE(6, 9, SpellBook.MODERN_MAGICS, SpellType.COMBAT,
				new Item[]{new Item(EARTH_RUNE, 2), new Item(AIR_RUNE, 1),
						new Item(MIND_RUNE, 1)}, "earthStrike", null),

		/**
		 * Fire Strike.
		 */
		FIRE_STRIKE(8, 13, SpellBook.MODERN_MAGICS, SpellType.COMBAT,
				new Item[]{new Item(FIRE_RUNE, 3), new Item(AIR_RUNE, 2),
						new Item(MIND_RUNE, 1)}, "fireStrike", null),

		/**
		 * Wind Bolt.
		 */
		WIND_BOLT(10, 17, SpellBook.MODERN_MAGICS, SpellType.COMBAT,
				new Item[]{new Item(AIR_RUNE, 2), new Item(CHAOS_RUNE, 1)},
				"windBolt", null),
		...
		WATER_BOLT(14, 23, SpellBook.MODERN_MAGICS, SpellType.COMBAT,
				new Item[]{new Item(AIR_RUNE, 2), new Item(WATER_RUNE, 2),
						new Item(CHAOS_RUNE, 1)}, "waterBolt", null),

		/**
		 * Earth Bolt.
		 */
		EARTH_BOLT(17, 29, SpellBook.MODERN_MAGICS, SpellType.COMBAT,
				new Item[]{new Item(AIR_RUNE, 2), new Item(EARTH_RUNE, 3),
						new Item(CHAOS_RUNE, 1)}, "earthBolt", null),

		/**
		 * Fire Bolt.
		 */
		FIRE_BOLT(20, 35, SpellBook.MODERN_MAGICS, SpellType.COMBAT,
				new Item[]{new Item(AIR_RUNE, 3), new Item(FIRE_RUNE, 5),
						new Item(CHAOS_RUNE, 1)}, "fireBolt", null),

		/**
		 * Wind Blast.
		 */
		WIND_BLAST(24, 41, SpellBook.MODERN_MAGICS, SpellType.COMBAT,
				new Item[]{new Item(AIR_RUNE, 3), new Item(DEATH_RUNE, 1)},
				"windBlast", null),
		...
		WATER_BLAST(27, 47, SpellBook.MODERN_MAGICS, SpellType.COMBAT,
				new Item[]{new Item(AIR_RUNE, 3), new Item(WATER_RUNE, 3),
						new Item(DEATH_RUNE, 1)}, "waterBlast", null),

		/**
		 * Earth Blast.
		 */
		EARTH_BLAST(33, 53, SpellBook.MODERN_MAGICS, SpellType.COMBAT,
				new Item[]{new Item(AIR_RUNE, 3), new Item(EARTH_RUNE, 4),
						new Item(DEATH_RUNE, 1)}, "earthBlast", null),

		/**
		 * Fire Blast.
		 */
		FIRE_BLAST(38, 59, SpellBook.MODERN_MAGICS, SpellType.COMBAT,
				new Item[]{new Item(AIR_RUNE, 4), new Item(FIRE_RUNE, 5),
						new Item(DEATH_RUNE, 1)}, "fireBlast", null),

		/**
		 * Wind Wave.
		 */
		WIND_WAVE(45, 62, SpellBook.MODERN_MAGICS, SpellType.COMBAT,
				new Item[]{new Item(AIR_RUNE, 5), new Item(BLOOD_RUNE, 1)},
				"windWave", null),
		...
		WATER_WAVE(48, 65, SpellBook.MODERN_MAGICS, SpellType.COMBAT,
				new Item[]{new Item(AIR_RUNE, 5), new Item(WATER_RUNE, 7),
						new Item(BLOOD_RUNE, 1)}, "waterWave", null),

		/**
		 * Earth Wave.
		 */
		EARTH_WAVE(53, 73, SpellBook.MODERN_MAGICS, SpellType.COMBAT,
				new Item[]{new Item(AIR_RUNE, 5), new Item(EARTH_RUNE, 7),
						new Item(BLOOD_RUNE, 1)}, "earthWave", null),

		/**
		 * Fire Wave.
		 */
		FIRE_WAVE(56, 75, SpellBook.MODERN_MAGICS, SpellType.COMBAT,
				new Item[]{new Item(AIR_RUNE, 5), new Item(FIRE_RUNE, 7),
						new Item(BLOOD_RUNE, 1)}, "fireWave", null),

		/**
		 * Trident of the seas
		 */
		TRIDENT_OF_THE_SEAS(-1, 75, SpellBook.MODERN_MAGICS, SpellType.COMBAT,
				new Item[0], "tridentOfTheSeas", null),

		/**
		 * Trident of the swamp
		 */
		TRIDENT_OF_THE_SWAMP(-1, 75, SpellBook.MODERN_MAGICS, SpellType.COMBAT,
				new Item[0], "tridentOfTheSwamp", null),

		/**
		 * Saradomin Strike.
		 */
		SARADOMIN_STRIKE(41, 60, SpellBook.MODERN_MAGICS, SpellType.COMBAT,
				new Item[]{new Item(FIRE_RUNE, 2), new Item(BLOOD_RUNE, 2),
						new Item(AIR_RUNE, 4)}, "saradominStrike", new Item(
				2415)),
		...
		CLAWS_OF_GUTHIX(42, 60, SpellBook.MODERN_MAGICS, SpellType.COMBAT,
				new Item[]{new Item(FIRE_RUNE, 1), new Item(BLOOD_RUNE, 2),
						new Item(AIR_RUNE, 4)}, "clawsOfGuthix",
				new Item(2416)),
		...
		FLAMES_OF_ZAMORAK(43, 60, SpellBook.MODERN_MAGICS, SpellType.COMBAT,
				new Item[]{new Item(FIRE_RUNE, 4), new Item(BLOOD_RUNE, 2),
						new Item(AIR_RUNE, 1)}, "flamesOfZamorak", new Item(
				2417)),
		...
		IBAN_BLAST(29, 50, SpellBook.MODERN_MAGICS, SpellType.COMBAT,
				new Item[]{new Item(FIRE_RUNE, 5), new Item(DEATH_RUNE, 1)},
				"ibanBlast", new Item(1409)),

		/**
		 * Magic Dart.
		 */
		MAGIC_DART(31, 50, SpellBook.MODERN_MAGICS, SpellType.COMBAT,
				new Item[]{new Item(MIND_RUNE, 4), new Item(DEATH_RUNE, 1)},
				"magicDart", new Item(4170)),

		/**
		 * Magic Dart upgraded
		 */
		MAGIC_DART2(31, 50, SpellBook.MODERN_MAGICS, SpellType.COMBAT,
				new Item[]{new Item(MIND_RUNE, 4), new Item(DEATH_RUNE, 1)},
				"magicDart", new Item(21255)),
		
		/**
		 * Crumble Undead.
		 */
		CRUMBLE_UNDEAD(22, 39, SpellBook.MODERN_MAGICS, SpellType.COMBAT,
				new Item[]{new Item(EARTH_RUNE, 2), new Item(AIR_RUNE, 2),
						new Item(CHAOS_RUNE, 1)}, "crumbleUndead", null),

		/**
		 * Smoke Rush.
		 */
		SMOKE_RUSH(74, 50, SpellBook.ANCIENT_MAGICKS, SpellType.COMBAT,
				new Item[]{new Item(DEATH_RUNE, 2), new Item(CHAOS_RUNE, 2),
						new Item(FIRE_RUNE, 1)}, "smokeRush", null),
		...
		SHADOW_RUSH(78, 52, SpellBook.ANCIENT_MAGICKS, SpellType.COMBAT,
				new Item[]{new Item(DEATH_RUNE, 2), new Item(CHAOS_RUNE, 2),
						new Item(SOUL_RUNE, 1)}, "shadowRush", null),
		...
		BLOOD_RUSH(70, 56, SpellBook.ANCIENT_MAGICKS, SpellType.COMBAT,
				new Item[]{new Item(DEATH_RUNE, 2), new Item(CHAOS_RUNE, 2),
						new Item(BLOOD_RUNE, 1)}, "bloodRush", null),
		...
		ICE_RUSH(66, 58, SpellBook.ANCIENT_MAGICKS, SpellType.COMBAT,
				new Item[]{new Item(DEATH_RUNE, 2), new Item(CHAOS_RUNE, 2),
						new Item(WATER_RUNE, 2)}, "iceRush", null),

		/**
		 * Smoke Burst.
		 */
		SMOKE_BURST(76, 62, SpellBook.ANCIENT_MAGICKS, SpellType.COMBAT,
				new Item[]{new Item(DEATH_RUNE, 2), new Item(CHAOS_RUNE, 4),
						new Item(AIR_RUNE, 2)}, "smokeBurst", null),

		/**
		 * Shadow Burst.
		 */
		SHADOW_BURST(80, 64, SpellBook.ANCIENT_MAGICKS, SpellType.COMBAT,
				new Item[]{new Item(DEATH_RUNE, 2), new Item(CHAOS_RUNE, 4),
						new Item(AIR_RUNE, 2), new Item(AIR_RUNE, 2)},
				"shadowBurst", null),
		...
		BLOOD_BURST(72, 68, SpellBook.ANCIENT_MAGICKS, SpellType.COMBAT,
				new Item[]{new Item(DEATH_RUNE, 2), new Item(CHAOS_RUNE, 4),
						new Item(BLOOD_RUNE, 2)}, "bloodBurst", null),
		...
		ICE_BURST(68, 70, SpellBook.ANCIENT_MAGICKS, SpellType.COMBAT,
				new Item[]{new Item(DEATH_RUNE, 2), new Item(CHAOS_RUNE, 4),
						new Item(WATER_RUNE, 2)}, "iceBurst", null),

		/**
		 * Smoke Blitz.
		 */
		SMOKE_BLITZ(75, 74, SpellBook.ANCIENT_MAGICKS, SpellType.COMBAT,
				new Item[]{new Item(DEATH_RUNE, 2), new Item(FIRE_RUNE, 2),
						new Item(BLOOD_RUNE, 2), new Item(AIR_RUNE, 2)},
				"smokeBlitz", null),

		/**
		 * Shadow Blitz.
		 */
		SHADOW_BLITZ(79, 76, SpellBook.ANCIENT_MAGICKS, SpellType.COMBAT,
				new Item[]{new Item(DEATH_RUNE, 2), new Item(BLOOD_RUNE, 2),
						new Item(AIR_RUNE, 2), new Item(SOUL_RUNE, 2)},
				"shadowBlitz", null),
		...
		BLOOD_BLITZ(
				71,
				80,
				SpellBook.ANCIENT_MAGICKS,
				SpellType.COMBAT,
				new Item[]{new Item(DEATH_RUNE, 2), new Item(BLOOD_RUNE, 4)},
				"bloodBlitz", null),
		...
		ICE_BLITZ(67, 82, SpellBook.ANCIENT_MAGICKS, SpellType.COMBAT,
				new Item[]{new Item(DEATH_RUNE, 2), new Item(BLOOD_RUNE, 2),
						new Item(WATER_RUNE, 3)}, "iceBlitz", null),

		/**
		 * Smoke Barrage.
		 */
		SMOKE_BARRAGE(77, 86, SpellBook.ANCIENT_MAGICKS, SpellType.COMBAT,
				new Item[]{new Item(DEATH_RUNE, 4), new Item(BLOOD_RUNE, 2),
						new Item(AIR_RUNE, 4), new Item(FIRE_RUNE, 4)},
				"smokeBarrage", null),

		/**
		 * Shadow Barrage.
		 */
		SHADOW_BARRAGE(81, 88, SpellBook.ANCIENT_MAGICKS, SpellType.COMBAT,
				new Item[]{new Item(DEATH_RUNE, 4), new Item(BLOOD_RUNE, 2),
						new Item(AIR_RUNE, 4), new Item(SOUL_RUNE, 3)},
				"shadowBarrage", null),
		...
		BLOOD_BARRAGE(73, 92, SpellBook.ANCIENT_MAGICKS, SpellType.COMBAT,
				new Item[]{new Item(DEATH_RUNE, 4), new Item(BLOOD_RUNE, 4),
						new Item(SOUL_RUNE, 1)}, "bloodBarrage", null),
		...
		ICE_BARRAGE(69, 94, SpellBook.ANCIENT_MAGICKS, SpellType.COMBAT,
				new Item[]{new Item(DEATH_RUNE, 4), new Item(BLOOD_RUNE, 2),
						new Item(WATER_RUNE, 6)}, "iceBarrage", null),

		/**
		 * Bind.
		 */
		BIND(12, 20, SpellBook.MODERN_MAGICS, SpellType.COMBAT, new Item[]{
				new Item(EARTH_RUNE, 3), new Item(WATER_RUNE, 3),
				new Item(NATURE_RUNE, 2)}, "bind", null),

		/**
		 * Snare.
		 */
		SNARE(30, 50, SpellBook.MODERN_MAGICS, SpellType.COMBAT, new Item[]{
				new Item(EARTH_RUNE, 4), new Item(WATER_RUNE, 4),
				new Item(NATURE_RUNE, 3)}, "snare", null),

		/**
		 * Entangle.
		 */
		ENTANGLE(57, 79, SpellBook.MODERN_MAGICS, SpellType.COMBAT, new Item[]{
				new Item(EARTH_RUNE, 5), new Item(WATER_RUNE, 5),
				new Item(NATURE_RUNE, 4)}, "entangle", null),

		/**
		 * Confuse.
		 */
		CONFUSE(2, 3, SpellBook.MODERN_MAGICS, SpellType.COMBAT, new Item[]{
				new Item(WATER_RUNE, 3), new Item(EARTH_RUNE, 2),
				new Item(BODY_RUNE, 1)}, "confuse", null),

		/**
		 * Weaken.
		 */
		WEAKEN(7, 11, SpellBook.MODERN_MAGICS, SpellType.COMBAT, new Item[]{
				new Item(WATER_RUNE, 3), new Item(EARTH_RUNE, 2),
				new Item(BODY_RUNE, 1)}, "weaken", null),

		/**
		 * Curse.
		 */
		CURSE(11, 19, SpellBook.MODERN_MAGICS, SpellType.COMBAT, new Item[]{
				new Item(WATER_RUNE, 2), new Item(EARTH_RUNE, 3),
				new Item(BODY_RUNE, 1)}, "curse", null),

		/**
		 * Vulnerability.
		 */
		VULNERABILITY(51, 76, SpellBook.MODERN_MAGICS, SpellType.COMBAT,
				new Item[]{new Item(WATER_RUNE, 6), new Item(EARTH_RUNE, 6),
						new Item(SOUL_RUNE, 1)}, "vulnerability", null),

		/**
		 * Enfeeble.
		 */
		ENFEEBLE(54, 73, SpellBook.MODERN_MAGICS, SpellType.COMBAT, new Item[]{
				new Item(WATER_RUNE, 8), new Item(EARTH_RUNE, 8),
				new Item(SOUL_RUNE, 1)}, "enfeeble", null),

		/**
		 * Stun.
		 */
		STUN(58, 80, SpellBook.MODERN_MAGICS, SpellType.COMBAT, new Item[]{
				new Item(WATER_RUNE, 12), new Item(EARTH_RUNE, 12),
				new Item(SOUL_RUNE, 1)}, "stun", null),

		/**
		 * Varrock teleport.
		 */
		VARROCK_TELEPORT(16, 25, SpellBook.MODERN_MAGICS, SpellType.NON_COMBAT,
				new Item[]{new Item(LAW_RUNE, 1), new Item(FIRE_RUNE, 1),
						new Item(AIR_RUNE, 3)}, "varrockTeleport", null),

		/**
		 * Bounty Locate
		 */
		BOUNTY_LOCATE(63, 85, SpellBook.MODERN_MAGICS, SpellType.NON_COMBAT,
				new Item[]{new Item(LAW_RUNE, 1), new Item(CHAOS_RUNE, 1),
						new Item(DEATH_RUNE, 1)}, "bountyLocate", null),

		BOUNTY_LOCATE_ANCIENTS(92, 85, SpellBook.ANCIENT_MAGICKS, SpellType.NON_COMBAT,
				new Item[]{new Item(LAW_RUNE, 1), new Item(CHAOS_RUNE, 1),
						new Item(DEATH_RUNE, 1)}, "bountyLocate", null),
		BOUNTY_LOCATE_LUNAR(123, 85, SpellBook.ANCIENT_MAGICKS, SpellType.NON_COMBAT,
				new Item[]{new Item(LAW_RUNE, 1), new Item(CHAOS_RUNE, 1),
						new Item(DEATH_RUNE, 1)}, "bountyLocate", null),
		 ...
		 * Lumbridge teleport.
		 */
		LUMBRIDGE_TELEPORT(19, 31, SpellBook.MODERN_MAGICS,
				SpellType.NON_COMBAT, new Item[]{new Item(LAW_RUNE, 1),
				new Item(EARTH_RUNE, 1), new Item(AIR_RUNE, 3)},
		...
		FALADOR_TELEPORT(22, 37, SpellBook.MODERN_MAGICS, SpellType.NON_COMBAT,
				new Item[]{new Item(LAW_RUNE, 1), new Item(WATER_RUNE, 1),
						new Item(AIR_RUNE, 3)}, "faladorTeleport", null),

		/**
		 * Camelot teleport.
		 */
		CAMELOT_TELEPORT(27, 45, SpellBook.MODERN_MAGICS, SpellType.NON_COMBAT,
				new Item[]{new Item(LAW_RUNE, 1), new Item(AIR_RUNE, 5)},
				"camelotTeleport", null),

		/**
		 * Ardougne teleport.
		 */
		ARDOUGNE_TELEPORT(33, 51, SpellBook.MODERN_MAGICS,
				SpellType.NON_COMBAT, new Item[]{new Item(LAW_RUNE, 2),
				new Item(WATER_RUNE, 2)}, "ardougneTeleport", null),

		/**
		 * Watchtower teleport.
		 */
		WATCHTOWER_TELEPORT(38, 58, SpellBook.MODERN_MAGICS,
				SpellType.NON_COMBAT, new Item[]{new Item(LAW_RUNE, 2),
				new Item(EARTH_RUNE, 2)}, "watchtowerTeleport", null),

		/**
		 * Trollheim teleport.
		 */
		TROLLHEIM_TELEPORT(45, 61, SpellBook.MODERN_MAGICS,
				SpellType.NON_COMBAT, new Item[]{new Item(LAW_RUNE, 2),
				new Item(FIRE_RUNE, 2)}, "trollheimTeleport", null),

		/**
		 * Paddewwa teleport.
		 */
		PADDEWWA_TELEPORT(84, 54, SpellBook.ANCIENT_MAGICKS,
				SpellType.NON_COMBAT, new Item[]{new Item(LAW_RUNE, 2),
				new Item(FIRE_RUNE, 2)}, "paddewwaTeleport", null),

		/**
		 * Senntisten teleport.
		 */
		SENNTISTEN_TELEPORT(85, 60, SpellBook.ANCIENT_MAGICKS,
				SpellType.NON_COMBAT, new Item[]{new Item(LAW_RUNE, 2),
				new Item(SOUL_RUNE, 1)}, "senntistenTeleport", null),

		/**
		 * Kharyrll teleport.
		 */
		KHARYRLL_TELEPORT(86, 66, SpellBook.ANCIENT_MAGICKS,
				SpellType.NON_COMBAT, new Item[]{new Item(LAW_RUNE, 2),
				new Item(BLOOD_RUNE, 1)}, "kharyrllTeleport", null),

		/**
		 * Lassar teleport.
		 */
		LASSAR_TELEPORT(87, 72, SpellBook.ANCIENT_MAGICKS,
				SpellType.NON_COMBAT, new Item[]{new Item(LAW_RUNE, 2),
				new Item(WATER_RUNE, 4)}, "lassarTeleport", null),

		/**
		 * Dareeyak teleport.
		 */
		DAREEYAK_TELEPORT(88, 78, SpellBook.ANCIENT_MAGICKS,
				SpellType.NON_COMBAT, new Item[]{new Item(LAW_RUNE, 2),
				new Item(FIRE_RUNE, 3), new Item(AIR_RUNE, 2)},
				"dareeyakTeleport", null),

		/**
		 * Carrallangar teleport.
		 */
		CARRALLANGAR_TELEPORT(89, 84, SpellBook.ANCIENT_MAGICKS,
				SpellType.NON_COMBAT, new Item[]{new Item(LAW_RUNE, 2),
				new Item(SOUL_RUNE, 2)}, "carrallangarTeleport", null),
		
		/**
		 * Bounty locate teleport.
		 */
		TELEPORT_TO_BOUNTY_TARGET(92, 85, SpellBook.ANCIENT_MAGICKS,
				SpellType.NON_COMBAT, new Item[]{new Item(LAW_RUNE, 2),
				new Item(DEATH_RUNE, 1), new Item(CHAOS_RUNE, 1)}, "teleportToBountyTarget", null),
		
		/**
		 * Annakarl teleport.
		 */
		ANNAKARL_TELEPORT(90, 90, SpellBook.ANCIENT_MAGICKS,
				SpellType.NON_COMBAT, new Item[]{new Item(LAW_RUNE, 2),
				new Item(BLOOD_RUNE, 2)}, "annakarlTeleport", null),

		/**
		 * Ghorrock teleport.
		 */
		GHORROCK_TELEPORT(91, 96, SpellBook.ANCIENT_MAGICKS,
				SpellType.NON_COMBAT, new Item[]{new Item(LAW_RUNE, 2),
				new Item(WATER_RUNE, 8)}, "ghorrockTeleport", null),
		/**
		 * Bake Pie
		 */
		BAKE_PIE(96, 65, SpellBook.LUNAR_MAGICS, SpellType.NON_COMBAT,
				new Item[]{new Item(ASTRAL_RUNE, 1), new Item(FIRE_RUNE, 5),
						new Item(WATER_RUNE, 1)}, "bakePie", null),

		/**
		 * NPC Contact
		 */
		NPC_CONTACT(96, 67, SpellBook.LUNAR_MAGICS, SpellType.NON_COMBAT,
				new Item[]{new Item(ASTRAL_RUNE, 1), new Item(COSMIC_RUNE, 1),
						new Item(AIR_RUNE, 2)}, "npcContact", null),
		...
		VENGEANCE(134, 94, SpellBook.LUNAR_MAGICS, SpellType.NON_COMBAT,
				new Item[]{new Item(ASTRAL_RUNE, 4), new Item(DEATH_RUNE, 2),
						new Item(EARTH_RUNE, 10)}, "vengeance", null),
		...
		VENGEANCE_OTHER(132, 93, SpellBook.LUNAR_MAGICS, SpellType.NON_COMBAT,
				new Item[]{new Item(ASTRAL_RUNE, 4), new Item(DEATH_RUNE, 2),
						new Item(EARTH_RUNE, 10)}, "vengeanceOther", null),
		
		/**
		 * Spellbook Swap
		 */
		SPELLBOOK_SWAP(136, 96, SpellBook.LUNAR_MAGICS, SpellType.NON_COMBAT,
				new Item[]{new Item(ASTRAL_RUNE, 3),
						new Item(COSMIC_RUNE, 2), new Item(LAW_RUNE, 1)},
				"spellbookSwap", null),
		...
		BARROWS_TELEPORT(172, 83, SpellBook.ARCEUUS_MAGICS, SpellType.NON_COMBAT,
				new Item[]{new Item(LAW_RUNE, 2), new Item(SOUL_RUNE, 2),
						new Item(BLOOD_RUNE, 1)}, "barrowsTeleport", null),
		 ...
		 */
		HOME_TELEPORT_MODERN(1, 1, SpellBook.MODERN_MAGICS,
				SpellType.NON_COMBAT, new Item[0], "homeTeleport", null),

		/**
		 * Home teleport lunar
		 */
		HOME_TELEPORT_LUNAR(95, 1, SpellBook.LUNAR_MAGICS,
				SpellType.NON_COMBAT, new Item[0], "homeTeleport", null),

		/**
		 * Home teleport ancient
		 */
		HOME_TELEPORT_ANCIENT(93, 1, SpellBook.ANCIENT_MAGICKS,
				SpellType.NON_COMBAT, new Item[0], "homeTeleport", null),
		
		/**
		 * Home teleport modern
		 */
		HOME_TELEPORT_ARCEUUS(141, 1, SpellBook.ARCEUUS_MAGICS,
				SpellType.NON_COMBAT, new Item[0], "homeTeleport", null),
		...
		CHARGE(58, 80, SpellBook.MODERN_MAGICS, SpellType.NON_COMBAT,
				new Item[]{new Item(FIRE_RUNE, 3), new Item(BLOOD_RUNE, 3),
						new Item(AIR_RUNE, 3)}, "charge", null),

		/**
		 * Bandos spiritual mage
		 */
		BANDOS_SPIRITUAL_MAGE(900, 1, SpellBook.MODERN_MAGICS,
				SpellType.COMBAT, new Item[]{}, "bandosSpiritualMage", null),
		TELE_BLOCK(61, 85, SpellBook.MODERN_MAGICS, SpellType.COMBAT, new Item[]{
				new Item(CHAOS_RUNE, 1), new Item(DEATH_RUNE, 1),
				new Item(LAW_RUNE, 1)}, "teleBlock", null);
		...
		public static Spell forId(int spellId, SpellBook spellBook) {
			for (Spell spell : spells) {
				if (spell.getSpellBook().getSpellBookId() == spellBook
						.getSpellBookId() && spell.getSpellId() == spellId) {
					return spell;
				}
			}
			return null;
		 ...
		 * @return
		 */
		private Spell(int id, int levelRequired, SpellBook spellBook,
					  SpellType spellType, Item[] runes, String spellName,
					  Item requiredItem) {
			this.id = id;
	...
	public static enum SpellBook {
		 ...
		 * @return The spell book, or <code>null</code> if the id is not a spell
		 * book.
		 */
