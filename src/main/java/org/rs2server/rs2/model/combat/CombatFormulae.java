...
public final class CombatFormulae {
	...
	public static int calculateMeleeMaxHit(Mob mob, Mob victim, boolean special) {
		...
		double otherBonusMultiplier = 1; //TODO: void melee = 1.2, salve amulet = 1.15, salve amulet(e) = 1.2
		...
		if (fullDharok(mob)) {
            double dharokMultiplier = ((1 - ((float) mob.getSkills().getLevel(Skills.HITPOINTS) / (float) mob.getSkills().getLevelForExperience(Skills.HITPOINTS)) * 1.7)) + 1;
	...
	public static boolean hasBerserkerNecklaceBonus(Mob mob) {
		return mob.getEquipment().contains(11128) && (mob.getEquipment().contains(6523) || mob.getEquipment().contains(6528)
	...
	public static boolean hasBlackMaskOrSlayerHelm(Mob mob) {
		return mob.getEquipment() != null
				&& (mob.getEquipment().contains(8901) || mob.getEquipment().contains(11864) || mob.getEquipment().contains(11865) || mob.getEquipment().contains(19639) || mob.getEquipment().contains(19641)
						|| mob.getEquipment().contains(19643) || mob.getEquipment().contains(19645) || mob.getEquipment().contains(19647) || mob.getEquipment().contains(19649) || mob.getEquipment().contains(21264) 
						|| mob.getEquipment().contains(21266));
	}

	public static boolean hasImbuedSlayerHelm(Mob mob) {
		return mob.getEquipment() != null && mob.getEquipment().contains(11865);
	}
	...
	public static boolean fullAhrimDamned(Mob mob) {
		return mob.getEquipment() != null
				...
				&& mob.getEquipment().contains(4712)
				&& mob.getEquipment().contains(4714)
				&& mob.getEquipment().contains(12851);
	}
	...
	public static double dragonfireReduction(Mob mob) {
		boolean dragonfireShield = mob.getEquipment() != null && (mob.getEquipment().contains(1540) || mob.getEquipment().contains(11283) || mob.getEquipment().contains(11284) || mob.getEquipment().contains(2890) || mob.getEquipment().contains(20714) || mob.getEquipment().contains(11285));
	...
	public static int getCombatCooldownDelay(Mob mob) {
		...
		if(getActiveCombatAction(mob) == RangeCombatAction.getAction()) {
			if(mob.getCombatState().getCombatStyle() != CombatStyle.AGGRESSIVE_1 || (mob.getEquipment().contains(12926) && (mob.getInteractingEntity() != null && mob.getInteractingEntity() instanceof Player))) {
				/**
		...
		return (int) ((mob.getEquipment() != null && mob.getEquipment().get(3) != null) ? mob.getEquipment().get(3).getEquipmentDefinition().getSpeed() + extra : 4);
	}
	...
	public static boolean hasAccumulator(Mob attacker) {
		return attacker.getEquipment().get(Equipment.SLOT_CAPE) != null && (attacker.getEquipment().get(Equipment.SLOT_CAPE).getId() == 10499
