...
public class VasaNistirio extends AbstractCombatAction {
	
	/**
	 * The singleton instance.
	 */
	private static final VasaNistirio INSTANCE = new VasaNistirio();
	...
	public void hit(final Mob attacker, final Mob victim) {
		...
		switch(style) {
		case RANGE:
			attacker.playGraphics(Graphic.create(1329, 0, 0));
			hitDelay = 2;
			...
			for(final Player near : localPlayers) {
				if(near != null && near != attacker && near.getSkills().getLevel(Skills.HITPOINTS) > 0) {
					if (attacker.getCentreLocation().isWithinDistance(attacker, near, 10)) {
						...
						attacker.playProjectile(Projectile.create(attacker.getCentreLocation(), near.getCentreLocation(), 1330, 45, 50, rClientSpeed, 43, 35, near.getProjectileLockonIndex(), 10, 48));
		...
		case MAGIC:
			...
			blockAnimation = false;
			maxHit = 30;
			final Collection<Player> localPlayers1 = World.getWorld().getRegionManager().getLocalPlayers(attacker);
			for(final Player near : localPlayers1) {
				if(near != null && near != attacker && near.getSkills().getLevel(Skills.HITPOINTS) > 0) {
					if (attacker.getCentreLocation().isWithinDistance(attacker, near, 10)) {
						int rClientSpeed;
						if(attacker.getLocation().isWithinDistance(attacker, near, 1)) {
						...
						attacker.playProjectile(Projectile.create(attacker.getCentreLocation(), near.getCentreLocation(), 1202, 45, 50, rClientSpeed, 43, 35, near.getProjectileLockonIndex(), 10, 48));
				...
				public void execute() {
					...
					for(final Player near : localPlayers1) {
						if(near != null && near != attacker && near != victim && near.getSkills().getLevel(Skills.HITPOINTS) > 0) {
							if(attacker.getCentreLocation().isWithinDistance(attacker, near, 10)) {
								int hitz = MagicCombatAction.getAction().damage(maxHit, attacker, near, AttackType.MAGIC, Skills.MAGIC, Prayers.PROTECT_FROM_MISSILES, false, false);
			...
			randomHit = MagicCombatAction.getAction().damage(maxHit, attacker, victim, AttackType.MAGIC, Skills.MAGIC, Prayers.PROTECT_FROM_MAGIC, false, false);
