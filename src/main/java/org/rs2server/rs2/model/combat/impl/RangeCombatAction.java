...
public class RangeCombatAction extends AbstractCombatAction {
                    ...
                    if (arrows == null) {
                        attacker.getActionSender().sendMessage("There are no " +
                                ((bowType == BowType.KARILS_XBOW || bowType == BowType.BRONZE_CBOW || bowType == BowType.IRON_CBOW || bowType == BowType.STEEL_CBOW
                                        || bowType == BowType.MITH_CBOW || bowType == BowType.ADAMANT_CBOW || bowType == BowType.RUNE_CBOW || bowType == BowType.DRAGON_CBOW || bowType == BowType.DRAGON_CBOW) ? "bolts" : "arrows")
                                + " left in your quiver.");
                        return false;
                    }

                    if (bowType == BowType.TWISTED_BOW) {
    ...
    public void hit(final Mob attacker, final Mob victim) {
        ...
        if (pullback == null && projectile == -1 && attacker.getEquipment() != null) {
            ...
            if (attacker.isPlayer() && weapon.getId() == 12926) {// toxic blowpipe swag system
                Player player = (Player) attacker;
                ...
                if (charges > 0 && chargedWith > 0) {
                    ...
                    if (rangeWeaponType != null) {
                        attacker.getCombatState().setBonus(12, (int) (65 + (weaponEquipDef.getBonus(12) + chargeDef.getBonus(12) * 1.8)));// ranged strength of toxic blowpipe + ranged strength of dart
        ...
        if (!special || (bowType == BowType.BRONZE_CBOW || bowType == BowType.IRON_CBOW || bowType == BowType.STEEL_CBOW
                || bowType == BowType.MITH_CBOW || bowType == BowType.ADAMANT_CBOW || bowType == BowType.RUNE_CBOW || bowType == BowType.DRAGON_CBOW)) {
            if (pullback != null) {
            ...
            if (rangeWeaponType != null) {
                ...
                if (bowType == BowType.KARILS_XBOW || bowType == BowType.BRONZE_CBOW || bowType == BowType.IRON_CBOW || bowType == BowType.STEEL_CBOW || bowType == BowType.MITH_CBOW
                        || bowType == BowType.ADAMANT_CBOW || bowType == BowType.RUNE_CBOW || bowType == BowType.DRAGON_CBOW) {
                    clientSpeed = 55;
                    showDelay = 45;
                    slope = 5;
                } else {
            ...
            if (bowType == BowType.DORGESHUUN_CBOW) {
                clientSpeed = 55;
                showDelay = 45;
                slope = 5;
            } else {
            	if (ammunition != null && ammunition.getId() != 8882) {
                int distance = attacker.getLocation().distanceToEntity(attacker, victim);
                clientSpeed = 55 + (distance * 5);
                if (distance > 2) {
                    hitDelay += 1;
            ...
            if (bowType == BowType.TWISTED_BOW) {
                hitDelay += 1;
                clientSpeed += 15;
                attacker.playProjectile(Projectile.create(attacker.getLocation(), victim, projectile, showDelay, 50, clientSpeed - 10, 41, 31, slope - 6, 64));
                attacker.playProjectile(Projectile.create(attacker.getLocation(), victim, projectile, showDelay, 50, clientSpeed, 46, 36, slope + 3, 64));
            } else {
                attacker.playProjectile(Projectile.create(attacker.getLocation(), victim, projectile, showDelay, 50, clientSpeed, 38, 36, slope, 64));
            }
            ...
            if (bowType == BowType.TWISTED_BOW) {
                ...
                if (ammunition != null && ammunition.getId() != 11212) {
                    ...
                    slope = 15;
                    clientSpeed += 30;
                    attacker.playProjectile(Projectile.create(attacker.getLocation(), victim, projectile, showDelay, 50, clientSpeed, 41, 31, 3, 36));
                    attacker.playProjectile(Projectile.create(attacker.getLocation(), victim, projectile, showDelay, 50, clientSpeed + 10, 46, 36, slope + 6, 36));
                }
            }
        ...
        int maxHit = CombatFormulae.calculateRangeMaxHit(attacker, victim, special);
        ...
        if (weapon != null && weapon.getId() == 12926 && attacker.isPlayer()) {
            ...
            if (special) {
                ...
                attacker.playProjectile(Projectile.create(attacker.getLocation(), victim, projectile, 20, 50, 35, 38, 36, 13, 64));
                randomHit *= 1.5;
            ...
            if (bowType == BowType.BRONZE_CBOW || bowType == BowType.IRON_CBOW || bowType == BowType.STEEL_CBOW
                    || bowType == BowType.MITH_CBOW || bowType == BowType.ADAMANT_CBOW || bowType == BowType.RUNE_CBOW || bowType == BowType.DRAGON_CBOW || bowType == BowType.ARMADYL_CBOW) {
                if (random.nextInt(10) == 1) {
                    switch (ammunition.getId()) {
                        case 9244:
                            boolean fire = true;
                            Item shield = victim.getEquipment().get(Equipment.SLOT_SHIELD);
                            if (shield != null && (shield.getId() == 11283 || shield.getId() == 1540 || shield.getId () == 20714 || shield.getId () == 2890)) {
                                fire = false;
                            }
                            if (fire) {
								if (bowType != BowType.ARMADYL_CBOW) {
									special = true;
								}
                                ...
                                double protectionPrayer = victim.getCombatState().getPrayer(Prayers.PROTECT_FROM_MISSILES) ? 0.40 : 1;
                                if (protectionPrayer != 1) {
                                    double protectionHit = randomHit * protectionPrayer; // +1 as its exclusive
                        ...
                        case 9245:
                            ...
                            break;
                        case 9242:
                            int selfDamage = (int) (attacker.getSkills().getLevel(Skills.HITPOINTS) * 0.01);
            ...
            } else if (bowType == BowType.TWISTED_BOW) {
                if (special) {
        ...
        if (CombatFormulae.hasAccumulator(attacker)) {
        ...
        if (weapon != null && weapon.getId() != 12926 && bowType != BowType.CRYSTAL_BOW && ammunition != null && !avaEffect) {
        ...
        World.getWorld().submit(new Tickable(hitDelay) {
            @Override
            public void execute() {
				/* Double hitsplat with half damage if the attacker has full karil's and amulet of the damned */
				if (random.nextFloat() <= 0.25f && CombatFormulae.fullKaril(attacker) && CombatFormulae.hasAmuletOfTheDamned(attacker)) {
                ...
                if (finalAmmunition != null) {
                    ...
                    if (r >= finalDropRate) {
                        ...
                        if (victim.isNPC()) {
                            NPC n = (NPC) victim;
                            if (n.getId() == 1101 || n instanceof Zulrah || n instanceof Kraken || n instanceof Whirlpool) {
                                dropUnder = true;
                            }
                        }
                        World.getWorld().createGroundItem(new GroundItem(attacker.getUndefinedName(), new Item(finalAmmunition.getId(), 1), dropUnder ? attacker.getLocation() : victim.getLocation()), attacker.isPlayer() ? ((Player) attacker) : null);
                    }
                }
        ...
        if (special || bowType == BowType.TWISTED_BOW) {
    ...
    public void addExperience(Mob attacker, int damage) {
        ...
        if (attacker.getCombatState().getCombatStyle() == CombatStyle.DEFENSIVE) {
            //Longrange
            attacker.getSkills().addExperience(Skills.RANGE, (damage) * 4 * Constants.COMBAT_EXP * 1);
            attacker.getSkills().addExperience(Skills.DEFENCE, (damage) * 4 * Constants.COMBAT_EXP * 1);
        } else {
            attacker.getSkills().addExperience(Skills.RANGE, (damage) * 4 * Constants.COMBAT_EXP * 1);
        }
    }
    ...
    public static enum BowType {
        ...
        OAK_SHORTBOW(new ArrowType[]{ArrowType.BRONZE_ARROW, ArrowType.IRON_ARROW, ArrowType.STEEL_ARROW},
                new int[]{7, 7, 9}),
        
        TWISTED_BOW(new ArrowType[]{ArrowType.RUNE_ARROW, ArrowType.DRAGON_ARROW},
                new int[]{10, 10, 10}),
        ...
        MAPLE_LONGBOW(new ArrowType[]{ArrowType.BRONZE_ARROW, ArrowType.IRON_ARROW,
                ArrowType.STEEL_ARROW, ArrowType.MITHRIL_ARROW, ArrowType.ADAMANT_ARROW},
                new int[]{10, 10, 10}),
        ...
        MAGIC_SHORTBOW(new ArrowType[]{ArrowType.BRONZE_ARROW, ArrowType.IRON_ARROW,
                ...
                ArrowType.ADAMANT_ARROW, ArrowType.RUNE_ARROW},
                new int[]{7, 7, 9}),
		THIRD_AGE_BOW(new ArrowType[]{ArrowType.BRONZE_ARROW, ArrowType.IRON_ARROW,
				ArrowType.STEEL_ARROW, ArrowType.MITHRIL_ARROW,
				ArrowType.ADAMANT_ARROW, ArrowType.RUNE_ARROW},
				new int[]{7, 7, 9}),
        ...
        HEAVY_BALLISTA(new ArrowType[]{ ArrowType.BRONZE_JAVELIN, ArrowType.IRON_JAVELIN,
		ArrowType.STEEL_JAVELIN, ArrowType.MITHRIL_JAVELIN, ArrowType.ADAMANT_JAVELIN,
		ArrowType.RUNE_JAVELIN, ArrowType.DRAGON_JAVELIN},
                new int[]{7, 7, 9}),
        ...
        DORGESHUUN_CBOW(new ArrowType[]{ArrowType.BONE_BOLT},
                new int[]{7, 7, 9}),
        ...
        RUNE_CBOW(new ArrowType[]{ArrowType.BRONZE_BOLT, ArrowType.IRON_BOLT,
                ArrowType.STEEL_BOLT, ArrowType.MITHRIL_BOLT,
                ArrowType.ADAMANT_BOLT, ArrowType.RUNE_BOLT, ArrowType.BROAD_BOLT},
                new int[]{7, 7, 9}),
        
        DRAGON_CBOW(new ArrowType[] {ArrowType.BRONZE_BOLT, ArrowType.IRON_BOLT,
                ArrowType.STEEL_BOLT, ArrowType.MITHRIL_BOLT,
                ArrowType.ADAMANT_BOLT, ArrowType.RUNE_BOLT, ArrowType.BROAD_BOLT},
                new int[]{7, 7, 9}),
        
        ARMADYL_CBOW(new ArrowType[]{ArrowType.BRONZE_BOLT, ArrowType.IRON_BOLT,
                ArrowType.STEEL_BOLT, ArrowType.MITHRIL_BOLT,
                ArrowType.ADAMANT_BOLT, ArrowType.RUNE_BOLT, ArrowType.BROAD_BOLT},
                new int[]{7, 7, 9}),;
        ;
    ...
    public static enum ArrowType {
        ...
        BRONZE_JAVELIN(0.75, null, 200),
        
        IRON_JAVELIN(0.7, null, 201),
        
        STEEL_JAVELIN(0.65, null, 202),
        
        MITHRIL_JAVELIN(0.6, null, 203),
        
        ADAMANT_JAVELIN(0.5, null, 204),
        
        RUNE_JAVELIN(0.4, null, 205),
        
        DRAGON_JAVELIN(0.3, null, 1301),
