...
public abstract class AbstractCombatAction implements CombatAction {
	...
	private static final int SLAYER_HELM_i = 11865;
	private static final int MIRROR_SHIELD = 4156;
	private static final int NOSE_PEG = 4168;
	private static final int FACE_MASK = 4164;
	private static final int WITCHWOOD_ICON = 8923;
	...
	public boolean canHit(Mob attacker, Mob victim, boolean messages, boolean cannon) {
		...
		if (attacker.isPlayer() && victim.isPlayer() && BoundaryManager.isWithinBoundaryNoZ(attacker.getLocation(), "ClanWarsFFA") && BoundaryManager.isWithinBoundaryNoZ(victim.getLocation(), "ClanWarsFFA")) {
			return true;
		}
		...
		if (victim.isNPC() && attacker.isPlayer()) {
			...
			if (BoundaryManager.isWithinBoundaryNoZ(attacker.getLocation(), "StrongholdCave"))
			{
				if(!attacker.getSlayer().getSlayerTask().getName().contains(n.getDefinition().getName()))
				{
					...
					attacker.getInterfaceState().setNextDialogueId(0, -1);
					return false;
				}
		...
		if ((attacker.isPlayer() && !attacker.isInWilderness() && victim.isPlayer()) || (victim.isPlayer() && !victim.isInWilderness() && attacker.isPlayer())) {
			attacker.getActionSender().sendMessage("You and your target must both be in the wilderness.");
			return false;
		}
		if (attacker.isNPC()) {
			NPC n = (NPC) attacker;
		...
		boolean bhTarget = false, bhTarget2 = false;
		boolean bh = false, bh2 = false;
		if (attacker.isPlayer()) {
			Player pAttacker = (Player) attacker;
			bh = pAttacker.getBounty() != null;
			if (pAttacker.getBounty() != null && pAttacker.getBountyTarget() != null && pAttacker.getBountyTarget() == victim) {
				bhTarget = true;
			}
		}
		if (victim.isPlayer()) {
			...
			if (pVictim.getBounty() != null && pVictim.getBountyTarget() != null && pVictim.getBountyTarget() == attacker) {
				bhTarget = true;
			}
		}
		...
		if (bh && bh2) {
			if (!bhTarget) {
				if (attacker.getCombatState().getLastHitTimer() > (System.currentTimeMillis() + 4000)) { //10 cycles for tagging timer
					if (attacker.getCombatState().getLastHitBy() != null && victim != attacker.getCombatState().getLastHitBy()) {
						if (attacker.getCombatState().getLastAttackerWith() == victim && System.currentTimeMillis() - attacker.getCombatState().getLastCombatWith() < 6000) {
							return true;
						}
				...
				if (victim.getCombatState().getLastHitTimer() > (System.currentTimeMillis() + 4000)) { //10 cycles for tagging timer
					if (victim.getCombatState().getLastHitBy() != null && attacker != victim.getCombatState().getLastHitBy()) {
						if (victim.getCombatState().getLastAttackerWith() == attacker && System.currentTimeMillis() - victim.getCombatState().getLastCombatWith() < 6000) {
			...
			if (requires && (!BoundaryManager.isWithinBoundaryNoZ(attacker.getLocation(), "MultiCombat") || !BoundaryManager.isWithinBoundaryNoZ(victim.getLocation(), "MultiCombat")) && (!attacker.inMulti()) || !victim.inMulti()) {
	...
	public void hit(final Mob attacker, Mob victim) {
		...
		if (!BoundaryManager.isWithinBoundaryNoZ(attacker.getLocation(), "ClanWarsFFAFull") && !BoundaryManager.isWithinBoundaryNoZ(victim.getLocation(), "ClanWarsFFAFull") && !attacker.getCombatState().getDamageMap().getTotalDamages().containsKey(victim) && attacker.isPlayer() && victim.isPlayer()) {
		...
		if(victim.isPlayer() && attacker.isNPC())
		{
			...
			if(n.getCombatDefinition() != null)
			{
				...
				case 499: //thermo
					...
					{
						p.getSkills().slayerDecreaseSkills(49);
					}
					break;
				case 417: //basilisk
				case 419: //cockatrice
					if(p.getEquipment().contains(MIRROR_SHIELD))
					{
				...
				case 3209: //cave horror
					...
					{
						p.getSkills().slayerDecreaseSkills(17);
					}
					break;
				}
			}
		...
		if(victim.isNPC() && attacker.isPlayer()) 
		{
			...
			if (n.getCombatDefinition() != null && attacker.getSkills().getLevelForExperience(Skills.SLAYER) < n.getCombatDefinition().getSlayerLevelReq()) {///my swag code.
				attacker.getActionSender().sendMessage("You need a slayer level of " + n.getCombatDefinition().getSlayerLevelReq() + " to wound this creature.");
				n.setCanBeDamaged(false);
			} else {
			...
			if(n.getId() == 410|| n.getId () == 427 || n.getId() == 7405)
			{
				...
				{
					p.sendMessage("Your weapon fails to pierce the hide of this creature.");
					n.setCanBeDamaged(false);
				}
			}
		}
		...
		for (final Item item : attacker.getEquipment().getItems()) {
			...
			if (degradable != null && degradable.getDegradationStrategy() == ItemServiceImpl.DegradationStrategy.ON_ATTACK) {
		...
		for (final Item item : victim.getEquipment().getItems()) {
			...
			final ItemServiceImpl.Degradable degradable = ItemServiceImpl.Degradable.forItem(item);
			if (degradable != null && degradable.getDegradationStrategy() == ItemServiceImpl.DegradationStrategy.ON_DEFEND) {
				itemService.degradeItem(victim, item);
			}
		}
//		if (victim instanceof Whirlpool || victim instanceof Kraken) {
//			attacker.getActionQueue().clearAllActions();
//		}
//		if (attacker.isPlayer()) {
//			Player player = (Player) attacker;
//			for (Item item : attacker.getEquipment().toArray()) {
//				if (item == null) {
//					continue;
//				}
//				ItemServiceImpl.Degradable degradables = ItemServiceImpl.Degradable.forItem(item.getId());
//				if (degradables != null) {
//					ItemService itemDegradingService = Server.getInjector().getInstance(ItemService.class);
//					itemDegradingService.degradeItem(attacker, item);
//				}
//			}
//		}
	}
						...
						switch (weapon.getId()) {
							...
							case 11785:
								if (attacker.getEquipment().get(Equipment.SLOT_ARROWS).getCount() < 1) {
									attacker.getActionSender().sendMessage("You need atleast 1 bolt to perform this special.");
									attacker.getCombatState().setSpecial(false);
									return false;
								}
								break;
							case 12926:
								...
								if (attacker instanceof Player && service.getCharges((Player) attacker, weapon) < 1) {
									attacker.getActionSender().sendMessage("You need atleast 1 dart loaded in your blowpipe to perform this special.");
									attacker.getCombatState().setSpecial(false);
									return false;
								}
								break;
	...
	public void special(final Mob attacker, final Mob victim, int damage) {
		...
		if (bow == BowType.BRONZE_CBOW || bow == BowType.IRON_CBOW || bow == BowType.STEEL_CBOW
				|| bow == BowType.MITH_CBOW || bow == BowType.ADAMANT_CBOW || bow == BowType.RUNE_CBOW || bow == BowType.DRAGON_CBOW) {
		...
		switch (weapon.getId()) {
			...
			case 11235:
				...
				slope = 15;
				clientSpeed += 30;
				attacker.playProjectile(Projectile.create(attacker.getLocation(), victim, attacker.getEquipment().get(Equipment.SLOT_ARROWS).getId() == 11212 ? 1099 : 1101, showDelay, 50, clientSpeed - 10, 41, 31, 3, 36));
				attacker.playProjectile(Projectile.create(attacker.getLocation(), victim, attacker.getEquipment().get(Equipment.SLOT_ARROWS).getId() == 11212 ? 1099 : 1101, showDelay, 50, clientSpeed + 10, 46, 36, slope + 6, 36));
			...
			case 861:
				...
				attacker.playProjectile(Projectile.create(attacker.getLocation(), victim, 249, 30, 50, 40 + (distance * 5), 43, 35, 10, 36));
				attacker.playProjectile(Projectile.create(attacker.getLocation(), victim, 249, 60, 50, 65 + (distance * 5), 43, 35, 10, 36));
				break;
			case 11785:
				int d = attacker.getLocation().distanceToEntity(attacker, victim);
				attacker.playAnimation(Animation.create(4230));
				attacker.playProjectile(Projectile.create(attacker.getLocation(), victim, 301, 60, 50, 65 + (d * 5), 43, 35, 10, 36));
				break;
			case 805:
				...
				int count = 3;
				final int maxHit = CombatFormulae.calculateRangeMaxHit(attacker, victim, true);
				...
				if (BoundaryManager.isWithinBoundaryNoZ(attacker.getLocation(), "MultiCombat") || attacker.inMulti()) {
					...
					for (final Mob mob : closeMobs) {
						...
						lastMob.playProjectile(Projectile.create(lastMob.getLocation(), mob, 258, lastDelay, 50, lastSpeed, 43, 35, 13, 48));
	...
	public void special(Mob attacker, final Item item) {
		...
		switch (item.getId()) {
			case 4153:
				attacker.getCombatState().inverseSpecial();
				int damage = CombatFormulae.calculateMeleeMaxHit(attacker, attacker.getInteractingEntity(), true);
	...
	public int damage(int maxHit, Mob attacker, Mob victim, AttackType attackType, int skill, int prayer, boolean special, boolean ignorePrayers) {
		...
		if (victim.isPlayer()) {
			...
			if (victim.getInterfaceState().isInterfaceOpen(Equipment.SCREEN) || victim.getInterfaceState().isInterfaceOpen(Bank.BANK_INVENTORY_INTERFACE) || victim.getInterfaceState().isInterfaceOpen(Trade.TRADE_INVENTORY_INTERFACE) || victim.getInterfaceState().isInterfaceOpen(Shop.SHOP_INVENTORY_INTERFACE) || victim.getInterfaceState().isInterfaceOpen(PriceChecker.PRICE_INVENTORY_INTERFACE)) {
		...
		double attackCalc = attackBonus * (attacker.getSkills().getLevel(skill) * bonus); // +1 as its exclusive
		...
		if (skill == Skills.ATTACK) {
			...
			if (CombatFormulae.fullVoidMelee(attacker)) {
				attackCalc *= 1.10;
			}
			...
			if (attacker.getCombatState().getPrayer(Prayers.SHARP_EYE)) {
				...
				attackCalc *= 1.15;
			} else if (attacker.getCombatState().getPrayer(Prayers.RIGOUR)) {
				attackCalc *= 1.20;
			}
			
			if (CombatFormulae.fullVoidRange(attacker)) {
				attackCalc *= 1.10;
			}
		...
		if (victim.getCombatState().getPrayer(Prayers.THICK_SKIN)) {
			...
			defenceCalc *= 1.20;
				...
				victim.getCombatState().getPrayer(Prayers.RIGOUR) ||
		...
		if (hitSucceed < random.nextDouble()) {
			 ...
			 */
			int hit = maxHit;
			double protectionPrayer = (victim.getCombatState().getPrayer(prayer) && !veracEffect) ? (attacker.isNPC() ? 1.0 : 0.40) : 1;
			if (ignorePrayers) {
				protectionPrayer = 1;
			}// kk try
			if (protectionPrayer != 1 || (victim.getCombatState().getPrayer(prayer) && attacker.isNPC())) {
				...
				if ((!ignorePrayers && attacker.isNPC()) || attacker.isPlayer()) {
					double protectionHit = hit * protectionPrayer; // +1 as its exclusive
					hit -= protectionHit;
				}
				if (hit < 1)
					hit = 0;
			...
			if (victim.isPlayer()) {
				...
				if (necklace != null && necklace.getId() == 11090) {
					if ((victim.getSkills().getLevel(Skills.HITPOINTS) - hit) < victim
							.getSkills()
							.getLevelForExperience(Skills.HITPOINTS) * 0.20000000000000001D
							&& victim.getSkills().getLevel(Skills.HITPOINTS)
							- hit > 0) {
						victim.getEquipment().set(Equipment.SLOT_AMULET, null);
						victim.getActionSender().sendMessage("The Pheonix Necklace saves you but was destroyed in the process.");
						victim.getSkills().setLevel(Skills.HITPOINTS, (int) (victim.getSkills().getLevel(Skills.HITPOINTS) + victim.getSkills().getLevelForExperience(Skills.HITPOINTS) * 0.3D));
					}
				}
			}
			if (victim.isNPC()) {
				NPC n = (NPC) victim;
				if (n.getCombatDefinition() != null) {
					if (n.getCombatDefinition().isArenaNPC() && attacker.getCombatState().getAttackType() != AttackType.MAGIC) {
						hit = 0;
					}
					if (n.getId() == 2266 && attacker.getCombatState().getAttackType() != AttackType.RANGE || n.getId() == 2265 && attacker.getCombatState().getAttackType() == AttackType.RANGE || attacker.getCombatState().getAttackType() == AttackType.MAGIC) {
						hit = 0;
					}
					if (n.getId() == 1101) {
						boolean bolts = false;
						if (attacker.getCombatState().getAttackType() == AttackType.RANGE && attacker.getEquipment().get(Equipment.SLOT_ARROWS) != null) {
							...
							if (!bolts) {
								if (attacker.getActionSender() != null) {
									attacker.getActionSender().sendMessage("The bolts you are using seem to have no effect on the sea snake!");
								}
								hit = 0;
							}
						} else {
	...
	public void addExperience(Mob attacker, int damage) {
		attacker.getSkills().addExperience(Skills.HITPOINTS, (damage * 1/*.1*/) * Constants.COMBAT_EXP * 1);
	}
	...
	public void recoil(Mob attacker, Mob victim, int damage) {
		...
		if (random.nextFloat() <= 0.25f && CombatFormulae.fullDharok(attacker) && CombatFormulae.hasAmuletOfTheDamned(attacker) && damage > 0) {
