...
public class BarrowsCoffinObjectListener extends EventListener {
	...
	public boolean objectAction(final Player player, int objectId, GameObject gameObject, Location location, ClickOption option) {
		...
		if (player.getKilledBrothers().containsKey(coffinBrother.getNpcId()) && player.getKilledBrothers().get(coffinBrother.getNpcId())) {
			player.getActionSender().sendMessage("The sarcophagus is empty.");
			return false;
		}
		NPC brother = new NPC(coffinBrother.getNpcId(),
				coffinBrother.getSpawn(), player.getLocation(),
