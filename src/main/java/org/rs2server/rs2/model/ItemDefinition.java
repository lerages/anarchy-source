...
public final class ItemDefinition {
	...
	public static void init() throws IOException {
		Logger.getAnonymousLogger().info("Loading JSON item definitions...");

		Gson gson = new Gson();
		ItemDefinition[] definitions = gson.fromJson(new BufferedReader(new FileReader("data/json/item_definitions.json")), ItemDefinition[].class);
	...
	public static void loadExchangePrices() {
		try {
//			DataInputStream dat = new DataInputStream(new FileInputStream("./data/grand_exchange_prices.dat"));
//			while (true) {
...
//			}
			for (String string : FileUtilities.readFile("data/prices.txt")) {
...
//				def.storePrice = dat.readInt();
//			}
            for (String string : FileUtilities.readFile("data/alchprices.txt")) {
                int id = Integer.parseInt(string.split(":")[0]);
...
//				def.storePrice = dat.readInt();
//			}
            for (String string : FileUtilities.readFile("data/examines.txt")) {
                int id = Integer.parseInt(string.split(":")[0]);
	...
	public boolean isStackable() {
		return stackable || noted || org.rs2server.cache.format.CacheItemDefinition.get(id).stackable || org.rs2server.cache.format.CacheItemDefinition.get(id).isNoted();
	}
	...
	/**
	 * @param fullHat the fullHat to set
	 */
	 ...
	 */
	 ...
	 public boolean isTradable() {
		 if (noted) {
			 ...
			 if (unnoted.isTradable()) return true;
		 }
		 return tradable;
	 }
