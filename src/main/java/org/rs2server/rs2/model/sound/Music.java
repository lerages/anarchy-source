...
public class Music {
    ...
    public static void playMusic(Player player) {
            ...
            for (int i = 0; i < songs.length; i++) {
                    if (unlocked[songs[i].array] == false && which == songs[i]) {
                            //player.getActionSender().sendMessage("<col=992D2D>You have unlocked the song: "+songs[i].name);
                    		//player.getActionSender().sendMessage("<col=992D2D>You have unlocked a new music track: "+songs[i].name);
    ...
    public static boolean[] unlocked = new boolean[385]; 
   
    // name, swX, swY, neX, neY, song id, childId, arraySlot
    public static final Music[] songs = {
    	new Music("Pick & Shovel", 3710, 5630, 3770, 5690, 426, 100, 384),//426 Motherlode Mine
