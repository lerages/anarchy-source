...
public class PriceChecker {
	...
	public static void deposit(Player player, int slot, int id, int amount) 
	{
				...
				new Item(item.getId(), transferAmount), -1)) 
		{
			...
			player.totalPrice += item.getDefinition().getStorePrice() * transferAmount;
			updatePrice(player, player.totalPrice);
			/*if (player.priceList.get(slot) != null) {
				player.priceList.set(slot, item.getDefinition().getStorePrice());
			} else {
				player.priceList.add(item.getDefinition().getStorePrice());
			}*/
			
			//Creates an array of 28 objects that we can populate for price checker
			Object[] items = new Object[28];
			for (int i = 0; i < items.length; i++)
			{
				//Populates our array with items of type object if they aren't null
				Item it = player.getPriceChecker().get(i);
				items[i] = it != null ? it.getDefinition().getStorePrice() : 0;
//				params[i] = i < player.priceList.size() ? player.priceList.get(i) : 0;
			}
	...
	public static void withdraw(Player player, int slot, int id, int amount) 
	{
		...
		try {
			...
			if (player.getInventory().add(new Item(item.getId(), transferAmount), -1)) 
			{
				...
				{// try
					Item it = player.getPriceChecker().get(i);
					params[i] = it != null ? it.getDefinition().getStorePrice() : 0;
				}
	...
	{
		player.getActionSender().sendString(464, 12, "Total guide price:<col=ffffff> " + NumberFormat.getInstance().format(amount) + "</col>");
		//Misc.formatCurrency(amount) if you want K or M 
	}
