...
public class Bank {
	...
	public static void open(Player player) {
		...
		if (permissionService.is(player, PermissionService.PlayerPermissions.ULTIMATE_IRON_MAN)) {
			...
			player.getActionSender().sendDialogue("", ActionSender.DialogueType.MESSAGE_MODEL_LEFT, 12813, null, "You are an ultimate ironman and cannot use a bank.");
			return;
		}
		if (requestTime != null && requestedPin != null && requestTime.plus(Duration.standardDays(bankEntity.getRecoveryDelay())).isBefore(now)) {
		...
		player.getActionSender().sendString(BANK_INVENTORY_INTERFACE, 4, "The Bank of RuneScape");
	...
	public static void withdraw(Player player, int slot, int id, int amount) {
		...
		// now add it to inv
		if (newId != -1 && transferAmount > 0 && player.getInventory().add(new Item(newId, transferAmount),
				-1)) {
	 ...
	 * @param amount The amount of the item to deposit.
	 */
	public static void deposit(Player player, Container container, int slot,
							   int id, int amount, boolean fireItemsChanged) {
	 ...
	 *
	 * @param player The player.
	 * @param slot   The slot in the player's equipment.
	 * @param id     The item id.
	 * @param amount The amount of the item to deposit.
	 */
