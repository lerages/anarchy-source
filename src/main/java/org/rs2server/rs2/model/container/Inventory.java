...
public class Inventory {
	...
	public static int removeRune(Mob mob, Item item) {
		...
		if (item.getId() == 453 && mob.isPlayer()) {
			...
			if (player.getInventory().getCount(item.getId()) <= 0 && player.getDatabaseEntity().getCoalBagAmount() >= item.getCount()) {
				player.getDatabaseEntity().setCoalBagAmount(player.getDatabaseEntity().getCoalBagAmount() - item.getCount());
				return 0;
			}
		}
	...
	public static boolean ownsItem(Player player, int i) {
		return player.getInventory().getCount(i) > 0 || player.getBank().getCount(i) > 0 || player.getEquipment().getCount(i) > 0;
	}
