...
import org.rs2server.rs2.model.player.Banking;
...
public class Container implements Cloneable, Streamable {
    ...
    /**
     * Attemps to add a item into the next free slot ignoring stackable issues with the original method
     *
     * @param item
     * @return <code>true</code> if the item was added, <code>false</code> if
     *         not.
	...
	public int remove(Item itemToRemove, int preferredSlot, boolean allowZero) {
		int removed = 0;
		if ((itemToRemove.getDefinition().isStackable() || type.equals(Type.ALWAYS_STACK)) && !type.equals(Type.NEVER_STACK)) {
