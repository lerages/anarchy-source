...
public class WeaponContainerListener implements ContainerListener {
	...
	private void sendWeapon(int id, String originalName, String name, String genericName) {
		...
		} else if (genericName.contains("scimitar") || genericName.contains("butterfly net") || name.equals("excalibur") || name.equals("katana") || name.endsWith("light") || (genericName.contains("sword") && !name.contains("2h") && !name.contains("god") && !name.contains("saradomin"))) {
			switch (player.getCombatState().getCombatStyle()) {
			case ACCURATE:
			case CONTROLLED_1:
				player.getCombatState().setAttackType(AttackType.SLASH);
		...
		} else if (genericName.startsWith("maul") || genericName.endsWith("warhammer") || name.endsWith("hammers") || name.equalsIgnoreCase("tzhaar-ket-om") || name.equalsIgnoreCase("elder maul")) {
			switch (player.getCombatState().getCombatStyle()) {
			case ACCURATE:
			case CONTROLLED_1:
				player.getCombatState().setAttackType(AttackType.CRUSH);
