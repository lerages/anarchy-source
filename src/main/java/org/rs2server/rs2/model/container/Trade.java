...
//		.sendAccessMask(1026, 335, 27, 0, 28)
	...
	public static void offerItem(Player player, int slot, int id, int amount) {
		...
		if(!item.getDefinition().isTradable() && permissionService.isNot(player, PermissionService.PlayerPermissions.ADMINISTRATOR) && permissionService.isNot(partner, PermissionService.PlayerPermissions.ADMINISTRATOR)) {
		...
		try {
			...
			if(player.getTrade().add(new Item(item.getId(), transferAmount), -1)) {
				System.out.println("Removing " + transferAmount + " of " + item.getDefinition().getName());
