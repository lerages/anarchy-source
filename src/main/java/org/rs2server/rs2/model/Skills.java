...
public class Skills {
    ...
    public int getTotalExperienceint() {
        int total = 0;
        for (int i = 0; i < levels.length; i++) {
            total += getExperience(i);
        }
     ...
     * Decrements a level.
     *
     * @param skill The skill to decrementBoatTime.
     */
    ...
    public void slayerDecreaseSkills(int decrease_amount)
    {
    	for(int i = 0; i < SKILL_COUNT; i++)
		{
			if(i != 3)
			{
				if(getLevel(i) > decrease_amount)
					decreaseLevel(i, decrease_amount);
					else
					setLevel(i, 0);	
			}
    ...
    public void addExperience(int skill, double exp) {
        ...
        exps[skill] += exp * Constants.SKILL_EXP;
    ...
    public void increasePrayerPoints(double modification) {
        if (prayerPoints < getLevelForExperience(Skills.PRAYER)) {
            setPrayerPoints(prayerPoints + modification >= getLevelForExperience(Skills.PRAYER) ? getLevelForExperience(Skills.PRAYER) : prayerPoints + modification, true);
        }
    }
    ...
    public enum SkillCape {
		...
		ARDOUGNE_MAX_CAPE(-1, new Item(20760), new Item(20760), new Item(20760), Animation.create(7121), Graphic.create(1286), 9);
