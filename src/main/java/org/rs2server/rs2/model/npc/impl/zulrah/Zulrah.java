...
public final class Zulrah extends CombatNpc<Zulrah> {
    ...
    private static final int[] RANGE_BONUSES = {200, 200, 200, 200, 200, 200, 200, 200, -45, 80, 0, 180, 0};
    private static final int[] MELEE_BONUSES = {200, 200, 200, 200, 200, 200, 200, 200, 0, 350, 0, 200, 0};
    private static final int[] MAGE_BONUSES = {200, 200, 200, 200, 200, 200, 200, 200, 300, 0, 100, 0};
...
//		this.startTransforming = true;
        ...
        switch (patternIndex) {
            case 0:
                switch (patternStage) {
                    case 0: // Range
                        for (int i = 0; i < 4; i++)
                            attacks.add(new ZulrahAttack(ZulrahAttackType.SPAWN_VENOM, 3).setLocations(ZulrahBehavior.VENOM_CLOUD_LOCATIONS[i]));
                        break;
                    case 1: // Melee
                        for (int i = 0; i < 2; i++)
                    ...
                    case 3: // Range
                        ...
                        for (int i = 0; i < 2; i++) {
                            attacks.add(new ZulrahAttack(ZulrahAttackType.SPAWN_MINIONS, 3).setLocations(new Location[]{
                                    ZulrahBehavior.VENOM_CLOUD_LOCATIONS[3][i]
                            }));
                        }
                        attacks.add(new ZulrahAttack(ZulrahAttackType.SPAWN_VENOM, 3).setLocations(ZulrahBehavior.VENOM_CLOUD_LOCATIONS[1]));
                        attacks.add(new ZulrahAttack(ZulrahAttackType.SPAWN_VENOM, 3).setLocations(ZulrahBehavior.VENOM_CLOUD_LOCATIONS[0]));
                        attacks.add(new ZulrahAttack(ZulrahAttackType.SPAWN_VENOM, 3).setLocations(ZulrahBehavior.VENOM_CLOUD_LOCATIONS[3]));
                        for (int i = 0; i < 2; i++) {
                            attacks.add(new ZulrahAttack(ZulrahAttackType.SPAWN_MINIONS, 3).setLocations(new Location[]{ZulrahBehavior.VENOM_CLOUD_LOCATIONS[2][i]}));
                        }
                        break;
                    case 4: // Melee
                        for (int i = 0; i < 2; i++)
                    ...
                    case 6: // Range
                        for (int i = 0; i < 3; i++)
                            attacks.add(new ZulrahAttack(ZulrahAttackType.SPAWN_VENOM, 3).setLocations(ZulrahBehavior.VENOM_CLOUD_LOCATIONS[i]));
                        attacks.add(new ZulrahAttack(ZulrahAttackType.SPAWN_MINIONS, 3).setLocations(new Location[]{ZulrahBehavior.VENOM_CLOUD_LOCATIONS[2][0]}));
                        attacks.add(new ZulrahAttack(ZulrahAttackType.SPAWN_MINIONS, 3).setLocations(new Location[]{ZulrahBehavior.VENOM_CLOUD_LOCATIONS[2][0]}));
                        for (int i = 0; i < 2; i++)
                            attacks.add(new ZulrahAttack(ZulrahAttackType.SPAWN_MINIONS, 3).setLocations(new Location[]{ZulrahBehavior.VENOM_CLOUD_LOCATIONS[3][i]}));
                        break;
                    case 7:// Mage
                        for (int i = 0; i < 4; i++)
                            attacks.add(new ZulrahAttack(ZulrahAttackType.MAGE, 4));
            ...
            case 1:
                switch (patternStage) {
                    ...
                    case 3:
                        ...
                        attacks.add(new ZulrahAttack(ZulrahAttackType.SPAWN_VENOM, 3).setLocations(ZulrahBehavior.VENOM_CLOUD_LOCATIONS[3]));
                        attacks.add(new ZulrahAttack(ZulrahAttackType.SPAWN_VENOM, 3).setLocations(ZulrahBehavior.VENOM_CLOUD_LOCATIONS[1]));
                        attacks.add(new ZulrahAttack(ZulrahAttackType.SPAWN_MINIONS, 3).setLocations(new Location[]{ZulrahBehavior.VENOM_CLOUD_LOCATIONS[1][0]}));
                        attacks.add(new ZulrahAttack(ZulrahAttackType.SPAWN_MINIONS, 3).setLocations(new Location[]{ZulrahBehavior.VENOM_CLOUD_LOCATIONS[1][1]}));
                        attacks.add(new ZulrahAttack(ZulrahAttackType.SPAWN_MINIONS, 3).setLocations(new Location[]{ZulrahBehavior.VENOM_CLOUD_LOCATIONS[2][0]}));
                        attacks.add(new ZulrahAttack(ZulrahAttackType.SPAWN_MINIONS, 3).setLocations(new Location[]{ZulrahBehavior.VENOM_CLOUD_LOCATIONS[2][1]}));
                        break;
                    case 4:
                        attacks.add(new ZulrahAttack(ZulrahAttackType.MAGE, 4));
                        attacks.add(new ZulrahAttack(ZulrahAttackType.RANGE, 4));
        ...
        World.getWorld().submit(new Tickable(3) {
            ...
            public void execute() {
                ...
                switch (stage) {
                    case 1:
                        // Rebuild NPC list
                        setTeleportTarget(currentStage.getLocation().transform(0, 0, getZ()));
                        setTeleporting(true);
                        setTickDelay(2);
                        break;
                    case 2:
    ...
    private void facePlayer() {
        if (currentAttack != null && challenger != null && (currentAttack.getAttackType() == ZulrahAttackType.MELEE || currentAttack.getAttackType() == ZulrahAttackType.SPAWN_VENOM || currentAttack.getAttackType() == ZulrahAttackType.SPAWN_MINIONS))
            return;
    ...
    public final void dropLoot(final Mob mob) {
        ...
        String time = String.format("%02d:%02d", zulrahContent.getRecordedMinutes(), zulrahContent.getRecordedSeconds());
		...
		NPCLootTable.forID(this).getGeneratedLoot(chance).stream().filter(loot -> loot != null).forEach(loot -> {
			GroundItemDefinition g = new GroundItemDefinition(challenger.getUndefinedName(), challenger.getLocation(), loot.getItemID(), Misc.random(loot.getMinAmount(), loot.getMaxAmount()));
      ...
      /*  NPCLootTable.forID(this).getGeneratedLoot(chance).stream().
                .map(l -> new GroundItemDefinition(challenger.getUndefinedName(), challenger.getLocation(), l.getItemID(), Misc.random(l.getMinAmount(), l.getMaxAmount())))
                .forEach(g -> {
                    challenger.getActionSender().sendMessage("<col=880000><img=33>Drops: <col=880000>" + challenger.getName() + " has just received " + g.getCount() + "x " + CacheItemDefinition.get(g.getDefinition().isNoted() ? g.getId() - 1 : g.getId()).getName() + ".");
                    World.getWorld().createGroundItem(g, challenger);
                });*/
    }
