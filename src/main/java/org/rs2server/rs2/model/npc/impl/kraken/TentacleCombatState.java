 ...
 * @author Clank1337
 */
public class TentacleCombatState<T extends CombatNpc<?>> extends NpcCombatState<T> {

	private static final Animation ATTACK_ANIMATION = Animation.create(3618);

	public TentacleCombatState(T npc) {
		super(npc);
	}
	...
	private void magicAttack() {
		...
		if (player != null) {
			player.getCombatState().updateCombatWith(npc);
			...
			player.getActionSender().sendProjectile(npc.getCentreLocation(), player.getLocation(), 162, 45, 50, clientSpeed, 43, 35, 10, 48, player.getProjectileLockonIndex());
//			npc.playProjectile(Projectile.create(npc.getCentreLocation(), player.getLocation(), 162, 45, 50, clientSpeed, 43, 35, player.getProjectileLockonIndex(), 10, 48));
			int delay = (gfxDelay / 20) - 1;
			final int damage = Misc.random(npc.getDefaultCombatAction().damage(2, npc, player, CombatState.AttackType.MAGIC, Skills.MAGIC, Prayers.PROTECT_FROM_MAGIC, false, true));
	...
	@Override
	public int getId() {
		return KrakenServiceImpl.TENTACLE;
	}
