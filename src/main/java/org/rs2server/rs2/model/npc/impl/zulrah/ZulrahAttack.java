	...
	public void perform(Zulrah zulrah, Player challenger) {
		switch(attackType) {
			case SPAWN_VENOM:
                ...
                Arrays.stream(locations).forEach(l -> challenger.getActionSender().sendProjectile(zulrah.getCentreLocation(), l.transform(1, 1, zulrah.getZ()), 1045, 30, 50, 100, 60, 25, 10, 48, 0));
				World.getWorld().submit(new Tickable(3) {
					public void execute() {
						Arrays.stream(locations).map(l -> {
							GameObject venomCloud = new GameObject(l.transform(0, 0, zulrah.getZ()), ZulrahBehavior.VENOM_CLOUD, 10, 0, true);
			...
			case JAD_MAGE:
				...
				break;
			case SPAWN_MINIONS:
				Location spawn = locations[0].transform(2, 2, zulrah.getZ());
