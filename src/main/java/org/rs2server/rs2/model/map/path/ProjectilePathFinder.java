...
public class ProjectilePathFinder {
	...
	public static boolean clippedProjectile(Mob entity, Mob victim) {
		...
		while (currentTile != end) {
			...
			if (currentObject != null && currentObject.getLocation().getZ() == currentTile.getZ()) {
				if (nextObject != null) {
					if (!PrimitivePathFinder.canMove(currentTile, localDirection, 1, false) || !PrimitivePathFinder.canMove(nextTile, localDirectionInverse, 1, false))
						break;
				} else {
					if (!PrimitivePathFinder.canMove(currentTile, localDirection, 1, false) || !PrimitivePathFinder.canMove(nextTile, localDirectionInverse, 1, false))
						break;
				}
			} else if (nextObject != null && nextObject.getLocation().getZ() == nextTile.getZ()) {
				if (!PrimitivePathFinder.canMove(currentTile, localDirection, 1, false) || !PrimitivePathFinder.canMove(nextTile, localDirectionInverse, 1, false))
					break;
			}
			if (PrimitivePathFinder.canMove(currentTile, localDirection, 1, false) && PrimitivePathFinder.canMove(currentTile, localDirectionInverse, 1, false)) {
				currentTile = nextTile;
				continue;
			} else {
				boolean solid = (RegionClipping.getClippingMask(nextTile.getX(), nextTile.getY(), nextTile.getZ()) & 0x20000) != 0;
				boolean solid2 = (RegionClipping.getClippingMask(currentTile.getX(), currentTile.getY(), currentTile.getZ()) & 0x20000) != 0;
				if (!solid && !solid2) {
					currentTile = nextTile;
					continue;
				} else {
	...
	public static final int SOLID_FLAG = 256;
	 ...
	 public static boolean clearPath(Location loc, Location to) {
	        ...
	        for (int ii = 0; ii < max; ii++) {
	            ...
	            if (diffX < 0 && diffY < 0) {
	                if ((RegionClipping.getClippingMask(currentX + i - 1, currentY + i2 - 1, height) & SOLID_FLAG) != 0 || (RegionClipping.getClippingMask(currentX + i - 1, currentY + i2, height) & SOLID_FLAG) != 0
	                        || (RegionClipping.getClippingMask(currentX + i, currentY + i2 - 1, height) & SOLID_FLAG) != 0) {
	                    return false;
	                }
