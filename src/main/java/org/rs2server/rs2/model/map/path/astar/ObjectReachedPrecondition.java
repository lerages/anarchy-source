...
/**
 * A precondition which evaluates to true when the current x,y coordinate
 * in a path finding iteration is in reach of a target game object.
 *
 * @author tommo
 */
public class ObjectReachedPrecondition implements PathPrecondition {
	...
	public ObjectReachedPrecondition(final Mob mob, final GameObject gameObject) {
		...
		this.z = gameObject.getLocation().getZ();
	...
	public boolean targetReached(int currentX, int currentY, int destinationX, int destinationY) {
		if (type != 0) {
			if ((type < 5 || type == 10) && reachedWall(currentX, currentY,
					destinationX, destinationY, orientation, type - 1)) {
				return true;
			}
			if (type < 10 && reachedDecoration(currentX, currentY, destinationX,
					destinationY, type - 1, orientation)) {
				return true;
			}
		}

		if (width != 0 && length != 0 && reachedObject(currentX, currentY, destinationX, destinationY, width, length, surroundings)) {
			return true;
		}
	...
	public boolean reachedObject(int x, int y, int targetX, int targetY, int width, int height, int surroundings) {
		...
		// 1x1 objects should be interacted with from the basic cardinal directions (NESW) and not diagonal.
		if (width == 1 && height == 1) {
			// from the north
	...
	public boolean reachedWall(int initialX, int initialY, int finalX, int finalY, int orientation, int type) {
		...
		/*if (type == 9) {
			if (initialX == finalX && initialY == finalY + 1 && (clipping & ClippingFlag.BLOCK_SOUTH.getMask()) == 0) {
				return true;
			} else if (initialX == finalX && initialY == finalY - 1 && (clipping & ClippingFlag.BLOCK_NORTH.getMask()) == 0) {
				return true;
			} else if (initialX == finalX - 1 && initialY == finalY && (clipping & ClippingFlag.BLOCK_EAST.getMask()) == 0) {
				return true;
			} else if (initialX == finalX + 1 && initialY == finalY && (clipping & ClippingFlag.BLOCK_WEST.getMask()) == 0) {
				return true;
			}
		}*/
	...
	public boolean reachedDecoration(int initialY, int initialX, int finalX, int finalY, int type, int orientation) {
		...
		if (type == 6 || type == 7) {
			...
			if (orientation == GameObjectCardinality.NORTH.getFace()) {
				if (initialX == finalX + 1 && initialY == finalY && (clipping & ClippingFlag.BLOCK_WEST.getMask()) == 0) {
					return true;
				} else if (initialX == finalX && initialY == finalY - 1 && (clipping & ClippingFlag.BLOCK_NORTH.getMask()) == 0) {
					return true;
				}
			} else if (orientation == GameObjectCardinality.EAST.getFace()) {
				if (initialX == finalX - 1 && initialY == finalY && (clipping & ClippingFlag.BLOCK_EAST.getMask()) == 0) {
					return true;
				} else if (initialX == finalX && initialY == finalY - 1 && (clipping & ClippingFlag.BLOCK_NORTH.getMask()) == 0) {
					return true;
				}
				...
				if (initialX == finalX - 1 && initialY == finalY && (clipping & ClippingFlag.BLOCK_EAST.getMask()) == 0) {
					return true;
				} else if (initialX == finalX && initialY == finalY + 1 && (clipping & ClippingFlag.BLOCK_SOUTH.getMask()) == 0) {
					return true;
				}
			} else if (orientation == GameObjectCardinality.WEST.getFace()) {
				if (initialX == finalX + 1 && initialY == finalY && (clipping & ClippingFlag.BLOCK_WEST.getMask()) == 0) {
