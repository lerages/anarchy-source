...
public class ObjectPathFinder implements PathFinder{
	...
	public TilePath findPath(Mob mob, Location base, int srcX, int srcY, int dstX, int dstY, int z, int radius, boolean running, boolean ignoreLastStep, boolean moveNear) {
		...
		while (writePathPosition != readPosition) {
			...
			if (type != 0) {
				if ((type < 5 || type == 10) && reachedWall(curX, curY,
						dstX, dstY, orientation, type - 1)) {
					//System.out.println("Reached Wall");
					foundPath = true;
					break;
				}
				if (type < 10 && reachedDecoration(curX, curY, dstX,
						dstY, type - 1, orientation)) {
					foundPath = true;
					break;
				}
			}//sorry had 2 remove that not used

			if (width != 0 && length != 0 && reachedObject(curX, curY, dstX,
					dstY, length, surroundings, width)) {
				foundPath = true;
				break;
		...
		if (!foundPath) {
			...
			if (moveNear) {
					...
					for (int x = dstX - delta; x <= dstX + delta; x++) {
						for (int y = dstY - delta; y <= dstY + delta; y++) {
							if (x >= 0 && y >= 0 && x < 104 && y < 104
									&& cost[x][y] < highCost && RegionClipping.isPassable(location.getX() + x, location.getY() + y, location.getZ())) {
								foundPath = true;
								highCost = cost[x][y];
								curX = x;
								curY = y;
				...
				int thisCost = 100;
				int depth = 15;
				int xLength = width;//obj.getDefinition().getSizeX();// xlength should be width and ylength should be length (which is actually height?)
				int yLength = length;//obj.getDefinition().getSizeY(); out of curiosity lets do getSizeY() and getSizeX() here again but try the tree see if its fixed.
				for (int x = dstX - depth; x <= dstX + depth; x++) {
					for (int y = dstY - depth; y <= dstY + depth; y++) {
						if (x >= 0 && y >= 0 && x < 104 && y < 104 && cost[x][y] < 100) {
							...
							if (y < dstY)
								diffY = dstY - y;
							else if (y > dstY + yLength - 1)//i was trying to implement his fix in here
								diffY = y - (dstY + yLength - 1);
							if (!RegionClipping.isPassable(location.getX() + x, location.getY() + y, obj.getZ())) {
								continue;
							}
							int totalCost = diffX * diffX + diffY * diffY;//yes.. lol -.- the regionclipping was the fix for most
			...
			/*	if (fullCost == 1000) {
					//System.out.println("Shit mothafucka.");
					return state;
				}*/
			}
			if (!foundPath) {
