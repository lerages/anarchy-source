...
/**
 * A set of preconditions for when trying to interact with an npc. Currently only used for the packet we call 'trade'.
 *
 * @author Twelve
 */
public class NpcReachedPrecondition implements PathPrecondition {
	...
	public boolean targetReached(int currentX, int currentY, int destinationX, int destinationY) {
		final int flags = RegionClipping.getClippingMask(currentX, currentY, npc.getZ());
		...
		if (distance == 2) {
			...
			final List<GameObject> objects = npc.getRegion().getGameObjects().stream()
					.filter(this::canInteractThrough)
					.filter(o -> o.getLocation().distance(destinationX, destinationY, npc.getZ()) == 1)
					.collect(toList());
			...
			Optional<GameObject> objectOption = objects.stream().filter(o -> {
				...
				return o.getX() == (int) (currentX + cardinality.getFaceVector().getX()) && o.getY() == (int) (currentY + cardinality.getFaceVector().getY());
			}).findFirst();
			...
			if (objectOption.isPresent()) {
				...
				if (object.getX() + cardinality.getFaceVector().getX() == destinationX && object.getY() + cardinality.getFaceVector().getY() == destinationY) {
					return true;
				}
			}
		}
	...
	private boolean canInteractThrough(GameObject object) {
		return object.getDefinition().getName().contains("Bank booth")
				|| object.getId() == 24106; // Bar (falador bartender)
	}
