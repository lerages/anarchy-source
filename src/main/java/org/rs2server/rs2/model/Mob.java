...
public abstract class Mob extends Entity {
	...
	private static final Location NPC_HOME_LOCATION = Location.create(0, 0, 0);
	...
	/**
	 * Checks if the mob is idle.
	 * If the mob is not interacting, is not moving, and has no pending actions, it is idling.
	 *
	...
	public void resetInteractingEntity() {
		if (this.getInteractingEntity() != null) {
			if (this.getInteractionMode() == InteractionMode.TALK && this.getInteractingEntity().getInteractionMode() == InteractionMode.TALK && this.getInteractingEntity().getInteractingEntity() == this) {
				this.getInteractingEntity().setInteractingEntity(null, null); //endGame an infinite for loop of each other cancelling
	...
	public void setTeleportTarget(Location teleportTarget) {
		...
		getLocalNPCs().stream().filter(n -> n.getInteractingEntity() != null && !(n.getInteractingEntity() instanceof Pet)).forEach(n -> {
		...
		getLocalPlayers().stream().filter(p -> p.getInteractingEntity() != null && !(p.getInteractingEntity() instanceof Pet)).forEach(p -> {
...
//                    player.getActionSender().removeInterfaces(player.getAttribute("tabmode"), tab);
		...
		if (attacker != null && (this instanceof PestControlNpc || this instanceof PestControlPortal)) {
			int hitsDealt = (int) (attacker.getAttribute("hits_dealt") == null ? 0 : attacker.getAttribute("hits_dealt"));
		...
		if (combatState.getPrayer(Prayers.REDEMPTION) && getSkills().getLevel(Skills.HITPOINTS) > 0) {
			if (getSkills().getLevel(Skills.HITPOINTS) < (getSkills().getLevelForExperience(Skills.HITPOINTS) * 0.10)) {
				...
				if (getActionSender() != null) {
					...
					getActionSender().playSound(Sound.NO_PRAYER_LEFT);
				}
				...
				getActionSender().playSound(Sound.REDEMPTION_USED);
			}
		}
		...
		if (attacker != null) {
			...
			if (!BoundaryManager.isWithinBoundaryNoZ(attacker.getLocation(), "ClanWarsFFAFull") && !BoundaryManager.isWithinBoundaryNoZ(getLocation(), "ClanWarsFFAFull")) {
				if (!attacker.getCombatState().getDamageMap().getTotalDamages().containsKey(this) && attacker.isPlayer() && isPlayer()) {
					...
					public void execute() {
						for (Mob mob : getRegion().getMobs()) {
							if (!mob.getCombatState().isDead() && mob.isInWilderness()) {
								...
								if (mob.getLocation().isNextTo(getLocation()) && !locationsUsed.contains(mob.getLocation()) && !(mob instanceof CerberusGhost)) {
	...
	public void addCoordinateAction(int width, int height, Location location, int otherWidth, int otherHeight, int distance, Action action, int id) {
		actionQueue.clearAllActions();
		/*if (getLocation().isWithinDistance(width, height, location, otherWidth, otherHeight, distance)) {
			actionQueue.addAction(action);
		} else {*/
		actionQueue.addAction(new CoordinateAction(this, width, height, location, otherWidth, otherHeight, distance, action, id));
		//}
	}

	public void addCoordinateAction(int width, int height, Location location, int otherWidth, int otherHeight, int distance, Action action, Fishing fishing) {
		actionQueue.clearAllActions();
		/*if (getLocation().isWithinDistance(width, height, location, otherWidth, otherHeight, distance)) {
			actionQueue.addAction(action);
		} else {*/
		actionQueue.addAction(new CoordinateAction(this, width, height, location, otherWidth, otherHeight, distance, action, fishing));
		//}
	}
	...
	public boolean isInWilderness() {
		...
		return (!(getLocation().getX() >= 3187 && getLocation().getX() <= 3194 && getLocation().getY() >= 3958 && getLocation().getY() <= 3962) &&
				(getLocation().getX() >= 2941 && getLocation().getX() <= 3392
				&& getLocation().getY() >= 3525 && getLocation().getY() <= 3967) || (getLocation()
				.getX() >= 2941
				&& getLocation().getX() <= 3392
				&& getLocation().getY() >= 9918 && getLocation().getY() <= 10366));
	}

	public boolean inMulti() {//TODO CONVERT THIS TO BOUNDARIES
		int absX = getLocation().getX();
		int absY = getLocation().getY();
		if (BoundaryManager.isWithinBoundaryNoZ(getLocation(), "MultiCombat_Bandos") || BoundaryManager.isWithinBoundaryNoZ(getLocation(), "MultiCombat_Zammy") || BoundaryManager.isWithinBoundaryNoZ(getLocation(), "MultiCombat_Armadyl") || BoundaryManager.isWithinBoundaryNoZ(getLocation(), "MultiCombat_Bandos") || BoundaryManager.isWithinBoundaryNoZ(getLocation(), "MultiCombat_Sara") || BoundaryManager.isWithinBoundaryNoZ(getLocation(), "RFD")) {
			return true;
		}
