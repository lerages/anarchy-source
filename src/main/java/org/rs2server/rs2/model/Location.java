...
//	public static Object createobj(int x, int y, int z) {
//		return new Location(x, y, z);
//	}
//	
//	public static String createstring(int x, int y, int z) {
//		return null;
//	}
	...
	public Location closestFreeTileOrSelf(Location from, int width, int height) {
		for (int x = -width; x <= width; x++) {
			for (int y = -height; y <= height; y++) {
				Location loc = Location.create(this.x + x, this.y + y, this.z);
				if (!RegionClipping.isPassable(loc.getX(), loc.getY(), loc.getZ())) {
					continue;
				}
				return loc;
			}
	...
	public Location closestTileToMob(Mob mob) {
		...
		if (size < 2 && size < 2) {//? i didnt add that lol it might be a bug thats causing this no were talking bout objects ye and objects call these methods...
			return mob.getLocation();
		}
		Location location = null;
		for (int x = 0; x < size; x++) {
			for (int y = 0; y < size; y++) {
				Location loc = Location.create(mob.getLocation().getX() + x, mob.getLocation().getY() + y, mob.getLocation().getZ());
	...
	public static boolean inWild(Location location) {
		return ((location.getX() >= 2941 && location.getX() <= 3392 && location.getY() >= 3525 && location.getY() <= 3967)
