...
public class WalkingQueue {
	...
	public void reset() {
		runQueue = false;
		waypoints.clear();
		waypoints.add(new Point(mob.getLocation().getX(), mob
				.getLocation().getY(), -1));
	}
	...
	public void addStep(int x, int y) {
		...
		/*
		 * The following code is unique to Rs2-Server, as I have never seen it fixed in another server.
		 * 
		 * As you know, on RuneScape, if you move a direction, and someone logs in or teleports to that area, when they
		 * are added, they are facing the correct direction. However, on every RSPS, they face south. I have experienced
		 * this when creating a video for example, having lots of friends facing north, relogging, and then they are all
		 * south, however this code fixes it.
		 */
		...
		if(newDirection == -1) { //We aren't moving left or right, so the direction will still be at -1
			if(diffY != 0) { //We are moving up or down
				//Bear in mind we do not have to recheck for diagonals, otherwise diffX would not of == 0 at the previous check
	...
	public void processNextMovement() {
		...
		if((mob.getLastKnownRegion().getRegionX()-mob.getLocation().getRegionX()) >= 4 || (mob.getLastKnownRegion().getRegionX()-mob.getLocation().getRegionX()) <= -4) {
			changed = true;
		}
		if((mob.getLastKnownRegion().getRegionY()-mob.getLocation().getRegionY()) >= 4 || (mob.getLastKnownRegion().getRegionY()-mob.getLocation().getRegionY()) <= -4) {
			changed = true;
		}
		if (changed) {
			/*
