				...
				for(int i : definitions.keySet()) {
					...
					switch(def.getCombatActionType()) {
						...
						case CUSTOM:
							...
							//File[] files = new File("./src/main/java/org/rs2server/rs2/model/combat/npcs/").listFiles();//SERIOUS? NOT EVEN DIRECTED TO BIN LUL

							String packageName = "org.rs2server.rs2.model.combat.npcs.";
							File[] files = new File("bin/org/rs2server/rs2/model/combat/npcs/").listFiles();//FIXED LARQUISHA
							...
							for (File f : files) {
								...
								String requiredName = packageName + NameUtils.formatName(npc.getName()).replace(" ", "").replace("'", "").replace("-", "");
...
//		XMLController.writeXML(defs, new File("data/newcombat.xml"));
	...
	/*
	 * 6247 2899 5265 0 NORTH_EAST true Sara
6248 2904 5261 0 SOUTH_EAST true Sara
6250 2905 5268 0 EAST true Sara
6252 2897 5271 0 SOUTH true Sara
	 */
