...
public class Cannon {
	...
	public Cannon(Player player, Location location) {
		...
		World.getWorld().submit(new Tickable(3) {
			...
			public void execute() {
				this.stop();
			if(player.getInventory().hasItem(new Item(8)))
				addPart(new Item(8));
			}
		...
		World.getWorld().submit(new Tickable(6) {
			...
			public void execute() {
				this.stop();
			if(player.getInventory().hasItem(new Item(10)))
				addPart(new Item(10));
			}
		...
		World.getWorld().submit(new Tickable(9) {
			...
			public void execute() {
				this.stop();
			if(player.getInventory().hasItem(new Item(12)))
				addPart(new Item(12));
			}
	...
	public void destroy() {
		...
		player.getActionSender().playSound(Sound.CANNON_DROP);
		player.playAnimation(Animation.create(827));
	...
	public void fire() {
		...
		runningTick = new Tickable(1) {
			...
			public void execute() {
				...
				player.playSound(Sound.CANNON_TURNING);
					...
					for(final NPC npc : r.getNpcs()) {
						...
						if(newDist <= 5 && newDist >= 1) {
							...
							if(player.getActiveCombatAction().canHit(player, npc, false, true)) {
								gameObject.playProjectile(Projectile.create(gameObject.getCentreLocation(), npc, 53, 15 + (delay * 10), 50, 50 + (newDist * 10), 37, 37, 0, 96));
								player.playSound(Sound.CANNON_FIRING);
								cannonBalls--;
								delay += 1;
								World.getWorld().submit(new Tickable(delay) {
									...
									public void execute() {
										...
										player.getSkills().addExperience(Skills.RANGE, (1 * damage) * Constants.COMBAT_EXP);
