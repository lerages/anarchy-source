...
public class HTTPServerHandler extends SimpleChannelUpstreamHandler {
	...
	private void handleHttpRequest(ChannelHandlerContext ctx, HttpRequest req) throws Exception {
		...
		String addr = ctx.getChannel().getLocalAddress().toString();



		if(req.getMethod() == GET) {
			if(req.headers().get(USER_AGENT).startsWith("Java")) {
					HttpResponse res = new DefaultHttpResponse(HTTP_1_1, OK);
					OutputStream out = new OutputStream();
					out.writeShort(WorldList.list.size());
					for(Iterator<Entry<Integer, WorldList>> it$ = WorldList.list.entrySet().iterator(); it$
							.hasNext();){
