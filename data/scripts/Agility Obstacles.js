...
function lavaDragonStep(player, obstacle, object) {
	if(player.getLocation().getY() == 3810) {
		y = -3;
		dir = 2;
	} else { 
		var y = 3;
	...
	var forceMovementVars =  [ 0, 0, 0, y, 33, 60, dir, 2 ];
	Agility.forceMovement(player, Animation.create(769), forceMovementVars, 1, true);
}

function taverlyFence(player, obstacle, object) 
{
	if(!player.getLocation().equals(Location.create(2936, 9810, 0)) && !player.getLocation().equals(Location.create(2934, 9810, 0))) 
	{
		player.removeAttribute("busy");
		return;
	}
	...
	Agility.forceMovement(player, Animation.create(2240), forceMovementVars, 1, true);
}

function slayerDungeonCrevice(player, obstacle, object) 
{
	if(!player.getLocation().equals(Location.create(2735, 10008, 0)) && !player.getLocation().equals(Location.create(2730, 10008, 0))) 
	{
		player.removeAttribute("busy");
		return;
	}
	...
	var xValue = 0;
	var face = 1;
	if (player.getLocation().getX() == 2735) {
		yValue = -4;
		yValue2 = -5;
		xValue = -1;
		face = 3;
...
//2585
function dorgRope1(player, obstacle, object) {
	if(player.getLocation().getY() != 5241) {
		player.removeAttribute("busy");
		return;
	}
	var dorgAgilityCourseLvl = player.getAttribute("dorgAgilityCourse");
	if(dorgAgilityCourseLvl != null) {
		player.setAttribute("dorgAgilityCourse", dorgAgilityCourseLvl + 1);
	}
	Agility.setRunningToggled(player, false, 12);
	Agility.forceWalkingQueue(player, Animation.create(762), 2721, 5234, 3, 10, false);
	Agility.forceWalkingQueue(player, Animation.create(762), 2721, 2131, 10, 3, true);
	player.getSkills().addExperience(Skills.AGILITY, obstacle.getExperience());
}
...
function draynorCrateJump(player, obstacle, object) {
	...
	if(player.getAttribute("draynorAgilityCourse") != null) {
		...
		if(courseLevel == 6) {
			player.getActionSender().sendMessage("You completed the course!");
			player.getSkills().addExperience(Skills.AGILITY, 40);
            var rand = Misc.random(1);
            if (rand == 0) {
                var amount = Misc.random(1, 4);
                if (player.getInventory().add(new Item(11849, amount))) {
                	player.getSkills().addExperience(Skills.AGILITY, 40);
                    player.getActionSender().sendMessage("You receive " + amount + "x Mark of grace.");
                }
            }
		}
...
function varrockWallClimb(player, obstacle, object) {
    if(player.getLocation().getX() != 3221) {
        player.removeAttribute("busy");
        return;
    }
    ...
    Agility.forceTeleport(player, Animation.create(737), Location.create(3219, 3414, 3), 0, 2);
    Agility.delayedAnimation(player, Animation.create(-1), 4);
    player.getSkills().addExperience(Skills.AGILITY, obstacle.getExperience());
}

function varrockClothesLine(player, obstacle, object) {
	if(player.getLocation().getY() != 3414) {
		player.removeAttribute("busy");
		return;
	}
	var varrockAgilityCourseLvl = player.getAttribute("varrockAgilityCourse");
	...
	Agility.setRunningToggled(player, false, 12);
	Agility.forceWalkingQueue(player, Animation.create(762), 3208, 3414, 3, 10, true);
	player.getSkills().addExperience(Skills.AGILITY, obstacle.getExperience());
}

function varrockLeapGap1(player, obstacle, object) {
	if(player.getLocation().getX() != 3201) {
		player.removeAttribute("busy");
		return;
	}
	var varrockAgilityCourseLvl = player.getAttribute("varrockAgilityCourse");
	...
	Agility.delayedAnimation(player, Animation.create(2588), 0);
	Agility.forceTeleport(player, Animation.create(-1), Location.create(3197, 3416, 1), 0, 0);
	player.getSkills().addExperience(Skills.AGILITY, obstacle.getExperience());
}

function varrockBalanceWall(player, obstacle, object) {
    if (player.getLocation().getX() != 3194) {
    	player.sendMessage("MEMERY")
        player.removeAttribute("busy");
        return;
    }
    var varrockAgilityCourseLvl = player.getAttribute("varrockAgilityCourse");
    ...
    Agility.setRunningToggled(player, false, 6);
    Agility.forceWalkingQueue(player, Animation.create(762), 3192, 3406, 3, 6, true);
    player.getSkills().addExperience(Skills.AGILITY, obstacle.getExperience());
}

function varrockLeapGap2(player, obstacle, object) {
	if(player.getLocation().getX() != 3195) {
		player.removeAttribute("busy");
		return;
	}
	var varrockAgilityCourseLvl = player.getAttribute("varrockAgilityCourse");
	...
	Agility.delayedAnimation(player, Animation.create(2588), 0);
	Agility.forceTeleport(player, Animation.create(-1), Location.create(3193, 3398, 3), 0, 0);
	player.getSkills().addExperience(Skills.AGILITY, obstacle.getExperience());
}

function varrockLeapGap3(player, obstacle, object) {
	if(player.getLocation().getX() != 3208) {
		player.removeAttribute("busy");
		return;
	}
	var varrockAgilityCourseLvl = player.getAttribute("varrockAgilityCourse");
...
function varrockLeapGap4(player, obstacle, object) {
	if(player.getLocation().getX() != 3232) {
		player.removeAttribute("busy");
		return;
	}
	var varrockAgilityCourseLvl = player.getAttribute("varrockAgilityCourse");
	...
	Agility.delayedAnimation(player, Animation.create(2588), 0);
	Agility.forceTeleport(player, Animation.create(-1), Location.create(3236, 3403, 3), 0, 0);
	player.getSkills().addExperience(Skills.AGILITY, obstacle.getExperience());
}

function varrockWallHurdle(player, obstacle, object) {
	...
	Agility.delayedAnimation(player, Animation.create(2588), 0);
	Agility.forceTeleport(player, Animation.create(-1), Location.create(3238, 3410, 3), 0, 0);
	player.getSkills().addExperience(Skills.AGILITY, obstacle.getExperience());
}

function varrockEdgeJump(player, obstacle, object) {
	if(player.getLocation().getY() != 3415) {
		player.removeAttribute("busy");
		return;
	}
	var varrockAgilityCourseLvl = player.getAttribute("varrockAgilityCourse");
	...
	Agility.delayedAnimation(player, Animation.create(2588), 0);
	Agility.forceTeleport(player, Animation.create(-1), Location.create(3237, 3417, 0), 0, 0);
	player.getSkills().addExperience(Skills.AGILITY, 60);
	player.getSkills().addExperience(Skills.AGILITY, obstacle.getExperience());
}
...
function ardyGapJump4(player, obstacle, object) {
    ...
    if(player.getAttribute("ardyAgilityCourse") != null) {
        ...
        if(courseLevel == 6) {
            ...
            var rand = Misc.random(1);
            if (rand == 0) {
                var amount = Misc.random(1, 8);
                if (player.getInventory().add(new Item(11849, amount))) {
                    player.getActionSender().sendMessage("You receive " + amount + "x Mark of grace.");
                    player.getSkills().addExperience(Skills.AGILITY, 80);
                }
            }
        }
...
function kharidTree(player, obstacle, object) {//1122, 1124, 2458
    if(player.getLocation().getY() != 3165) {
        player.removeAttribute("busy");
        return;
    }
    var kharidAgilityCourseLvl = player.getAttribute("kharidAgilityCourse");
